  ...
  try {
    ...
    await Promise.all(
      items.map(async (item) => {
        ...
        if (!key || !text) {
          failedKeys.push(String((item as Record<string, unknown>)?.key ?? '?'))
          return
        }
    ...
    return c.json(
      {
        ...
        deleted_csp_reports: deletedCspReports,
      },
      200,
    )
  } catch (e) {
...
admin.post('/migrate-character', async (c) => {
  try {
    ...
    const existing = (await db
      .prepare('SELECT id FROM characters WHERE kv_origin = ?')
      .bind(key)
      .first()) as { id: string } | null
    if (existing) {
      return c.json(
        { ok: false, error: `D1 row already exists for ${key}`, d1Id: existing.id },
        409,
      )
    }
    ...
    `,
      ...
      .bind(
        ...
        insert.faction_id,
        insert.behavior,
        insert.character_type,
        insert.character_class,
        insert.race,
        insert.background,
        insert.alignment,
        insert.conditions,
        ...
        insert.immunities,
        insert.known_spells,
        insert.prepared_spells,
        insert.cantrips_known,
        insert.currency,
        insert.resource_pools,
        insert.xp,
        insert.alias,
        insert.age,
        insert.gender,
        ...
        insert.weight_1,
        insert.weight_2,
        insert.perception_float,
        insert.thread_id,
        insert.state_stage,
        insert.state_stage_timer,
        insert.kv_origin,
        insert.current_room_id,
        insert.perception_bonus,
        insert.stealth_bonus,
        insert.origin,
        insert.created_at,
        insert.updated_at,
      )
...
admin.post('/relations', async (c) => {
  ...
  const attitude =
    body.attitude !== undefined && body.attitude !== null ? Number(body.attitude) : null
...
admin.patch('/relations/:id', async (c) => {
  ...
  try {
    const existing = await db
      .prepare('SELECT id FROM entity_relations WHERE id = ?')
      .bind(id)
      .first()
    ...
    await db
      .prepare(`UPDATE entity_relations SET ${setClauses} WHERE id = ?`)
      .bind(...values, id)
      .run()
...
admin.get('/export', async (c) => {
  try {
    ...
    return c.json(
      { ok: true, keys, exported_at: new Date().toISOString(), key_count: keys.length },
      200,
    )
  } catch (e) {
    console.error(`[admin] ${c.req.method} ${c.req.path}:`, e)
...
admin.post('/import', async (c) => {
  try {
    ...
    const items: unknown[] = Array.isArray((body as Record<string, unknown>)?.keys)
    ...
    return c.json(
      { ok: failedKeys.length === 0, imported, failed: failedKeys.length, failedKeys },
      200,
    )
  } catch (e) {
    console.error(`[admin] ${c.req.method} ${c.req.path}:`, e)
...
admin.post('/quests/:questId/milestones', async (c) => {
  try {
    ...
    const { title, notes, status, linked_entity_type, linked_entity_id, color, is_private } =
    ...
    const maxResult = (await db
      .prepare('SELECT MAX(sort_order) as max_order FROM quest_milestones WHERE quest_id = ?')
      .bind(questId)
...
admin.patch('/quests/:questId/milestones/:milestoneId', async (c) => {
  try {
    ...
    if (title !== undefined) {
      sets.push('title = ?')
      vals.push(title ? String(title).trim() : '')
    }
    if (notes !== undefined) {
      sets.push('notes = ?')
    ...
    if (linked_entity_type !== undefined) {
      sets.push('linked_entity_type = ?')
      vals.push(linked_entity_type ? String(linked_entity_type) : null)
    }
    if (linked_entity_id !== undefined) {
      sets.push('linked_entity_id = ?')
      vals.push(linked_entity_id ? String(linked_entity_id) : null)
    }
    if (color !== undefined) {
      sets.push('color = ?')
      vals.push(color ? String(color) : null)
    }
    if (is_private !== undefined) {
      sets.push('is_private = ?')
      vals.push(is_private ? 1 : 0)
    }
    ...
    await db
      .prepare(`UPDATE quest_milestones SET ${sets.join(', ')} WHERE id = ?`)
      .bind(...vals)
      .run()
