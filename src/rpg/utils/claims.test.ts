...
describe('Claims System', () => {
  describe('getClaim', () => {
    ...
    it('should throw error for non-existent character', async () => {
      await expect(getClaim(mockEnv, mockDb, 'non-existent')).rejects.toThrow('Character not found: non-existent')
    })
  })
  ...
  describe('resolveTickConflicts', () => {
    ...
    it('should handle active claims (locked by me vs locked by others)', async () => {
      ...
      if (otherResult?.status === 'modified') {
        expect(otherResult.modification.narrativeContext).toContain('already claimed by creature:shaper-alpha')
      }
    })
