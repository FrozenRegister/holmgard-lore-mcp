...
export async function handleInventoryManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'get': {
      ...
      const totalWeight = results.reduce(
        (sum: number, r: Record<string, unknown>) =>
          sum + ((r.weight as number) ?? 0) * ((r.quantity as number) ?? 1),
        0,
      )
      return ok({
        success: true,
    ...
    case 'add': {
      ...
      const existing = (await db
        .prepare('SELECT quantity FROM inventory_items WHERE character_id = ? AND item_id = ?')
        .bind(a.characterId, a.itemId)
      ...
      if (existing) {
        await db
          ...
          .bind(a.quantity, a.characterId, a.itemId)
          .run()
      } else {
        await db
          .prepare(
            'INSERT INTO inventory_items (character_id, item_id, quantity, equipped) VALUES (?, ?, ?, 0)',
          )
          .bind(a.characterId, a.itemId, a.quantity)
          .run()
      }
      return ok({
        success: true,
    ...
    case 'remove': {
      ...
      if (existing.quantity <= a.quantity) {
        await db
          .prepare('DELETE FROM inventory_items WHERE character_id = ? AND item_id = ?')
          .bind(a.characterId, a.itemId)
          .run()
      } else {
        await db
    ...
    case 'equip': {
      ...
      await db
        ...
        .bind(a.slot ?? null, a.characterId, a.itemId)
        .run()
      return ok({
        success: true,
        actionType: 'equip',
    ...
    case 'transfer': {
      if (!a.characterId || !a.itemId || !a.targetCharacterId)
        return err('"characterId", "itemId", and "targetCharacterId" are required')
      const existing = (await db
        .prepare('SELECT quantity FROM inventory_items WHERE character_id = ? AND item_id = ?')
        .bind(a.characterId, a.itemId)
        .first()) as { quantity: number } | null
      if (!existing || existing.quantity < a.quantity)
        return err('Insufficient quantity to transfer')
      if (existing.quantity <= a.quantity) {
        await db
          .prepare('DELETE FROM inventory_items WHERE character_id = ? AND item_id = ?')
          .bind(a.characterId, a.itemId)
      ...
      const dest = (await db
        .prepare('SELECT quantity FROM inventory_items WHERE character_id = ? AND item_id = ?')
        .bind(a.targetCharacterId, a.itemId)
        .first()) as { quantity: number } | null
      if (dest) {
        await db
          .prepare(
          ...
          .bind(a.quantity, a.targetCharacterId, a.itemId)
          .run()
      } else {
        await db
          .prepare(
          ...
          .bind(a.targetCharacterId, a.itemId, a.quantity)
          .run()
      }
      return ok({
        success: true,
        actionType: 'transfer',
        fromCharacterId: a.characterId,
        toCharacterId: a.targetCharacterId,
        itemId: a.itemId,
        quantity: a.quantity,
      })
    }
