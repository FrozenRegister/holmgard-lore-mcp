...
const ALIASES: Record<string, CombatAction> = {
  ...
  join: 'add_combatant',
  spawn: 'add_combatant',
  remove: 'remove_combatant',
  kill: 'remove_combatant',
  flee: 'remove_combatant',
  begin: 'start',
  init: 'start',
  finish: 'end',
  resolve: 'end',
  ...
  switch (match.matched) {
    case 'create_encounter': {
      if (a.regionId) {
        ...
        if (!region)
          return err(
            `"regionId" "${a.regionId}" does not exist in the regions table. Omit "regionId" to create an unattached encounter, or create the region first.`,
          )
      }
      ...
      await db
        ...
        .bind(id, a.regionId ?? null, JSON.stringify(tokens), 1, 'setup', now, now)
        .run()
      return ok({
        success: true,
    ...
    case 'get_encounter': {
      ...
      const { results: log } = await db
        .prepare(
          'SELECT * FROM combat_action_log WHERE encounter_id = ? ORDER BY round, turn_index LIMIT 50',
        )
        .bind(a.id)
        .all()
      return ok({
        success: true,
        actionType: 'get_encounter',
        encounter: { ...row, tokens: parseTokens((row as any).tokens) },
        actionLog: log,
      })
    }
    case 'list_encounters': {
      ...
      if (a.filter && a.filter !== 'all') {
        query += ' WHERE status = ?'
        binds.push(a.filter === 'active' ? 'active' : 'completed')
      }
      const { results } = await db
        .prepare(query + ' ORDER BY created_at DESC LIMIT 50')
        .bind(...binds)
        .all()
      return ok({
        success: true,
        actionType: 'list_encounters',
        encounters: results,
    ...
    case 'add_combatant': {
      ...
      if (!token) {
        if (a.characterId) {
          ...
          return err(
            '"token" (object with {name, type}) is required when characterId is not provided. Example: { name: "Goblin Archer", type: "enemy" }',
          )
        }
      }
      const row = (await db
      ...
      await db
        ...
        .bind(JSON.stringify(tokens), now, a.id)
        .run()
      return ok({
        success: true,
        actionType: 'add_combatant',
        encounterId: a.id,
        token: newToken,
    ...
    case 'remove_combatant': {
      ...
      const row = (await db
        ...
        .first()) as { tokens: string } | null
      if (!row) return err(`Encounter not found: ${a.id}`)
      const tokens = (parseTokens(row.tokens) as Array<{ id: string }>).filter(
        (t) => t.id !== a.tokenId,
      )
      await db
        .prepare('UPDATE encounters SET tokens = ?, updated_at = ? WHERE id = ?')
      ...
      return ok({
        success: true,
        actionType: 'remove_combatant',
        encounterId: a.id,
        tokenId: a.tokenId,
        remainingCombatants: tokens.length,
      })
    }
    case 'start': {
      if (!a.id) return err('"id" is required')
      ...
      await db
        .prepare(
          "UPDATE encounters SET status = 'active', tokens = ?, active_token_id = ?, round = 1, updated_at = ? WHERE id = ?",
        )
        .bind(JSON.stringify(tokens), firstToken, now, a.id)
        .run()
      return ok({
        success: true,
        actionType: 'start',
        ...
        status: 'active',
        round: 1,
        firstTurn: firstToken,
      })
    }
    case 'end': {
      if (!a.id) return err('"id" is required')
      await db
        .prepare("UPDATE encounters SET status = 'completed', updated_at = ? WHERE id = ?")
        .bind(now, a.id)
        .run()
    ...
    case 'next_turn': {
      if (!a.id) return err('"id" is required')
      const row = (await db
        .prepare('SELECT tokens, active_token_id, round FROM encounters WHERE id = ?')
        .bind(a.id)
        .first()) as { tokens: string; active_token_id: string; round: number } | null
      if (!row) return err(`Encounter not found: ${a.id}`)
      ...
      await db
        .prepare(
          'UPDATE encounters SET active_token_id = ?, round = ?, updated_at = ? WHERE id = ?',
        )
        .bind(nextToken, newRound, now, a.id)
        .run()
      return ok({
        success: true,
        ...
        round: newRound,
        newRoundStarted: nextIdx === 0,
      })
    }
    case 'get_state': {
      if (!a.id) return err('"id" is required')
      const row = await db
        .prepare('SELECT id, round, active_token_id, status, tokens FROM encounters WHERE id = ?')
        .bind(a.id)
        .first()
      if (!row) return err(`Encounter not found: ${a.id}`)
      return ok({
        ...
        encounterId: a.id,
        round: (row as any).round,
        activeTokenId: (row as any).active_token_id,
        status: (row as any).status,
        tokens: parseTokens((row as any).tokens),
        tokenCount: parseTokens((row as any).tokens).length,
      })
    }
    case 'death_save': {
      if (!a.characterId) return err('"characterId" is required')
      const char = (await db
        .prepare('SELECT hp, resource_pools FROM characters WHERE id = ?')
        .bind(a.characterId)
        .first()) as { hp: number; resource_pools: string } | null
      ...
      if (roll === 20) {
        ...
        await db
          .prepare('UPDATE characters SET hp = 1, resource_pools = ?, updated_at = ? WHERE id = ?')
          .bind(JSON.stringify(pools), now, a.characterId)
          .run()
        return ok({
          success: true,
      ...
      if (deathSaves.failures >= 3) {
        ...
        status = 'dying'
        pools.death_saves = deathSaves
      }

      await db
        .prepare('UPDATE characters SET resource_pools = ?, updated_at = ? WHERE id = ?')
        .bind(JSON.stringify(pools), now, a.characterId)
        .run()
      return ok({
        success: true,
    ...
    case 'legendary_action': {
      ...
      const char = (await db
        .prepare(
          'SELECT legendary_actions, legendary_actions_remaining FROM characters WHERE id = ?',
        )
        .bind(a.characterId)
        .first()) as {
      ...
      if (remaining < a.cost)
        return err(`Not enough legendary actions remaining (has ${remaining}, needs ${a.cost})`)
      const newRemaining = remaining - a.cost
      await db
        .prepare(
          'UPDATE characters SET legendary_actions_remaining = ?, updated_at = ? WHERE id = ?',
        )
        .bind(newRemaining, now, a.characterId)
        .run()
      if (a.encounterId) {
        await db
          .prepare(
            'INSERT INTO combat_action_log (encounter_id, round, turn_index, actor_id, actor_name, action_type, result_summary, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          )
          .bind(
            a.encounterId,
            0,
    ...
    case 'lair_action': {
      if (!a.characterId) return err('"characterId" is required')
      const char = (await db
        .prepare('SELECT has_lair_actions FROM characters WHERE id = ?')
        .bind(a.characterId)
        .first()) as { has_lair_actions: number | null } | null
      if (!char) return err(`Character not found: ${a.characterId}`)
      if (!char.has_lair_actions)
        return err(`Character ${a.characterId} does not have lair actions`)
      if (a.encounterId) {
        await db
          .prepare(
            'INSERT INTO combat_action_log (encounter_id, round, turn_index, actor_id, actor_name, action_type, result_summary, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          ...
          .bind(
            ...
            a.characterId,
            'lair_action',
            a.actionName ?? 'Lair action triggered',
            now,
          )
          .run()
      }
      return ok({
        ...
        characterId: a.characterId,
        actionName: a.actionName ?? null,
        note: 'Lair action triggered at initiative count 20 — narrate its effect; no automatic mechanical effect is applied.',
      })
    }
  }
}
