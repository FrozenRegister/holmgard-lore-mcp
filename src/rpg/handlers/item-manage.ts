...
export async function handleItemManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'create': {
      ...
      await db
        ...
        .bind(
          ...
          a.weight,
          a.value,
          a.properties ? JSON.stringify(a.properties) : null,
          now,
          now,
        )
        .run()
    ...
    case 'get': {
      ...
      return ok({
        success: true,
        actionType: 'get',
        item: { ...row, properties: row.properties ? JSON.parse(row.properties as string) : null },
      })
    }
    case 'list': {
    ...
    case 'update': {
      ...
      if (a.properties) {
        ...
        vals.push(JSON.stringify(a.properties))
      }
      vals.push(a.id)
      await db
    ...
    case 'search': {
      ...
      const { results } = await db
        .prepare(
          'SELECT id, name, type, weight, value FROM items WHERE name LIKE ? OR description LIKE ? LIMIT ?',
        )
        .bind(pattern, pattern, a.limit)
        .all()
      return ok({
        success: true,
        actionType: 'search',
