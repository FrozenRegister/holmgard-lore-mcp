...
const ALIASES: Record<string, SpawnAction> = {
  ...
  encounter: 'spawn_encounter', new_encounter: 'spawn_encounter', setup_encounter: 'spawn_encounter',
...
export async function handleSpawnManage(env: AppBindings, args: Record<string, unknown>): Promise<McpResponse> {
  ...
  switch (match.matched) {
    case 'spawn_character': {
      ...
      await db.prepare('INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, character_type, character_class, race, conditions, resistances, vulnerabilities, immunities, known_spells, prepared_spells, cantrips_known, currency, resource_pools, xp, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
        .bind(id, a.name, JSON.stringify(stats), hp, maxHp, a.ac, a.level, a.characterType, a.characterClass, a.race, '[]', '[]', '[]', '[]', '[]', '[]', '[]', '{"gold":0,"silver":0,"copper":0}', '{}', 0, now, now).run()
    ...
    case 'add_to_encounter': {
      if (!a.encounterId || !a.characterId) return err('"encounterId" and "characterId" are required')
      const row = await db.prepare('SELECT tokens FROM encounters WHERE id = ?').bind(a.encounterId).first() as { tokens: string } | null
      if (!row) return err(`Encounter not found: ${a.encounterId}`)
      const char = await db.prepare('SELECT id, name, character_type, hp, ac FROM characters WHERE id = ?').bind(a.characterId).first() as Record<string, unknown> | null
      if (!char) return err(`Character not found: ${a.characterId}`)
      const tokens = JSON.parse(row.tokens ?? '[]') as object[]
      const token = { id: a.characterId, name: char.name, type: char.character_type, hp: char.hp, ac: char.ac, initiative: a.initiative ?? 0, position: a.position ?? { x: 0, y: 0 } }
      tokens.push(token)
      await db.prepare('UPDATE encounters SET tokens = ?, updated_at = ? WHERE id = ?').bind(JSON.stringify(tokens), now, a.encounterId).run()
      return ok({ success: true, actionType: 'add_to_encounter', encounterId: a.encounterId, characterId: a.characterId, token, totalCombatants: tokens.length })
    }
    case 'list_spawned': {
