...
export const DEFAULT_ZONE_TYPES: ReadonlyArray<{
  name: string
  glyph: string | null
...
export async function seedDefaultZoneTypes(db: D1Database, worldId: string): Promise<number> {
  ...
  const { results: existingRows } = (await db
    .prepare('SELECT name FROM zone_types WHERE world_id = ?')
    .bind(worldId)
  ...
  for (const z of DEFAULT_ZONE_TYPES) {
    ...
    await db
      .prepare(
        'INSERT INTO zone_types (id, world_id, name, glyph, color_hex, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      )
      .bind(crypto.randomUUID(), worldId, z.name, z.glyph, z.colorHex, null, now, now)
      .run()
    seeded++
  }
  return seeded
...
export async function getZoneTypeRegistry(
  db: D1Database,
  worldId: string,
): Promise<Map<string, { glyph: string | null; colorHex: string | null }>> {
  const { results } = (await db
    .prepare('SELECT name, glyph, color_hex FROM zone_types WHERE world_id = ?')
    .bind(worldId)
    .all()) as { results: Array<{ name: string; glyph: string | null; color_hex: string | null }> }
  return new Map(results.map((r) => [r.name, { glyph: r.glyph, colorHex: r.color_hex }]))
}

export async function handleZoneTypeManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'register': {
      ...
      if (glyph !== null && [...glyph].length !== 1)
        return err('"glyph" must be exactly 1 character, or null for no overlay')
      const colorHex = a.colorHex ?? null
      if (colorHex !== null && !HEX_COLOR.test(colorHex))
        return err('"colorHex" must be a 6-digit hex color like #A1B2C3, or null')
      const existing = await db
        .prepare('SELECT id FROM zone_types WHERE world_id = ? AND name = ?')
      ...
      await db
        ...
        .bind(id, a.worldId, a.name, glyph, colorHex, a.description ?? null, now, now)
        .run()
      return ok({
        success: true,
        actionType: 'register',
    ...
    case 'list': {
      ...
      const { results } = await db
        .prepare(
          'SELECT id, name, glyph, color_hex, description FROM zone_types WHERE world_id = ? ORDER BY name',
        )
        .bind(a.worldId)
        .all()
      return ok({
    ...
    case 'get': {
      ...
      if (!targetId && !(a.worldId && a.name))
        return err('"id"/"zoneTypeId", or "worldId" + "name", is required')
      const row = targetId
        ? await db.prepare('SELECT * FROM zone_types WHERE id = ?').bind(targetId).first()
        : await db
            .prepare('SELECT * FROM zone_types WHERE world_id = ? AND name = ?')
            .bind(a.worldId, a.name)
            .first()
    ...
    case 'update': {
      ...
      const existing = await db
        .prepare('SELECT id FROM zone_types WHERE id = ?')
        .bind(targetId)
        .first()
      ...
      if (a.glyph !== undefined) {
        if (a.glyph !== null && [...a.glyph].length !== 1)
          return err('"glyph" must be exactly 1 character, or null for no overlay')
        sets.push('glyph = ?')
        vals.push(a.glyph)
      }
      if (a.colorHex !== undefined) {
        if (a.colorHex !== null && !HEX_COLOR.test(a.colorHex))
          return err('"colorHex" must be a 6-digit hex color like #A1B2C3, or null')
        sets.push('color_hex = ?')
      ...
      vals.push(targetId)
      await db
        .prepare(`UPDATE zone_types SET ${sets.join(', ')} WHERE id = ?`)
        .bind(...vals)
        .run()
    ...
    case 'delete': {
      const targetId = a.id ?? a.zoneTypeId
      if (!targetId) return err('"id" or "zoneTypeId" is required')
      const zoneType = (await db
        .prepare('SELECT world_id, name FROM zone_types WHERE id = ?')
        .bind(targetId)
        .first()) as { world_id: string; name: string } | null
      if (!zoneType) return err(`Zone type not found: ${targetId}`)
      const landmarkRef = await db
        .prepare('SELECT 1 FROM landmarks WHERE world_id = ? AND zone_type = ? LIMIT 1')
        .bind(zoneType.world_id, zoneType.name)
        .first()
      if (landmarkRef)
        return err(
          `Cannot delete zone type "${zoneType.name}" — referenced by existing landmarks in this world`,
        )
    ...
    case 'seed_defaults': {
      ...
      return ok({
        ...
        worldId: a.worldId,
        seeded,
        totalDefaults: DEFAULT_ZONE_TYPES.length,
      })
    }
  }
}
