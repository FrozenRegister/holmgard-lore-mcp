...
const ALIASES: Record<string, SpatialAction> = {
  ...
  new_network: 'network_create',
  get_network: 'network_get',
  fetch_network: 'network_get',
...
export async function handleSpatialManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'look': {
      ...
      const room = (await db
        .prepare('SELECT * FROM room_nodes WHERE id = ?')
        .bind(a.roomId)
      ...
      return ok({
        ...
        biome: room.biome_context,
        worldId: room.world_id,
        atmosphere: JSON.parse((room.atmospherics as string) ?? '[]'),
        exits,
    ...
    case 'generate': {
      ...
      const desc =
        a.description && a.description.trim().length >= 10
          ? a.description
          : `${a.name}: a location waiting to be explored.`
      await db
        .prepare(
          'INSERT INTO room_nodes (id, name, base_description, biome_context, atmospherics, exits, entity_ids, created_at, updated_at, visited_count, world_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(
          id,
          a.name,
          desc,
          a.biome,
          JSON.stringify(a.atmosphere),
          JSON.stringify(a.exits),
      ...
      return ok({
        ...
        name: a.name,
        biome: a.biome,
        worldId: a.worldId ?? null,
        exitCount: a.exits.length,
      })
    }
    case 'update': {
      if (!a.roomId) return err('"roomId" is required')
      ...
      if (a.name !== undefined) {
        ...
        binds.unshift(a.name)
      }
      if (a.description !== undefined && a.description.trim().length >= 10) {
        updates.unshift('base_description = ?')
        binds.unshift(a.description)
      }
      if (a.biome !== undefined) {
        updates.unshift('biome_context = ?')
        binds.unshift(a.biome)
      }
      if (a.atmosphere.length > 0) {
        updates.unshift('atmospherics = ?')
        binds.unshift(JSON.stringify(a.atmosphere))
      }
      if (a.exits.length > 0) {
        updates.unshift('exits = ?')
        binds.unshift(JSON.stringify(a.exits))
      }
      if (a.worldId !== undefined) {
        updates.unshift('world_id = ?')
        binds.unshift(a.worldId)
      }
      binds.push(a.roomId)
      await db
        .prepare(`UPDATE room_nodes SET ${updates.join(', ')} WHERE id = ?`)
        .bind(...binds)
        .run()
    ...
    case 'get_exits': {
      ...
      if (!room) return err(`Room not found: ${a.roomId}`)
      const exits = JSON.parse(room.exits ?? '[]') as Array<{
        direction: string
        targetRoomId: string
        label?: string
      }>
    ...
    case 'move': {
      ...
      const nextRoom = (await db
        .prepare('SELECT id, name, base_description FROM room_nodes WHERE id = ?')
      ...
      return ok({
        ...
        description: nextRoom.base_description,
      })
    }
    case 'list': {
    ...
    case 'network_create': {
      ...
      await db
        .prepare(
          'INSERT INTO node_networks (id, name, type, world_id, center_x, center_y, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(id, a.name, a.networkType, a.worldId, 0, 0, now, now)
        .run()
      return ok({
        success: true,
        actionType: 'network_create',
        networkId: id,
        name: a.name,
        worldId: a.worldId,
        type: a.networkType,
    ...
    case 'network_get': {
      ...
      const net = (await db
        .prepare('SELECT * FROM node_networks WHERE id = ?')
        .bind(a.networkId)
        .first()) as Record<string, unknown> | null
      if (!net) return err(`Network not found: ${a.networkId}`)
      const { results: nodes } = await db
        .prepare('SELECT id, name, biome_context FROM room_nodes WHERE network_id = ?')
        .bind(a.networkId)
        .all()
      return ok({
        success: true,
        actionType: 'network_get',
        network: net,
        nodes,
    ...
    case 'network_list': {
      const { results } = await db
        .prepare(
          'SELECT id, name, type, world_id, created_at FROM node_networks ORDER BY created_at DESC LIMIT ?',
        )
        .bind(a.limit)
        .all()
      return ok({
        success: true,
        actionType: 'network_list',
        networks: results,
