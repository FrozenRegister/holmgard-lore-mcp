...
const ALIASES: Record<string, AgentAction> = {
  ...
  tokens: 'budget',
  usage: 'budget',
  upsert_slice: 'set_slice',
  add_slice: 'set_slice',
  update_slice: 'set_slice',
  delete_slice: 'remove_slice',
  enable_slice: 'toggle_slice',
  disable_slice: 'toggle_slice',
  get_slices: 'list_slices',
  observe: 'narrate',
  feed: 'narrate',
  broadcast_observation: 'broadcast',
  dry_run: 'preview_prompt',
  preview: 'preview_prompt',
  secret: 'add_secret',
  add_knowledge: 'add_secret',
  ...
  async function resolveAgentId(): Promise<string | null> {
    ...
    if (a.characterId) {
      const agent = (await db
        .prepare('SELECT id FROM agents WHERE character_id = ?')
        .bind(a.characterId)
  ...
  switch (match.matched) {
    ...
    case 'create': {
      ...
      try {
        ...
        if (
          e instanceof Error &&
          e.message.includes('UNIQUE constraint failed: agents.character_id')
        ) {
    ...
    case 'get': {
      const row = agentId
        ...
        : a.characterId
          ? await db
              .prepare('SELECT * FROM agents WHERE character_id = ?')
              .bind(a.characterId)
              .first()
          : null
      if (!row)
        return err(
          agentId
            ...
            : a.characterId
              ? `No agent for character: ${a.characterId}`
              : '"id"/"agentId" or "characterId" is required',
        )
    ...
    case 'update': {
      ...
      if (a.autoOnTurn !== undefined) {
        sets.push('auto_on_turn = ?')
        vals.push(a.autoOnTurn ? 1 : 0)
      }
      if (a.temperature !== undefined) {
        sets.push('temperature = ?')
        vals.push(a.temperature)
      }
      if (a.maxTokens !== undefined) {
        sets.push('max_tokens = ?')
        vals.push(a.maxTokens)
      }
      if (a.budgetTokens !== undefined) {
        sets.push('budget_tokens = ?')
        vals.push(a.budgetTokens)
      }
      vals.push(agentId)
      await db
    ...
    case 'resume': {
      ...
      return ok({
        ...
        actionType: 'resume',
        agentId,
        message: 'Circuit closed, failure counter reset, status set to active.',
      })
    }
    ...
    case 'health': {
      ...
      const budgetRemaining = agent.budget_tokens
        ? (agent.budget_tokens as number) - (agent.tokens_used as number)
        : null
      return ok({
        success: true,
        actionType: 'health',
    ...
    case 'budget': {
      ...
      if (a.budgetTokens !== undefined) {
        await db
          .prepare('UPDATE agents SET budget_tokens = ?, updated_at = ? WHERE id = ?')
          .bind(a.budgetTokens, now, agentId)
          .run()
        return ok({ success: true, actionType: 'budget', agentId, budgetTokens: a.budgetTokens })
      }
      const agent = (await db
        .prepare('SELECT tokens_used, budget_tokens FROM agents WHERE id = ?')
        .bind(agentId)
        .first()) as Record<string, unknown> | null
      if (!agent) return err(`Agent not found: ${agentId}`)
      return ok({
    ...
    case 'set_slice': {
      ...
      if (!resolvedAgentId || !a.kind || !a.content)
        return err('"agentId"/"id"/"characterId", "kind", and "content" are required')
      ...
      await db
        ...
        .bind(sliceId, agentId, a.kind, a.label ?? null, a.content, a.orderIndex ?? 0, now)
        .run()
      ...
      if (!a.sliceId) {
        const existing = await db
          ...
          .bind(agentId, a.kind, a.label ?? null, sliceId)
          .first()
        ...
        if (existing) {
          ...
          await db
            ...
            .bind(a.content, a.orderIndex ?? 0, now, existing.id)
            .run()
          ...
          return ok({
            success: true,
            actionType: 'set_slice',
            agentId,
            sliceId: existing.id as string,
            kind: a.kind,
          })
        }
      }
    ...
    case 'toggle_slice': {
      ...
      await db
        .prepare('UPDATE agent_prompt_slices SET enabled = ?, updated_at = ? WHERE id = ?')
        .bind(a.enabled ? 1 : 0, now, a.sliceId)
        .run()
      return ok({
        success: true,
        actionType: 'toggle_slice',
        sliceId: a.sliceId,
        enabled: a.enabled,
      })
    }
    ...
    case 'list_slices': {
      ...
      const agentId = resolvedAgentId
      let query =
        'SELECT id, kind, label, content, order_index, enabled FROM agent_prompt_slices WHERE agent_id = ?'
      const binds: unknown[] = [agentId]
      if (a.filter === 'enabled') {
        query += ' AND enabled = 1'
      } else if (a.filter === 'disabled') {
        query += ' AND enabled = 0'
      } else if (a.kind) {
        query += ' AND kind = ?'
        binds.push(a.kind)
    ...
    case 'narrate': {
      const resolvedAgentId = await resolveAgentId()
      if (!resolvedAgentId || !a.observation)
        return err('"agentId"/"id"/"characterId" and "observation" are required')
      const agentId = resolvedAgentId
      ...
      await db
        ...
        .bind(sliceId, agentId, a.label ?? null, a.observation, now)
        .run()
      return ok({ success: true, actionType: 'narrate', agentId, sliceId })
    }

    case 'broadcast': {
      if (!a.agentIds || a.agentIds.length === 0 || !a.observation)
        return err('"agentIds" (array) and "observation" are required')

      // Filter out non-existent agents
      const validAgents = await Promise.all(
        a.agentIds.map(async (aid) => {
      ...
      const rows = await Promise.all(
        validAgentIds.map(async (aid) => {
          const sliceId = crypto.randomUUID()
          await db
            .prepare(
              "INSERT INTO agent_prompt_slices (id, agent_id, kind, label, content, order_index, enabled, updated_at) VALUES (?,?,'narrative_feed',?,?,999,1,?)",
            )
            .bind(sliceId, aid, a.label ?? null, a.observation, now)
            .run()
    ...
    case 'add_secret': {
      const resolvedAgentId = await resolveAgentId()
      if (!resolvedAgentId || !a.content)
        return err('"agentId"/"id"/"characterId" and "content" are required')
      const agentId = resolvedAgentId
      ...
      if (a.importance && !validImportance.includes(a.importance)) {
        return err(
          `Invalid importance level: ${a.importance}. Valid levels are: ${validImportance.join(', ')}`,
        )
      }
      ...
      await db
        ...
        .bind(secretId, agentId, a.content, a.importance ?? 'medium', now)
        .run()
      return ok({
        success: true,
    ...
    case 'remove_secret': {
      ...
      const agentIdResult = (await db
        .prepare('SELECT agent_id FROM agent_secrets WHERE id = ?')
        .bind(a.secretId)
        .first()) as { agent_id: string } | null
      if (!agentIdResult) return err(`Secret not found: ${a.secretId}`)
      const agentId = agentIdResult.agent_id

      // Delete the secret
      const deleteResult = await db
        .prepare('DELETE FROM agent_secrets WHERE id = ?')
        .bind(a.secretId)
        .run()
    ...
    case 'add_journal': {
      ...
      if (a.journalKind && !validKinds.includes(a.journalKind)) {
        return err(
          `Invalid journal kind: ${a.journalKind}. Valid kinds are: ${validKinds.join(', ')}`,
        )
      }
      ...
      return ok({
        ...
        agentId,
        entryId,
        kind: a.journalKind ?? 'observation',
      })
    }
    ...
    case 'get_journal': {
      ...
      const agentId = resolvedAgentId
      let query =
        'SELECT id, kind, encounter_id, round, content, created_at FROM agent_journal WHERE agent_id = ?'
      const binds: unknown[] = [agentId]
    ...
    case 'invoke': {
      if (!agentId) return err('"id" or "agentId" is required')
      if (!env.AI)
        return err('AI binding not configured — enable the "ai" binding in wrangler.jsonc')
      const agent = (await db
        .prepare('SELECT * FROM agents WHERE id = ?')
        .bind(agentId)
        .first()) as Record<string, unknown> | null
      ...
      if (agent.circuit_state === 'open')
        return ok({
          ...
          actionType: 'invoke',
          status: 'circuit_open',
          reason: 'Circuit breaker open — call resume to reset',
        })
    ...
    case 'replay': {
      ...
      const storedCall = (await db
        .prepare('SELECT * FROM agent_calls WHERE id = ?')
        .bind(a.callId)
        .first()) as Record<string, unknown> | null
      ...
      const agent = (await db
        .prepare('SELECT model, temperature, max_tokens FROM agents WHERE id = ?')
        .bind(replayAgentId)
        .first()) as Record<string, unknown> | null
