...
const ALIASES: Record<string, ResourceAction> = {
  drop_crate: 'crate_drop',
  prize_drop: 'crate_drop',
  use: 'consume',
  tick_degrade: 'degrade',
  tick: 'degrade',
  jury_rig: 'improvise',
  macgyver: 'improvise',
  forage: 'scavenge',
...
const CRATE_TABLE: CrateItem[] = [
  {
    ...
    weight: 20,
    minDay: 0,
    degradationDays: null,
    expiresOnDay: null,
    note: 'Stops bleeding, +4 to first aid.',
  },
  {
    name: 'Antiseptic Vial',
    category: 'medical',
    weight: 15,
    minDay: 0,
    degradationDays: null,
    expiresOnDay: 25,
    note: 'Prevents infection on one wound.',
  },
  {
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Ignore -2 injury penalty for 6 hours.',
  },
  {
    name: 'Suture Kit',
    category: 'medical',
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: '+6 to first aid, closes deep wounds.',
  },
  {
  ...
  },
  {
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Stabilizes critical injury, prevents death.',
  },
  {
  ...
  {
    name: 'High-Energy Ration',
    category: 'food',
    weight: 15,
    minDay: 10,
    degradationDays: 3,
    expiresOnDay: null,
    note: '2 days, 1 person, +1 CON for 12h.',
  },
  {
  ...
  {
    name: 'MRE with Heating Element',
    category: 'food',
    weight: 8,
    minDay: 5,
    degradationDays: 7,
    expiresOnDay: null,
    note: 'Hot meal, +3 morale.',
  },
  {
    name: 'Luxury Item',
    category: 'food',
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: '+5 audience approval if shared.',
  },
  {
  ...
  },
  {
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Knife, pliers, screwdriver — +2 to craft.',
  },
  {
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Survive cold night without shelter. Tears with use.',
  },
  {
    name: 'Signal Mirror',
    category: 'tools',
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Can signal other Yields or extraction boat at distance.',
  },
  {
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Shelter, rain protection. Degrades in sun.',
  },
  {
    name: 'Survival Knife',
    category: 'weapon',
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: '1d4 damage, +1 to craft. Dulls with use.',
  },
  {
    name: 'Bear Spray',
    category: 'weapon',
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Deterrent, predator flees on failed CON save DC 15. Single use.',
  },
  {
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: '1d6, reach. Breaks on crit fail.',
  },
  {
    name: 'Map Fragment',
    category: 'intel',
    ...
    degradationDays: null,
    expiresOnDay: null,
    note: 'Reveals 5x5 tile area, shows predator nests.',
  },
  {
    name: 'Radio Receiver',
    category: 'intel',
    ...
    degradationDays: 3,
    expiresOnDay: null,
    note: 'Hears Production broadcasts, 50% chance of useful intel. Battery: 3 days.',
  },
]
...
const BIOME_RESOURCES: Record<string, BiomeResource[]> = {
  pine_forest: [
    {
      ...
      dc: 12,
      kind: 'forage',
      note: 'Vitamin-rich, WIS check to identify safely.',
    },
    ...
    { name: 'Lean-to shelter', dc: 13, kind: 'craft', note: 'Rain and wind cover.' },
    {
      ...
      dc: 10,
      kind: 'craft',
      note: '1d4, breaks on 1-2 (improvised quality).',
    },
  ],
  meadow: [
  ...
  beach: [
    ...
    {
      name: 'Salt preservative',
      dc: 13,
      kind: 'craft',
      note: 'Extends food shelf life by 2 days.',
    },
...
const STARVATION_TIERS: Record<
  number,
  { conPenalty: number; strPenalty: number; moralePenalty: number; note: string }
> = {
  ...
  2: {
    ...
    strPenalty: -2,
    moralePenalty: -10,
    note: 'Day 2 — encounter threshold +5% (weakened, easier prey).',
  },
  3: {
    conPenalty: -3,
    strPenalty: -3,
    moralePenalty: -15,
    note: 'Day 3 — disadvantage on STR/DEX saves.',
  },
}

function starvationTier(daysWithoutFood: number): {
  conPenalty: number
  strPenalty: number
...
export async function degradeOwnerResources(
  db: D1Database,
  ownerType: 'character' | 'party',
  ownerId: string,
  worldId: string,
  ...
  const { results } = (await db
    .prepare(
      'SELECT * FROM resource_inventory WHERE owner_type = ? AND owner_id = ? AND quantity > 0 AND spoiled = 0',
    )
  ...
  for (const row of results) {
    ...
    if (row.degradation_timer !== null) {
      ...
      if (newTimer <= 0) spoiled = true
        ...
        await db
          .prepare(
            'UPDATE resource_inventory SET degradation_timer = ?, updated_at = ? WHERE id = ?',
          )
          .bind(newTimer, now, row.id)
          .run()
    }
    if (spoiled) {
      await db
        .prepare('UPDATE resource_inventory SET spoiled = 1, updated_at = ? WHERE id = ?')
        .bind(now, row.id)
        .run()
  ...
  const state = (await db
    .prepare(
      'SELECT days_without_food FROM resource_owner_state WHERE owner_type = ? AND owner_id = ?',
    )
    .bind(ownerType, ownerId)
  ...
  await db
    ...
    .bind(ownerType, ownerId, worldId, daysWithoutFood, now)
    .run()
  ...
  return {
    ...
    spoiled: spoiledNow,
    daysWithoutFood,
    starvation: starvationTier(daysWithoutFood),
  }
}
...
export async function handleResourceManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'crate_drop': {
      ...
      const available = CRATE_TABLE.filter(
        (item) => item.minDay <= a.day && (!a.categoryBias || item.category === a.categoryBias),
      )
      ...
      const pool =
        available.length > 0 ? available : CRATE_TABLE.filter((item) => item.minDay <= a.day)
      ...
      if (q === undefined || r === undefined) {
        ...
        const { results: candidates } = (await db
          .prepare('SELECT q, r FROM hexes WHERE world_id = ? ORDER BY RANDOM() LIMIT 20')
          .bind(a.worldId)
          .all()) as { results: Array<{ q: number; r: number }> }
        for (const c of candidates) {
          const tooClose = a.avoidPositions.some(
            (p) => hexDistance(p.q, p.r, c.q, c.r) < a.minDistance,
          )
          if (!tooClose) {
            q = c.q
            r = c.r
      ...
      await db
        .prepare(
          'INSERT INTO crate_drops (id, world_id, day, q, r, contents, claimed, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?)',
        )
        .bind(
          id,
          a.worldId,
          ...
          r,
          JSON.stringify(
            contents.map((c) => ({ name: c.name, category: c.category, note: c.note })),
          ),
          now,
          now,
        )
    ...
    case 'consume': {
      ...
      if (a.inventoryId) {
        row = (await db
          .prepare(
            'SELECT * FROM resource_inventory WHERE id = ? AND owner_type = ? AND owner_id = ?',
          )
          .bind(a.inventoryId, a.ownerType, a.ownerId)
          .first()) as Record<string, unknown> | null
      } else {
        row = (await db
          .prepare(
            'SELECT * FROM resource_inventory WHERE owner_type = ? AND owner_id = ? AND item_name = ? AND quantity > 0 LIMIT 1',
          )
          .bind(a.ownerType, a.ownerId, a.itemName)
          .first()) as Record<string, unknown> | null
      }
      if (!row)
        return err(
          `Resource not found for ${a.ownerType} ${a.ownerId}: ${a.itemName ?? a.inventoryId}`,
        )
      const remaining = Math.max(0, (row.quantity as number) - a.quantity)
      await db
        .prepare('UPDATE resource_inventory SET quantity = ?, updated_at = ? WHERE id = ?')
        .bind(remaining, now, row.id)
        .run()
      ...
      return ok({
        ...
        consumed: a.quantity,
        remaining,
        effect: catalogEntry?.note ?? null,
      })
    }
    case 'degrade': {
      if (!a.ownerType || !a.ownerId || !a.worldId)
        return err('"ownerType", "ownerId", and "worldId" are required')
      const result = await degradeOwnerResources(
        db,
        a.ownerType,
        a.ownerId,
        a.worldId,
    ...
    case 'craft': {
      ...
      const entry = list.find(
        (r) => r.name.toLowerCase() === a.itemName!.toLowerCase() && r.kind === 'craft',
      )
      ...
      if (succeeded && a.ownerType && a.ownerId && a.worldId) {
        ...
        await db
          .prepare(
            'INSERT INTO resource_inventory (id, world_id, owner_type, owner_id, item_name, category, quantity, degradation_timer, expires_on_day, acquired_day, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, NULL, NULL, ?, ?, ?)',
          )
          .bind(
            id,
            a.worldId,
    ...
    case 'scavenge': {
      ...
      if (forageable.length === 0)
        return err(`No forageable resources known for biome "${a.biomeName}"`)
      ...
      if (succeeded && a.ownerType && a.ownerId && a.worldId) {
        ...
        await db
          ...
          .bind(id, a.worldId, a.ownerType, a.ownerId, entry.name, 'foraged', a.day, now, now)
          .run()
      }
    ...
    case 'get_state': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM resource_inventory WHERE owner_type = ? AND owner_id = ? ORDER BY category, item_name',
        )
        .bind(a.ownerType, a.ownerId)
        .all()
      const state = (await db
        .prepare(
          'SELECT days_without_food, updated_at FROM resource_owner_state WHERE owner_type = ? AND owner_id = ?',
        )
        .bind(a.ownerType, a.ownerId)
        .first()) as { days_without_food: number; updated_at: string } | null
