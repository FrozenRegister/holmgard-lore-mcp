...
const ALIASES: Record<string, CharAction> = {
  ...
  level: 'level_up',
  levelup: 'level_up',
  find_character: 'search',
  query: 'search',
  find: 'find_by_name',
  lookup: 'find_by_name',
  resolve_name: 'find_by_name',
  cast: 'cast_spell',
  castspell: 'cast_spell',
  snap: 'snapshot',
  save_state: 'snapshot',
  switch: 'activate',
  take_control: 'activate',
  possess: 'activate',
  set_driver: 'activate',
  passengers: 'list_passengers',
  list_dormant: 'list_passengers',
  co_habitants: 'list_passengers',
  get_driver: 'list_passengers',
  recompute: 'recompute_derived',
  refresh_derived: 'recompute_derived',
  sync_derived_stats: 'recompute_derived',
  die: 'kill',
  slay: 'kill',
  defeat: 'kill',
  relocate: 'move_to_location',
  set_location: 'move_to_location',
  move_to_hex: 'move_to_tile',
  place_on_map: 'move_to_tile',
...
const InputSchema = z.object({
  ...
  spellSlots: z
    .record(z.object({ max: z.number().int().min(0), current: z.number().int().min(0) }))
    .optional(),
  pactMagicSlots: z
    .object({
...
function parseChar(row: Record<string, unknown>) {
  const stats = row.stats
    ? JSON.parse(row.stats as string)
...
export async function handleCharacterManage(
  ...
  switch (match.matched) {
    case 'create': {
      ...
      `,
        ...
        .bind(
          ...
          JSON.stringify(a.resistances ?? []),
          JSON.stringify(a.vulnerabilities ?? []),
          JSON.stringify(a.immunities ?? []),
          JSON.stringify(a.knownSpells ?? []),
          JSON.stringify(a.preparedSpells ?? []),
          JSON.stringify(a.cantripsKnown ?? []),
          a.spellSlots ? JSON.stringify(a.spellSlots) : null,
          a.pactMagicSlots ? JSON.stringify(a.pactMagicSlots) : null,
          a.maxSpellLevel ?? 0,
          a.concentratingOn ?? null,
          a.legendaryActions ?? null,
          a.legendaryActionsRemaining ?? null,
          a.legendaryResistances ?? null,
          a.legendaryResistancesRemaining ?? null,
          a.hasLairActions ? 1 : 0,
          JSON.stringify(currency),
          a.currentRoomId ?? null,
          perceptionBonus,
          stealthBonus,
          JSON.stringify(a.resourcePools ?? {}),
          0,
          a.hostBodyId ?? null,
    ...
    case 'get': {
      ...
      if (charId) {
        ...
        return ok({
          success: true,
          actionType: 'get',
          character: parseChar(row as Record<string, unknown>),
        })
      }
      ...
      if (results.length > 1) {
        return err(
          `Multiple characters with name '${a.name}' exist. Use "id"/"characterId" for an unambiguous lookup.`,
          {
      ...
      return ok({
        success: true,
        actionType: 'get',
        character: parseChar(results[0] as Record<string, unknown>),
      })
    }
    case 'list': {
      let query =
        'SELECT id, name, character_type, character_class, race, level, hp, max_hp, ac, world_id, born FROM characters'
      const binds: unknown[] = []
      const conditions: string[] = []
      if (a.characterTypeFilter) {
        conditions.push('character_type = ?')
        binds.push(a.characterTypeFilter)
      }
      if (a.worldId) {
    ...
    case 'update': {
      ...
      if (a.stats) {
        sets.push('stats = ?')
        vals.push(JSON.stringify(a.stats))
        ...
        if (a.perceptionBonus === undefined) {
          sets.push('perception_bonus = ?')
          vals.push(wisMod)
        }
        if (a.stealthBonus === undefined) {
          sets.push('stealth_bonus = ?')
          vals.push(dexMod)
        }
      }
      if (a.born !== undefined) {
      ...
      if (a.characterClass !== undefined) {
        sets.push('character_class = ?')
        vals.push(a.characterClass)
      }
      if (a.race !== undefined) {
        sets.push('race = ?')
        vals.push(a.race)
      ...
      if (a.background) {
        ...
        vals.push(a.background)
      }
      if (a.alignment) {
        sets.push('alignment = ?')
        vals.push(a.alignment)
      }
      if (a.conditions) {
        sets.push('conditions = ?')
        vals.push(JSON.stringify(a.conditions))
      }
      if (a.resistances) {
        sets.push('resistances = ?')
        vals.push(JSON.stringify(a.resistances))
      }
      if (a.vulnerabilities) {
        sets.push('vulnerabilities = ?')
        vals.push(JSON.stringify(a.vulnerabilities))
      }
      if (a.immunities) {
        sets.push('immunities = ?')
        vals.push(JSON.stringify(a.immunities))
      }
      if (a.spellSlots) {
        sets.push('spell_slots = ?')
        vals.push(JSON.stringify(a.spellSlots))
      }
      if (a.pactMagicSlots) {
        sets.push('pact_magic_slots = ?')
        vals.push(JSON.stringify(a.pactMagicSlots))
      }
      if (a.knownSpells) {
        sets.push('known_spells = ?')
        vals.push(JSON.stringify(a.knownSpells))
      }
      if (a.preparedSpells) {
        sets.push('prepared_spells = ?')
        vals.push(JSON.stringify(a.preparedSpells))
      }
      if (a.cantripsKnown) {
        sets.push('cantrips_known = ?')
        vals.push(JSON.stringify(a.cantripsKnown))
      }
      if (a.maxSpellLevel !== undefined) {
        sets.push('max_spell_level = ?')
        vals.push(a.maxSpellLevel)
      }
      if (a.concentratingOn !== undefined) {
        sets.push('concentrating_on = ?')
        vals.push(a.concentratingOn)
      }
      if (a.legendaryActions !== undefined) {
        sets.push('legendary_actions = ?')
        vals.push(a.legendaryActions)
      }
      if (a.legendaryActionsRemaining !== undefined) {
        sets.push('legendary_actions_remaining = ?')
        vals.push(a.legendaryActionsRemaining)
      }
      if (a.legendaryResistances !== undefined) {
        sets.push('legendary_resistances = ?')
        vals.push(a.legendaryResistances)
      }
      if (a.legendaryResistancesRemaining !== undefined) {
        sets.push('legendary_resistances_remaining = ?')
        vals.push(a.legendaryResistancesRemaining)
      }
      if (a.hasLairActions !== undefined) {
        sets.push('has_lair_actions = ?')
        vals.push(a.hasLairActions ? 1 : 0)
      }
      if (a.resourcePools) {
        sets.push('resource_pools = ?')
        vals.push(JSON.stringify(a.resourcePools))
      }
      if (a.currency) {
        sets.push('currency = ?')
        vals.push(JSON.stringify(a.currency))
      }
      if (a.factionId !== undefined) {
        sets.push('faction_id = ?')
        vals.push(a.factionId)
      }
      if (a.behavior !== undefined) {
        sets.push('behavior = ?')
        vals.push(a.behavior)
      }
      if (a.origin !== undefined) {
        sets.push('origin = ?')
        vals.push(a.origin)
      }
      if (a.currentRoomId !== undefined) {
        sets.push('current_room_id = ?')
        vals.push(a.currentRoomId)
      }
      if (a.hostBodyId !== undefined) {
        sets.push('host_body_id = ?')
        vals.push(a.hostBodyId)
      }
      ...
      if (a.active !== undefined) {
        sets.push('active = ?')
        vals.push(a.active ? 1 : 0)
      }
      if (a.perceptionBonus !== undefined) {
        sets.push('perception_bonus = ?')
        vals.push(a.perceptionBonus)
      }
      if (a.stealthBonus !== undefined) {
        sets.push('stealth_bonus = ?')
        vals.push(a.stealthBonus)
      }
      if (a.worldId !== undefined) {
        sets.push('world_id = ?')
        vals.push(a.worldId)
      }
      if (a.deathMode !== undefined) {
        sets.push('death_mode = ?')
        vals.push(a.deathMode)
      }
      if (a.dissolutionStage !== undefined) {
        sets.push('dissolution_stage = ?')
        vals.push(a.dissolutionStage)
      }
      if (a.dissolutionStages !== undefined) {
        sets.push('dissolution_stages = ?')
        vals.push(a.dissolutionStages)
      }
      if (a.dissolutionTerminal !== undefined) {
        sets.push('dissolution_terminal = ?')
        vals.push(a.dissolutionTerminal)
      }
      if (a.dissolutionId !== undefined) {
        sets.push('dissolution_id = ?')
        vals.push(a.dissolutionId)
      }
    ...
    case 'recompute_derived': {
      ...
      const charId = a.id ?? a.characterId
      if (!charId && !a.worldId)
        return err('"id"/"characterId", or "worldId" for a bulk recompute, is required')
      ...
      const { results } = (await db
        .prepare(query)
        .bind(...binds)
        .all()) as { results: Array<{ id: string; stats: string }> }
      ...
      for (const row of results) {
        ...
        await db
          .prepare(
            'UPDATE characters SET ac = ?, perception_bonus = ?, stealth_bonus = ?, updated_at = ? WHERE id = ?',
          )
          .bind(10 + dexMod, wisMod, dexMod, now, row.id)
          .run()
      ...
      return ok({
        success: true,
        actionType: 'recompute_derived',
        charactersUpdated: characterIds.length,
        characterIds,
      })
    }
    case 'add_xp': {
      ...
      if (!charId || xpToAdd === undefined)
        return err('"id"/"characterId" and "amount"/"xpAmount" are required')
      const row = (await db
      ...
      await db
        .prepare('UPDATE characters SET xp = ?, level = ?, updated_at = ? WHERE id = ?')
        .bind(newXp, newLevel, now, charId)
        .run()
      // Sync D1 character to KV as markdown projection
      await syncCharacterToKv(env, charId)
      return ok({
    ...
    case 'get_progression': {
      ...
      return ok({
        success: true,
        actionType: 'get_progression',
        characterId: charId,
        currentXp: row.xp,
        level: lvl,
        xpForNextLevel: nextLevel,
        xpNeeded: nextLevel ? nextLevel - row.xp : null,
      })
    }
    case 'level_up': {
      const charId = a.id ?? a.characterId
      if (!charId) return err('"id" or "characterId" is required')
      const row = (await db
        .prepare('SELECT level, max_hp FROM characters WHERE id = ?')
        .bind(charId)
        .first()) as { level: number; max_hp: number } | null
      if (!row) return err(`Character not found: ${charId}`)
      ...
      await db
        .prepare(
          'UPDATE characters SET level = ?, max_hp = ?, hp = max_hp + ?, updated_at = ? WHERE id = ?',
        )
        .bind(newLevel, newMaxHp, hpIncrease, now, charId)
        .run()
      // Sync D1 character to KV as markdown projection
      await syncCharacterToKv(env, charId)
      return ok({
    ...
    case 'search': {
      ...
      let query =
        'SELECT id, name, character_type, character_class, race, level, world_id FROM characters WHERE name LIKE ?'
    ...
    case 'cast_spell': {
      try {
        ...
        const row = (await db
          .prepare(
            'SELECT known_spells, prepared_spells, cantrips_known, spell_slots, pact_magic_slots, concentrating_on FROM characters WHERE id = ?',
          )
          .bind(charId)
          .first()) as {
          known_spells: string
        ...
        if (
          !isCantrip &&
          !knownSpells.includes(a.spellName) &&
          !preparedSpells.includes(a.spellName)
        ) {
          return err(
            `"${a.spellName}" is not in this character's known/prepared spells or cantrips — cast blocked`,
          )
        }
        ...
        if (!isCantrip) {
          ...
          if (usePactMagic) {
            ...
            await db
              ...
              .bind(JSON.stringify(pact), now, charId)
              .run()
            remainingSlots = pact
          } else {
            if (a.slotLevel === undefined)
              return err(
                '"slotLevel" is required to cast a leveled spell (use slotLevel: 0 for a cantrip)',
              )
            const slots = row.spell_slots ? JSON.parse(row.spell_slots) : {}
            const slot = slots[String(a.slotLevel)]
            if (!slot || slot.current <= 0)
              return err(`No level ${a.slotLevel} spell slots remaining`)
            slot.current -= 1
            await db
              .prepare('UPDATE characters SET spell_slots = ?, updated_at = ? WHERE id = ?')
              .bind(JSON.stringify(slots), now, charId)
              .run()
        ...
        if (requiresConcentration) {
          ...
          await db
            .prepare('DELETE FROM auras WHERE owner_id = ? AND requires_concentration = 1')
            .bind(charId)
            .run()
          await db
            .prepare(
              'INSERT INTO concentration (character_id, active_spell, spell_level, target_ids, started_at, max_duration, save_dc_base) VALUES (?, ?, ?, ?, ?, ?, ?)',
            )
            .bind(
              charId,
              a.spellName,
            ...
            .run()
          await db
            .prepare('UPDATE characters SET concentrating_on = ?, updated_at = ? WHERE id = ?')
            .bind(a.spellName, now, charId)
            .run()
        }
        ...
        return ok({
          ...
          isCantrip,
          slotLevel: a.slotLevel ?? null,
          usedPactMagic: a.usePactMagic ?? false,
          remainingSlots,
          concentrating: requiresConcentration,
        })
      } catch (e) {
        const msg = String(e)
    ...
    case 'snapshot': {
      ...
      `,
        ...
        .bind(
          ...
          statJson,
          row.hp as number,
          row.max_hp as number,
          row.level as number,
          row.ac as number,
          a.stateJson ? JSON.stringify(a.stateJson) : null,
          a.narrativeNote ?? null,
          now,
        )
        .run()
      ...
      return ok({
        ...
        characterId: charId,
        capturedAt,
        narrativeNote: a.narrativeNote ?? null,
      })
    }
    case 'activate': {
      const charId = a.id ?? a.characterId
      ...
      const target = (await db
        .prepare('SELECT id, host_body_id FROM characters WHERE id = ?')
        .bind(charId)
        .first()) as { id: string; host_body_id: string | null } | null
      ...
      if (!hostBodyId) {
        ...
        await db
          .prepare('UPDATE characters SET active = 1, updated_at = ? WHERE id = ?')
          .bind(now, charId)
          .run()
        await syncCharacterToKv(env, charId)
        return ok({
      ...
      const { results: siblingRows } = await db
        .prepare('SELECT id FROM characters WHERE host_body_id = ? AND id != ?')
        .bind(hostBodyId, charId)
        .all()
      ...
      await db.batch([
        db
          .prepare(
            'UPDATE characters SET active = 0, updated_at = ? WHERE host_body_id = ? AND id != ?',
          )
          .bind(now, hostBodyId, charId),
        db
          .prepare(
            'UPDATE characters SET active = 1, host_body_id = ?, updated_at = ? WHERE id = ?',
          )
          .bind(hostBodyId, now, charId),
      ])
    ...
    case 'find_by_name': {
      ...
      const { results: d1Rows } = await db
        ...
        .bind(normalizedName, `${normalizedName}%`, a.limit ?? 20)
        .all()
      ...
      if (env.LORE_DB) {
        try {
          ...
          do {
            const listed: any = await env.LORE_DB.list(
              cursor ? { cursor, prefix: 'character:' } : { prefix: 'character:' },
            )
          ...
          for (let i = 0; i < allKeys.length; i++) {
            ...
            const kvName = (extractRawField(text, 'Name') ?? allKeys[i].replace(/^character:/, ''))
              .trim()
              .toLowerCase()
            ...
            else if (kvName.includes(normalizedName) || normalizedName.includes(kvName))
              confidence = 0.7
            ...
            if (confidence > 0) {
              matches.push({
                ...
                character_type:
                  extractRawField(text, 'Character-Type') ?? extractRawField(text, 'Type') ?? null,
                source: 'kv',
                confidence,
              })
            }
      ...
      if (top.length === 0) {
        ...
        try {
          const { results: suggestRows } = await db
            .prepare('SELECT name FROM characters WHERE LOWER(name) LIKE ? ORDER BY name LIMIT 3')
            .bind(`${normalizedName[0] ?? ''}%`)
            .all()
      ...
      const ambiguous =
        top.length > 1 && (top[0].confidence < 0.8 || top[1].confidence >= top[0].confidence - 0.15)
      return ok({
        found: true,
        query: a.name,
    ...
    case 'kill': {
      ...
      const row = (await db
        .prepare(
          'SELECT id, name, hp, max_hp, character_type, current_room_id, world_id FROM characters WHERE id = ?',
        )
        .bind(charId)
        .first()) as {
        id: string
      ...
      const statements: D1PreparedStatement[] = [
        db
          .prepare(
            'UPDATE characters SET hp = 0, conditions = ?, current_room_id = NULL, updated_at = ? WHERE id = ?',
          )
      ...
      if (a.triggerProductionPulse) {
        ...
        productionPulse = {
          triggered: true,
          approvalShift: -3,
          celesteComment: 'pre-recorded: death template',
        }
      }
      ...
      const { results: deadMemberships } = (await db
        .prepare('SELECT party_id FROM party_members WHERE character_id = ?')
        .bind(charId)
        .all()) as { results: Array<{ party_id: string }> }

      const partyUpdates: Array<{
        partyId: string
        remainingMembers: number
        archived: boolean
        soloSurvivorId: string | null
      }> = []
      ...
      return ok({
        ...
        event: eventId
          ? {
              id: eventId,
              verb: 'died',
              detail: `Killed by ${killerId ?? 'unknown'} at ${deathLocation ?? 'unknown location'}`,
            }
          : undefined,
    ...
    case 'move_to_location': {
      ...
      await db
        .prepare('UPDATE characters SET location_key = ?, updated_at = ? WHERE id = ?')
        .bind(a.locationKey, now, charId)
        .run()
      await syncCharacterToKv(env, charId)
      return ok({
        success: true,
    ...
    case 'move_to_tile': {
      ...
      await db
        ...
        .bind(a.q, a.r, a.mapId ?? 'main', now, charId)
        .run()
      await syncCharacterToKv(env, charId)
      return ok({
        success: true,
