...
export async function handleTimelineManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'get_events': {
      ...
      if (a.canonical_only) {
        parts.push('AND is_canonical = 1')
      }
      if (a.branch_id) {
        parts.push('AND (branch_id = ? OR branch_id IS NULL)')
        binds.push(a.branch_id)
      }
    ...
    case 'get_gap': {
      ...
      const [before, after] = await Promise.all([
        db
          ...
          .bind(a.before_event_id)
          .first() as Promise<TimelineEvent | null>,
        db
          .prepare('SELECT * FROM timeline_events WHERE id = ?')
          .bind(a.after_event_id)
          .first() as Promise<TimelineEvent | null>,
      ])
      ...
      const canonical = (await db
        .prepare(
          'SELECT * FROM timeline_events WHERE world_id = ? AND event_at > ? AND event_at < ? AND is_canonical = 1 ORDER BY event_at ASC',
        )
        .bind(before.world_id, before.event_at, after.event_at)
        .all()) as { results: TimelineEvent[] }
      const chars = (await db
        .prepare(
          'SELECT DISTINCT entity_id FROM timeline_events WHERE world_id = ? AND event_at > ? AND event_at < ? AND entity_id IS NOT NULL',
        )
        .bind(before.world_id, before.event_at, after.event_at)
    ...
    case 'get_perspectives': {
      ...
      return ok({
        success: true,
        actionType: 'get_perspectives',
        world_id: a.world_id,
        from: a.from,
        to: a.to,
        characters: rows.results.map((r) => r.entity_id),
      })
    }
    ...
    case 'create_branch': {
      ...
      await db
        ...
        .bind(branchId, a.world_id, a.name, a.forked_at_event_id, a.reason ?? null, now)
        .run()
      return ok({
        success: true,
        ...
        world_id: a.world_id,
        name: a.name,
        forked_at_event_id: a.forked_at_event_id,
      })
    }
    ...
    case 'switch_branch': {
      ...
      const branch = (await db
        .prepare('SELECT id FROM timeline_branches WHERE id = ? AND world_id = ?')
        .bind(a.branch_id, a.world_id)
        .first()) as { id: string } | null
      if (!branch) return err(`Branch not found: ${a.branch_id}`)
      await db
        .prepare('UPDATE timeline_branches SET is_active = 0 WHERE world_id = ?')
        .bind(a.world_id)
        .run()
      await db
        .prepare('UPDATE timeline_branches SET is_active = 1 WHERE id = ?')
        .bind(a.branch_id)
        .run()
      return ok({
        success: true,
    ...
    case 'compare_branches': {
      ...
      const [rowsA, rowsB] = await Promise.all([
        db
          .prepare('SELECT id FROM timeline_events WHERE branch_id = ?')
          .bind(a.branch_a)
          .all() as Promise<{ results: Array<{ id: string }> }>,
        db
          .prepare('SELECT id FROM timeline_events WHERE branch_id = ?')
          .bind(a.branch_b)
          .all() as Promise<{ results: Array<{ id: string }> }>,
    ...
    case 'merge_branch': {
      ...
      if (!a.event_ids || a.event_ids.length === 0)
        return err('"event_ids" must be a non-empty array')
      let merged = 0
      for (const eid of a.event_ids) {
        const row = (await db
          .prepare('SELECT branch_id FROM timeline_events WHERE id = ? AND branch_id = ?')
          .bind(eid, a.source_branch_id)
          .first()) as { branch_id: string } | null
        if (!row) continue
        await db
          .prepare('UPDATE timeline_events SET branch_id = ? WHERE id = ?')
          .bind(a.target_branch_id, eid)
          .run()
        merged++
      }
      return ok({
        ...
        source_branch_id: a.source_branch_id,
