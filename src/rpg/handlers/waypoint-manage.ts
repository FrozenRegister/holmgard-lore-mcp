...
interface DefaultGotlandWaypoint { name: string; lat: number; lon: number; kind: string; q: number; r: number }
interface DefaultGotlandDistance { from: string; to: string; distanceKm: number | null; routeSource: string }
...
export async function getWaypoint(db: D1Database, worldId: string, idOrName: string): Promise<WaypointRow | null> {
  const byId = await db.prepare('SELECT * FROM waypoints WHERE world_id = ? AND id = ?').bind(worldId, idOrName).first() as WaypointRow | null
  if (byId) return byId
  return await db.prepare('SELECT * FROM waypoints WHERE world_id = ? AND name = ?').bind(worldId, idOrName).first() as WaypointRow | null
}
...
export async function getWaypointDistance(db: D1Database, worldId: string, fromWaypointId: string, toWaypointId: string): Promise<WaypointDistanceResult> {
  const row = await db.prepare('SELECT distance_km FROM waypoint_distances WHERE world_id = ? AND from_waypoint_id = ? AND to_waypoint_id = ?')
    .bind(worldId, fromWaypointId, toWaypointId).first() as { distance_km: number | null } | null
...
export async function getGeoOrigin(db: D1Database, worldId: string): Promise<GeoOrigin | null> {
  ...
  return { originLat: row.geo_origin_lat, originLon: row.geo_origin_lon, kmPerHex: row.geo_km_per_hex }
}

export async function handleWaypointManage(env: AppBindings, args: Record<string, unknown>): Promise<McpResponse> {
  ...
  switch (match.matched) {
    ...
    case 'list': {
      ...
      const { results } = await db.prepare('SELECT id, name, q, r, lat, lon, kind FROM waypoints WHERE world_id = ? ORDER BY name').bind(a.worldId).all()
      return ok({ success: true, actionType: 'list', worldId: a.worldId, waypoints: results, count: results.length })
    }
    case 'get': {
      const targetId = a.id ?? a.waypointId
      if (!targetId && !(a.worldId && a.name)) return err('"id"/"waypointId", or "worldId" + "name", is required')
      const row = targetId
        ? await db.prepare('SELECT * FROM waypoints WHERE id = ?').bind(targetId).first()
        : await db.prepare('SELECT * FROM waypoints WHERE world_id = ? AND name = ?').bind(a.worldId, a.name).first()
    ...
    case 'update': {
      ...
      const existing = await db.prepare('SELECT id FROM waypoints WHERE id = ?').bind(targetId).first()
      if (!existing) return err(`Waypoint not found: ${targetId}`)
      const sets: string[] = ['updated_at = ?']
      const vals: unknown[] = [now]
      if (a.lat !== undefined) { sets.push('lat = ?'); vals.push(a.lat) }
      if (a.lon !== undefined) { sets.push('lon = ?'); vals.push(a.lon) }
      if (a.q !== undefined) { sets.push('q = ?'); vals.push(a.q) }
      if (a.r !== undefined) { sets.push('r = ?'); vals.push(a.r) }
      if (a.kind !== undefined) { sets.push('kind = ?'); vals.push(a.kind) }
      vals.push(targetId)
      await db.prepare(`UPDATE waypoints SET ${sets.join(', ')} WHERE id = ?`).bind(...vals).run()
      return ok({ success: true, actionType: 'update', waypointId: targetId })
    }
    case 'delete': {
      const targetId = a.id ?? a.waypointId
      if (!targetId) return err('"id" or "waypointId" is required')
      const waypoint = await db.prepare('SELECT world_id, name FROM waypoints WHERE id = ?').bind(targetId).first() as { world_id: string; name: string } | null
      if (!waypoint) return err(`Waypoint not found: ${targetId}`)
      const partyRef = await db.prepare('SELECT 1 FROM parties WHERE current_waypoint_id = ? OR travel_target_waypoint_id = ? LIMIT 1').bind(targetId, targetId).first()
      if (partyRef) return err(`Cannot delete waypoint "${waypoint.name}" — referenced by an existing party's location or march target`)
      await db.prepare('DELETE FROM waypoints WHERE id = ?').bind(targetId).run()
      await db.prepare('DELETE FROM waypoint_distances WHERE from_waypoint_id = ? OR to_waypoint_id = ?').bind(targetId, targetId).run()
      return ok({ success: true, actionType: 'delete', waypointId: targetId })
    }
    case 'validate': {
      if (!a.worldId || !a.name) return err('"worldId" and "name" are required')
      const row = await db.prepare('SELECT id FROM waypoints WHERE world_id = ? AND name = ?').bind(a.worldId, a.name).first()
      if (row) return ok({ success: true, actionType: 'validate', worldId: a.worldId, name: a.name, valid: true })
      const { results } = await db.prepare('SELECT name FROM waypoints WHERE world_id = ?').bind(a.worldId).all() as { results: Array<{ name: string }> }
    ...
    case 'seed_defaults': {
      ...
      const world = await db.prepare('SELECT id FROM worlds WHERE id = ?').bind(a.worldId).first()
      if (!world) return err(`World not found: ${a.worldId}`)
      const { results: existingRows } = await db.prepare('SELECT name FROM waypoints WHERE world_id = ?').bind(a.worldId).all() as { results: Array<{ name: string }> }
      const existingNames = new Set(existingRows.map(r => r.name))
      let waypointsSeeded = 0
      const nameToId = new Map<string, string>()
      for (const wp of DEFAULT_GOTLAND_WAYPOINTS) {
        if (existingNames.has(wp.name)) {
          const row = await db.prepare('SELECT id FROM waypoints WHERE world_id = ? AND name = ?').bind(a.worldId, wp.name).first() as { id: string }
          nameToId.set(wp.name, row.id)
          continue
        }
        const id = crypto.randomUUID()
        await db.prepare('INSERT INTO waypoints (id, world_id, name, q, r, lat, lon, kind, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
          .bind(id, a.worldId, wp.name, wp.q, wp.r, wp.lat, wp.lon, wp.kind, now, now).run()
      ...
      for (const dist of DEFAULT_GOTLAND_DISTANCES) {
        ...
        const existingDist = await db.prepare('SELECT 1 FROM waypoint_distances WHERE world_id = ? AND from_waypoint_id = ? AND to_waypoint_id = ?')
          .bind(a.worldId, fromId, toId).first()
        if (existingDist) continue
        await db.prepare('INSERT INTO waypoint_distances (world_id, from_waypoint_id, to_waypoint_id, distance_km, route_source, computed_at) VALUES (?, ?, ?, ?, ?, ?)')
          .bind(a.worldId, fromId, toId, dist.distanceKm, dist.routeSource, now).run()
        distancesSeeded++
      }
      return ok({
        success: true, actionType: 'seed_defaults', worldId: a.worldId,
        waypointsSeeded, totalDefaultWaypoints: DEFAULT_GOTLAND_WAYPOINTS.length,
        distancesSeeded, totalDefaultDistances: DEFAULT_GOTLAND_DISTANCES.length,
      })
    }
    case 'calibrate': {
      if (!a.worldId) return err('"worldId" is required')
      ...
      const world = await db.prepare('SELECT id FROM worlds WHERE id = ?').bind(a.worldId).first()
      if (!world) return err(`World not found: ${a.worldId}`)
      await db.prepare(`INSERT INTO world_state (world_id, geo_origin_lat, geo_origin_lon, geo_km_per_hex)
                  VALUES (?, ?, ?, ?)
                  ON CONFLICT(world_id) DO UPDATE SET
                    geo_origin_lat = excluded.geo_origin_lat,
                    geo_origin_lon = excluded.geo_origin_lon,
                    geo_km_per_hex = excluded.geo_km_per_hex`)
        .bind(a.worldId, a.originLat, a.originLon, a.kmPerHex).run()
      return ok({ success: true, actionType: 'calibrate', worldId: a.worldId, originLat: a.originLat, originLon: a.originLon, kmPerHex: a.kmPerHex })
    }
    case 'hex_to_latlon': {
      if (!a.worldId) return err('"worldId" is required')
      if (a.q === undefined || a.r === undefined) return err('"q" and "r" are required')
      const origin = await getGeoOrigin(db, a.worldId)
      if (!origin) return err(`World ${a.worldId} has not been geo-calibrated yet — call "calibrate" first`)
      const latLon = hexToLatLon({ q: a.q, r: a.r }, origin)
      return ok({ success: true, actionType: 'hex_to_latlon', worldId: a.worldId, q: a.q, r: a.r, lat: latLon.lat, lon: latLon.lon })
    }
  }
}

/** Exported for the offline precompute script and any future narrator-added waypoint. */
export function computeHexForLatLon(lat: number, lon: number, origin: GeoOrigin): { q: number; r: number } {
