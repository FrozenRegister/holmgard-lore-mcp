...
const ALIASES: Record<string, AuraAction> = {
  ...
  expire_old: 'expire',
  cleanup: 'expire',
  affecting: 'get_affecting',
  on: 'get_affecting',
  start_concentrate: 'concentrate',
  focus: 'concentrate',
  break: 'break_concentration',
  lose_concentration: 'break_concentration',
  concentration_check: 'check_save',
  save_check: 'check_save',
  constitution_save: 'check_save',
...
export async function handleAuraManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'create': {
      ...
      await db
        .prepare(
          'INSERT INTO auras (id, owner_id, spell_name, spell_level, radius, affects_allies, affects_enemies, affects_self, effects, started_at, max_duration, requires_concentration) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(
          id,
          a.ownerId,
          ...
          a.radius,
          a.affectsAllies ? 1 : 0,
          a.affectsEnemies ? 1 : 0,
          a.affectsSelf ? 1 : 0,
          JSON.stringify(a.effects ?? {}),
          now,
          a.maxDuration ?? null,
          a.requiresConcentration ? 1 : 0,
        )
        .run()
      return ok({
        success: true,
    ...
    case 'get': {
      if (!a.id)
        return err(
          '"id" (the aura instance ID returned by create, e.g. "uuid-of-the-aura") is required. Use "list" to find aura IDs for an owner, or pass "ownerId" (character UUID) instead',
        )
      ...
      return ok({
        success: true,
        actionType: 'get',
        aura: { ...row, effects: JSON.parse((row as any).effects ?? '{}') },
      })
    }
    case 'list': {
      const { results } = await db
        .prepare(
          'SELECT id, owner_id, spell_name, spell_level, radius, started_at FROM auras ORDER BY started_at DESC',
        )
        .all()
    ...
    case 'expire': {
      const result = await db
        .prepare(
          'DELETE FROM auras WHERE max_duration IS NOT NULL AND started_at + max_duration < ?',
        )
        .bind(now)
        .run()
    ...
    case 'get_affecting': {
      ...
      return ok({
        ...
        targetId: a.targetId,
        auras: results.map((r: Record<string, unknown>) => ({
          ...r,
          effects: JSON.parse((r as any).effects ?? '{}'),
        })),
    ...
    case 'concentrate': {
      ...
      await db
        .prepare(
          'INSERT OR REPLACE INTO concentration (character_id, active_spell, spell_level, target_ids, started_at, max_duration, save_dc_base) VALUES (?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(
          a.characterId,
          a.spellName,
    ...
    case 'break_concentration': {
      ...
      const conc = await db
        .prepare('SELECT active_spell FROM concentration WHERE character_id = ?')
        .bind(a.characterId)
        .first()
      if (!conc)
        return ok({
          success: true,
          actionType: 'break_concentration',
          characterId: a.characterId,
          hadConcentration: false,
        })
      ...
      return ok({
        ...
        actionType: 'break_concentration',
        characterId: a.characterId,
        hadConcentration: true,
        spellEnded: (conc as any).active_spell,
      })
    }
    case 'check_save': {
      if (!a.characterId) return err('"characterId" is required')
      const conc = (await db
        .prepare('SELECT active_spell FROM concentration WHERE character_id = ?')
        .bind(a.characterId)
        .first()) as { active_spell: string } | null
      if (!conc)
        return ok({
          success: true,
          actionType: 'check_save',
          characterId: a.characterId,
          wasConcentrating: false,
        })
      if (a.damage === undefined)
        return err('"damage" is required to compute the concentration save DC')
      ...
      return ok({
        ...
        actionType: 'check_save',
        characterId: a.characterId,
        wasConcentrating: true,
        spellName: conc.active_spell,
        dc,
        roll,
        maintained,
      })
    ...
    case 'check_duration': {
      ...
      const conc = (await db
        .prepare(
          'SELECT active_spell, started_at, max_duration FROM concentration WHERE character_id = ?',
        )
        .bind(a.characterId)
        .first()) as {
        active_spell: string
