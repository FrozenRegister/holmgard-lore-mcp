...
const ALIASES: Record<string, CorpseAction> = {
  ...
  rot: 'decay',
  advance_decay: 'decay',
  roll_loot: 'generate_loot',
  drop_loot: 'generate_loot',
  die: 'register',
  record_death: 'register',
  tick_decomposition: 'decompose',
  advance_decomposition: 'decompose',
  check_scavengers: 'scavenge_check',
  plunder: 'loot_corpse',
  loot_preserve: 'loot_corpse',
  production_recover: 'recover',
  memorial_recover: 'recover',
  preserve_state: 'get_state',
  wis_save: 'psychological_impact',
  trauma_check: 'psychological_impact',
...
const STAGES: DecompositionStage[] = [
  {
    ...
    diseaseDC: null,
    disease: null,
    scavengerAttraction: 0,
    lootDC: 8,
    lootDescription: 'All inventory, band (data), clothing.',
  },
  {
    name: 'early',
    minHours: 6,
    ...
    scavengerAttraction: 10,
    lootDC: 10,
    lootDescription: 'All inventory, band (data corrupted), clothing (stained).',
  },
  {
    name: 'bloat',
    minHours: 24,
    diseaseRisk: 25,
    diseaseDC: 14,
    disease: 'blood_poisoning',
    scavengerAttraction: 25,
    lootDC: 14,
    lootDescription: '50% inventory (contaminated), band (dead), clothing (ruined).',
  },
  {
    name: 'active_decay',
    minHours: 72,
    diseaseRisk: 50,
    diseaseDC: 16,
    disease: 'preserve_rot',
    scavengerAttraction: 40,
    lootDC: 16,
    lootDescription: '20% inventory (heavily contaminated).',
  },
  {
    name: 'advanced_decay',
    minHours: 168,
    ...
    scavengerAttraction: 10,
    lootDC: 12,
    lootDescription: 'Metal items only.',
  },
  {
    name: 'skeletal',
    minHours: 336,
    ...
    diseaseDC: null,
    disease: null,
    scavengerAttraction: 5,
    lootDC: 10,
    lootDescription: 'Metal items, bone fragments (crafting material).',
  },
]
...
export async function tickAllCorpseDecomposition(
  db: D1Database,
  worldId: string,
  now: string,
): Promise<
  Array<{ corpseId: string; previousStage: string; newStage: string; isLandmark: boolean }>
> {
  const { results } = (await db
    .prepare(
      'SELECT id, death_at, decomposition_stage FROM corpses WHERE world_id = ? AND recovered = 0 AND death_at IS NOT NULL',
    )
    .bind(worldId)
    .all()) as { results: Array<{ id: string; death_at: string; decomposition_stage: string }> }
  const out: Array<{
    corpseId: string
  ...
  for (const c of results) {
    ...
    await db
      .prepare(
        'UPDATE corpses SET decomposition_stage = ?, is_landmark = ?, updated_at = ? WHERE id = ?',
      )
      .bind(stage.name, isLandmark ? 1 : 0, now, c.id)
      .run()
    out.push({
      corpseId: c.id,
      previousStage: c.decomposition_stage,
      newStage: stage.name,
      isLandmark,
    })
  }
...
export async function handleCorpseManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'create': {
      if (!a.characterId || !a.characterName)
        return err('"characterId" and "characterName" are required')
      const id = crypto.randomUUID()
      await db
        .prepare(
          `INSERT INTO corpses (id, character_id, character_name, character_type, world_id, encounter_id, position_q, position_r, state, state_updated_at, harvestable_resources, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'fresh', ?, '[]', ?, ?)`,
        )
        .bind(
          id,
          a.characterId,
    ...
    case 'get': {
      ...
      const { results: loot } = await db
        .prepare(
          'SELECT ci.item_id, ci.quantity, ci.looted, i.name FROM corpse_inventory ci JOIN items i ON ci.item_id = i.id WHERE ci.corpse_id = ?',
        )
        .bind(a.id)
        .all()
      return ok({
        ...
        corpse: {
          ...row,
          harvestable_resources: JSON.parse((row as any).harvestable_resources ?? '[]'),
        },
        loot,
      })
    }
    case 'list': {
      let query =
        'SELECT id, character_name, character_type, state, looted, world_id, encounter_id FROM corpses WHERE 1=1'
      const binds: unknown[] = []
      if (a.worldIdFilter) {
        query += ' AND world_id = ?'
        binds.push(a.worldIdFilter)
      }
      if (a.filter === 'fresh') {
        query += " AND state = 'fresh'"
      }
      if (a.filter === 'unlooted') {
        query += ' AND looted = 0'
      }
      const { results } = await db
    ...
    case 'loot': {
      ...
      await db
        .prepare(
          'UPDATE corpses SET looted = 1, looted_by = ?, looted_at = ?, updated_at = ? WHERE id = ?',
        )
        .bind(a.lootedBy ?? 'unknown', now, now, a.id)
        .run()
      await db
        .prepare('UPDATE corpse_inventory SET looted = 1 WHERE corpse_id = ?')
        .bind(a.id)
        .run()
      const { results: items } = await db
        .prepare('SELECT item_id, quantity FROM corpse_inventory WHERE corpse_id = ?')
        .bind(a.id)
        .all()
    ...
    case 'decay': {
      if (!a.id) return err('"id" is required')
      const row = (await db
        .prepare('SELECT state FROM corpses WHERE id = ?')
        .bind(a.id)
        .first()) as { state: string } | null
      if (!row) return err(`Corpse not found: ${a.id}`)
      ...
      await db
        .prepare('UPDATE corpses SET state = ?, state_updated_at = ?, updated_at = ? WHERE id = ?')
        .bind(nextState, now, now, a.id)
        .run()
      return ok({
        success: true,
        actionType: 'decay',
    ...
    case 'generate_loot': {
      if (!a.id) return err('"id" is required')
      await db
        .prepare('UPDATE corpses SET loot_generated = 1, updated_at = ? WHERE id = ?')
        .bind(now, a.id)
        .run()
      return ok({
        success: true,
        actionType: 'generate_loot',
        corpseId: a.id,
        note: 'Loot generation is handled by combat logic. Mark corpse inventory items manually.',
      })
    }
    case 'delete': {
      if (!a.id) return err('"id" is required')
    ...
    case 'register': {
      ...
      const snapshotRes = await handleResourceManage(env, {
        action: 'get_state',
        ownerType: 'character',
        ownerId: a.characterId,
      })
      const snapshotBody = JSON.parse(snapshotRes.content[0].text) as {
        success?: boolean
        inventory?: Array<{ item_name: string; category: string; quantity: number }>
      }
      const snapshot =
        snapshotBody.success && snapshotBody.inventory
          ? snapshotBody.inventory.map((i) => ({
      ...
      await db
        .prepare(
          `INSERT INTO corpses (id, character_id, character_name, character_type, world_id, encounter_id, position_q, position_r, state, state_updated_at, harvestable_resources, created_at, updated_at, death_at, cause_of_death, decomposition_stage, preserve_inventory_snapshot) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'fresh', ?, '[]', ?, ?, ?, ?, 'fresh', ?)`,
        )
        .bind(
          id,
          a.characterId,
      ...
      return ok({
        ...
        characterName: a.characterName,
        deathAt,
        causeOfDeath: a.causeOfDeath ?? null,
    ...
    case 'decompose': {
      if (!a.id)
        return err(
          '"id" (corpse UUID) is required. Optional: "hoursSinceDeath" (number) to override the computed elapsed time',
        )
      const row = (await db
        .prepare('SELECT death_at, decomposition_stage, recovered FROM corpses WHERE id = ?')
        .bind(a.id)
        .first()) as {
      ...
      if (row.recovered)
        return err(
          `Corpse ${a.id} has already been recovered by Production and is no longer tracked`,
        )
      ...
      await db
        ...
        .bind(stage.name, isLandmark ? 1 : 0, now, a.id)
        .run()
      return ok({
        success: true,
        ...
        corpseId: a.id,
        hoursSinceDeath,
        previousStage: row.decomposition_stage,
        decompositionStage: stage.name,
        isLandmark,
        diseaseRiskPercent: stage.diseaseRisk,
        scavengerAttractionPercent: stage.scavengerAttraction,
      })
    }
    case 'scavenge_check': {
      if (!a.worldId || a.positionQ === undefined || a.positionR === undefined)
        return err('"worldId", "positionQ", and "positionR" are required')
      const { results } = (await db
        .prepare(
          'SELECT id, decomposition_stage FROM corpses WHERE world_id = ? AND position_q = ? AND position_r = ? AND recovered = 0',
        )
        .bind(a.worldId, a.positionQ, a.positionR)
        .all()) as { results: Array<{ id: string; decomposition_stage: string }> }
      ...
      for (const c of results) {
        ...
        breakdown.push({
          corpseId: c.id,
          stage: stage.name,
          attractionPercent: stage.scavengerAttraction,
        })
      }
      return ok({
        success: true,
        actionType: 'scavenge_check',
        worldId: a.worldId,
        positionQ: a.positionQ,
        positionR: a.positionR,
        corpseCount: results.length,
        totalScavengerAttractionPercent: totalAttraction,
        productionInterventionRecommended: results.length >= 3,
        corpses: breakdown,
      })
    }
    case 'loot_corpse': {
      if (!a.id)
        return err(
          '"id" (corpse UUID) is required. Also requires "looterCharacterId" (character UUID doing the looting)',
        )
      if (!a.looterCharacterId)
        return err(
          '"looterCharacterId" (character UUID) is required. Optional: "dexModifier" (number), "rollValue" (1-20)',
        )
      const row = (await db
        .prepare(
          'SELECT decomposition_stage, preserve_inventory_snapshot, recovered FROM corpses WHERE id = ?',
        )
        .bind(a.id)
        .first()) as {
        decomposition_stage: string
        preserve_inventory_snapshot: string
        recovered: number
      } | null
      if (!row) return err(`Corpse not found: ${a.id}`)
      ...
      const snapshot = JSON.parse(row.preserve_inventory_snapshot) as Array<{
        itemName: string
        category: string
        quantity: number
      }>
      ...
      if (stage.name === 'advanced_decay' || stage.name === 'skeletal') {
        ...
        if (stage.name === 'skeletal')
          lootable = [
            ...lootable,
            { itemName: 'Bone Fragments', category: 'crafting_material', quantity: 1 },
          ]
      ...
      await db
        .prepare(
          'UPDATE corpses SET preserve_inventory_snapshot = ?, looted = 1, looted_by = ?, looted_at = ?, updated_at = ? WHERE id = ?',
        )
        .bind(JSON.stringify(remaining), a.looterCharacterId, now, now, a.id)
        .run()
    ...
    case 'recover': {
      if (!a.id)
        return err(
          '"id" (corpse UUID) is required. Optional: "recoveryType" (memorial_package | warning_display | trophy_recovery | research_recovery). Corpse must be at Bloat stage or later',
        )
      const row = (await db
        .prepare('SELECT decomposition_stage, recovered FROM corpses WHERE id = ?')
        .bind(a.id)
        .first()) as { decomposition_stage: string; recovered: number } | null
      if (!row) return err(`Corpse not found: ${a.id}`)
      ...
      if (!eligible.includes(row.decomposition_stage))
        return err(
          `Corpse must be at Bloat stage or later to be recovered (currently ${row.decomposition_stage})`,
        )
      const recoveryType =
        a.recoveryType ?? RECOVERY_TYPES[Math.floor(Math.random() * RECOVERY_TYPES.length)]
      await db
        .prepare('UPDATE corpses SET recovered = 1, recovery_type = ?, updated_at = ? WHERE id = ?')
        .bind(recoveryType, now, a.id)
        .run()
    ...
    case 'get_state': {
      if (!a.id)
        return err(
          '"id" (corpse UUID) is required. Use "list" with worldIdFilter to find corpse IDs. Example: { action: "get_state", id: "corpse-uuid" }',
        )
      const row = (await db
        .prepare('SELECT * FROM corpses WHERE id = ?')
        .bind(a.id)
      ...
      return ok({
        ...
        decompositionStage: row.decomposition_stage,
        isLandmark: row.is_landmark === 1,
        recovered: row.recovered === 1,
        recoveryType: row.recovery_type,
        lootDC: stage.lootDC,
        lootDescription: stage.lootDescription,
        diseaseRiskPercent: stage.diseaseRisk,
        scavengerAttractionPercent: stage.scavengerAttraction,
    ...
    case 'psychological_impact': {
      ...
      if (!a.observerCharacterId)
        return err(
          '"observerCharacterId" (character UUID of the observer) is required. Optional: "relationship" (stranger | party_member | betrayed_them | saved_them), "wisModifier" (number), "rollValue" (1-20)',
        )
      const row = (await db
        .prepare('SELECT decomposition_stage FROM corpses WHERE id = ?')
        .bind(a.id)
        .first()) as { decomposition_stage: DecompositionStage['name'] } | null
      if (!row) return err(`Corpse not found: ${a.id}`)
      ...
      return ok({
        success: true,
        actionType: 'psychological_impact',
        corpseId: a.id,
        observerCharacterId: a.observerCharacterId,
        roll,
        total,
        dc,
        margin,
