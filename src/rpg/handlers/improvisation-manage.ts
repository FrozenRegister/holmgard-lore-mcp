...
export async function handleImprovisationManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'apply': {
      ...
      const result = await db
        .prepare(
          `INSERT INTO custom_effects (target_id, target_type, name, description, source_type, source_entity_id, source_entity_name, category, power_level, mechanics, duration_type, duration_value, rounds_remaining, triggers, removal_conditions, stackable, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        )
        .bind(
          a.targetId,
          a.targetType,
          ...
          a.sourceEntityName ?? null,
          a.category,
          a.powerLevel,
          JSON.stringify(a.mechanics ?? []),
          a.durationType,
          a.durationValue ?? null,
          a.durationType === 'rounds' ? (a.durationValue ?? null) : null,
          JSON.stringify(a.triggers ?? []),
          JSON.stringify(a.removalConditions ?? []),
          a.stackable ? 1 : 0,
          now,
        )
        .run()
    ...
    case 'get': {
      ...
      return ok({
        ...
        effect: {
          ...row,
          mechanics: JSON.parse((row as any).mechanics ?? '[]'),
          triggers: JSON.parse((row as any).triggers ?? '[]'),
          removal_conditions: JSON.parse((row as any).removal_conditions ?? '[]'),
        },
      })
    }
    case 'list': {
      const { results } = await db
        .prepare(
          'SELECT id, target_id, name, category, power_level, duration_type, rounds_remaining, is_active FROM custom_effects WHERE is_active = 1 ORDER BY created_at DESC',
        )
        .all()
    ...
    case 'tick': {
      await db
        .prepare(
          "UPDATE custom_effects SET rounds_remaining = rounds_remaining - ? WHERE duration_type = 'rounds' AND rounds_remaining IS NOT NULL AND is_active = 1",
        )
        .bind(a.rounds)
        .run()
      const expired = await db
        .prepare(
          "UPDATE custom_effects SET is_active = 0 WHERE duration_type = 'rounds' AND rounds_remaining <= 0 AND is_active = 1",
        )
        .run()
      return ok({
        success: true,
        actionType: 'tick',
        roundsAdvanced: a.rounds,
    ...
    case 'list_by_target': {
      ...
      const { results } = await db
        .prepare(
          'SELECT id, name, category, power_level, duration_type, rounds_remaining, is_active FROM custom_effects WHERE target_id = ? ORDER BY is_active DESC, created_at DESC',
        )
        .bind(a.targetId)
        .all()
      return ok({
        success: true,
        actionType: 'list_by_target',
        targetId: a.targetId,
        effects: results,
