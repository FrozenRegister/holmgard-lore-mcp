...
async function loadScroll(
  db: D1Database,
  id: string,
): Promise<{ name: string; properties: ScrollProperties } | null> {
  const row = (await db
    .prepare("SELECT name, properties FROM items WHERE id = ? AND type = 'scroll'")
    .bind(id)
    .first()) as { name: string; properties: string | null } | null
  if (!row) return null
  return {
    name: row.name,
    properties: row.properties
      ? JSON.parse(row.properties)
      : { spellName: '', spellLevel: 0, saveDc: 10, charges: 0, identified: false },
  }
}

export async function handleScrollManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'create': {
      ...
      await db
        .prepare(
          'INSERT INTO items (id, name, description, type, weight, value, properties, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(
          id,
          a.name,
    ...
    case 'use': {
      ...
      await db
        .prepare('UPDATE items SET properties = ?, updated_at = ? WHERE id = ?')
        .bind(JSON.stringify(scroll.properties), now, a.id)
        .run()
      return ok({
        success: true,
        actionType: 'use',
        scrollId: a.id,
        casterId: a.casterId ?? null,
        spellName: scroll.properties.spellName,
        spellLevel: scroll.properties.spellLevel,
        saveDc: scroll.properties.saveDc,
    ...
    case 'get_details': {
      ...
      if (!scroll.properties.identified) {
        return ok({
          ...
          name: scroll.name,
          identified: false,
          note: 'Unidentified — use action "identify" to reveal spell details.',
        })
      }
      return ok({
        success: true,
        ...
        spellLevel: scroll.properties.spellLevel,
        saveDc: scroll.properties.saveDc,
        charges: scroll.properties.charges,
      })
    }
  }
}
