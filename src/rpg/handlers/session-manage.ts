  ...
  switch (match.matched) {
    case 'initialize': {
      ...
      if (!worldId && a.createNew) {
        ...
        const w = (await db
          .prepare('SELECT id FROM worlds ORDER BY created_at DESC LIMIT 1')
      ...
      if (!partyId && a.createNew) {
        ...
        await db
          .prepare(
            'INSERT INTO parties (id, name, status, formation, world_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
          )
          .bind(
            partyId,
            a.partyName ?? 'Adventuring Party',
            'active',
            'standard',
            worldId ?? null,
            now,
        ...
        const p = (await db
          .prepare('SELECT id FROM parties WHERE status = ? ORDER BY created_at DESC LIMIT 1')
          .bind('active')
          .first()) as { id: string } | null
        partyId = p?.id
      }
      ...
      const [world, party] = await Promise.all([
        worldId
          ? db.prepare('SELECT id, name FROM worlds WHERE id = ?').bind(worldId).first()
          : null,
        partyId
          ? db.prepare('SELECT id, name FROM parties WHERE id = ?').bind(partyId).first()
          : null,
      ])
      ...
      if (partyId) {
        const res = await db
          .prepare(
            'SELECT c.id, c.name, c.character_class, c.level, c.hp, c.max_hp, pm.role FROM party_members pm JOIN characters c ON pm.character_id = c.id WHERE pm.party_id = ?',
          )
          .bind(partyId)
          .all()
        members = res.results
    ...
    case 'get_context': {
      ...
      if (a.includeParty && a.partyId) {
        const party = await db
          .prepare('SELECT id, name FROM parties WHERE id = ?')
          .bind(a.partyId)
          .first()
        if (party) {
          const { results: members } = await db
            .prepare(
              'SELECT c.id, c.name, c.level, c.hp, c.max_hp, c.ac, pm.role FROM party_members pm JOIN characters c ON pm.character_id = c.id WHERE pm.party_id = ?',
            )
            .bind(a.partyId)
            .all()
      ...
      if (a.includeQuests && a.worldId) {
        const { results } = await db
          .prepare(
            "SELECT id, name, status FROM quests WHERE world_id = ? AND status IN ('active', 'in_progress') ORDER BY created_at DESC LIMIT 10",
          )
          .bind(a.worldId)
          .all()
      ...
      if (a.includeWorld && a.worldId) {
        const world = await db
          .prepare('SELECT id, name FROM worlds WHERE id = ?')
          .bind(a.worldId)
          .first()
        context.world = world
      }
      ...
      if (a.includeNarrative && a.worldId) {
        const { results } = await db
          .prepare(
            "SELECT id, type, content, created_at FROM narrative_notes WHERE world_id = ? AND status = 'active' ORDER BY created_at DESC LIMIT ?",
          )
          .bind(a.worldId, a.narrativeLimit)
          .all()
      ...
      if (a.includeCombat) {
        const enc = await db
          .prepare(
            "SELECT id, round, status FROM encounters WHERE status = 'active' ORDER BY updated_at DESC LIMIT 1",
          )
          .first()
