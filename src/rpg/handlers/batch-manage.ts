...
const ALIASES: Record<string, BatchAction> = {
  bulk_characters: 'batch_create_characters',
  create_characters: 'batch_create_characters',
  many_characters: 'batch_create_characters',
  bulk_npcs: 'batch_create_npcs',
  create_npcs: 'batch_create_npcs',
  many_npcs: 'batch_create_npcs',
  distribute: 'batch_distribute_items',
  give_items: 'batch_distribute_items',
  bulk_items: 'batch_distribute_items',
...
const BUILT_IN_TEMPLATES = [
  {
    id: 'party-4',
    name: 'Standard Party (4 PCs)',
    category: 'party',
    description: 'Warrior, Mage, Rogue, Cleric',
  },
  {
    id: 'dungeon-guards',
    name: 'Dungeon Guards (6 NPCs)',
    category: 'encounter',
    description: '4 guards + 2 lieutenants',
  },
  {
    id: 'merchant-caravan',
    name: 'Merchant Caravan',
    category: 'npc',
    description: 'Merchant, 2 guards, handler',
  },
  {
    id: 'bandit-gang',
    name: 'Bandit Gang',
    category: 'encounter',
    description: 'Leader + 5 bandits',
  },
]

export async function handleBatchManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    ...
    case 'batch_create_npcs': {
      if (a.characters.length === 0)
        return err('"characters" array is required and must not be empty')
        ...
        try {
          ...
          `,
            ...
            .bind(
              ...
              JSON.stringify(spec.resistances ?? []),
              JSON.stringify(spec.vulnerabilities ?? []),
              JSON.stringify(spec.immunities ?? []),
              JSON.stringify(spec.knownSpells ?? []),
              JSON.stringify(spec.preparedSpells ?? []),
              JSON.stringify(spec.cantripsKnown ?? []),
              spec.spellSlots ? JSON.stringify(spec.spellSlots) : null,
              spec.pactMagicSlots ? JSON.stringify(spec.pactMagicSlots) : null,
              spec.maxSpellLevel ?? 0,
              spec.concentratingOn ?? null,
              spec.legendaryActions ?? null,
              spec.legendaryActionsRemaining ?? null,
              spec.legendaryResistances ?? null,
              spec.legendaryResistancesRemaining ?? null,
              spec.hasLairActions ? 1 : 0,
              JSON.stringify(currency),
              spec.currentRoomId ?? null,
              spec.perceptionBonus ?? 0,
              spec.stealthBonus ?? 0,
              JSON.stringify(spec.resourcePools ?? {}),
              0,
              now,
              now,
            )
          ...
          errors.push(
            `Failed to create "${spec.name}": ${e instanceof Error ? e.message : 'unknown error'}`,
          )
        }
      }
      return ok({
    ...
    case 'batch_distribute_items': {
      ...
      for (const dist of a.distributions) {
        try {
          await db
            .prepare(
              'INSERT INTO inventory_items (character_id, item_id, quantity, slot) VALUES (?, ?, ?, ?) ON CONFLICT(character_id, item_id) DO UPDATE SET quantity = quantity + excluded.quantity',
            )
            .bind(dist.characterId, dist.itemId, dist.quantity, null)
            .run()
      ...
      return ok({
        success: true,
        actionType: 'batch_distribute_items',
        results,
    ...
    case 'execute_workflow': {
      ...
      return ok({
        ...
        stepsExecuted: a.steps.length,
        stepResults,
        note: 'Workflow recorded; individual tool calls must be made separately via tools/call',
      })
    }
    case 'list_templates': {
      const filtered = a.category
        ? BUILT_IN_TEMPLATES.filter((t) => t.category === a.category)
        : BUILT_IN_TEMPLATES
      return ok({
        success: true,
    ...
    case 'get_template': {
      ...
      const tmpl = a.templateId
        ...
        : BUILT_IN_TEMPLATES.find((t) =>
            t.name.toLowerCase().includes((a.templateName ?? '').toLowerCase()),
          )
      if (!tmpl) return err(`Template not found: ${a.templateId ?? a.templateName}`)
      const templateData: Record<string, unknown> = {
        'party-4': {
          characters: [
            { name: 'Warrior', characterClass: 'Fighter', characterType: 'pc' },
            { name: 'Mage', characterClass: 'Wizard', characterType: 'pc' },
            { name: 'Rogue', characterClass: 'Rogue', characterType: 'pc' },
            { name: 'Cleric', characterClass: 'Cleric', characterType: 'pc' },
          ],
        },
        'dungeon-guards': {
          characters: [
            { name: 'Guard 1', characterClass: 'Fighter', characterType: 'enemy' },
            { name: 'Guard 2', characterClass: 'Fighter', characterType: 'enemy' },
            { name: 'Guard 3', characterClass: 'Fighter', characterType: 'enemy' },
            { name: 'Guard 4', characterClass: 'Fighter', characterType: 'enemy' },
            { name: 'Lieutenant 1', characterClass: 'Fighter', level: 3, characterType: 'enemy' },
            { name: 'Lieutenant 2', characterClass: 'Fighter', level: 3, characterType: 'enemy' },
          ],
        },
        'merchant-caravan': {
          characters: [
            { name: 'Merchant', characterClass: 'Commoner', characterType: 'npc' },
            { name: 'Guard', characterClass: 'Fighter', characterType: 'npc' },
            { name: 'Guard', characterClass: 'Fighter', characterType: 'npc' },
            { name: 'Handler', characterClass: 'Commoner', characterType: 'npc' },
          ],
        },
        'bandit-gang': {
          characters: [
            { name: 'Bandit Leader', characterClass: 'Fighter', level: 3, characterType: 'enemy' },
