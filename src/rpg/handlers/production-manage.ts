...
import {
  ...
  type DegradeResult,
} from './resource-manage'
import {
  runProductionIntervene,
  createPendingVote,
  type ProductionInterveneResult,
} from './broadcast-manage'
...
function computeHazard(day: number): { level: string; modifier: number; note: string } {
  ...
  if (day <= 20)
    return {
      level: 'high',
      modifier: 6,
      note: '+6%. Predators become more aggressive (curious -> hunting -> territorial -> starving).',
    }
  if (day <= 27)
    return {
      level: 'severe',
      modifier: 10,
      note: '+10%. New predator type introduced. Perimeter patrols active.',
    }
  return {
    level: 'critical',
    modifier: 15,
    note: '+15%. Tarrasque activity increases, extraction window open.',
  }
}
...
function rollWeather(nearBog: boolean): {
  weather: string
...
export async function handleProductionManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'advance_day': {
      ...
      const world = (await db
        .prepare('SELECT width, height FROM worlds WHERE id = ?')
        .bind(a.worldId)
        .first()) as { width: number; height: number } | null
      ...
      if (stateRow) {
        await db
          .prepare(
            'UPDATE world_state SET production_day = ?, perimeter_radius = ?, weather = ?, hazard_level = ?, encounter_modifier = ?, extraction_window = ?, last_advanced_at = ? WHERE world_id = ?',
          )
          .bind(
            day,
            perimeterRadius,
        ...
        await db
          .prepare(
            'INSERT INTO world_state (world_id, production_day, perimeter_radius, weather, hazard_level, encounter_modifier, extraction_window, last_advanced_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          )
          .bind(
            a.worldId,
            day,
      ...
      const { results: scheduled } = await db
        .prepare(
          'SELECT * FROM production_calendar WHERE world_id = ? AND day = ? AND triggered = 0',
        )
        .bind(a.worldId, day)
        .all()
      if (scheduled.length > 0) {
        await db
          .prepare(
            'UPDATE production_calendar SET triggered = 1, triggered_at = ? WHERE world_id = ? AND day = ? AND triggered = 0',
          )
          .bind(now, a.worldId, day)
          .run()
      }

      let crateDrop: Record<string, unknown> | null = null
      if (!a.skipCrateDrop) {
        const crateRes = await handleResourceManage(env, {
          action: 'crate_drop',
          worldId: a.worldId,
          day,
      ...
      const resourceDegradation: DegradeResult[] = await tickAllOwnersDegradation(
        db,
        a.worldId,
        day,
      )
      ...
      const intervention: ProductionInterveneResult = await runProductionIntervene(
        db,
        a.worldId,
        day,
        a.interventionSignals,
      )
      ...
      const { results: upcomingEvents } = await db
        .prepare(
          'SELECT day, event_type, event_data FROM production_calendar WHERE world_id = ? AND day > ? AND triggered = 0 ORDER BY day LIMIT 10',
        )
        .bind(a.worldId, day)
        .all()
      ...
      return ok({
        ...
        hazardNote: hazard.note,
        encounterModifier,
        weather: weatherRoll.weather,
        fog: weatherRoll.fog,
        movementModifier: weatherRoll.movementModifier,
        extractionWindow,
        crateDrop,
        resourceDegradation,
        pendingVoteId,
        intervention,
        corpseDecomposition,
        triggeredScheduledEvents: scheduled,
        upcomingEvents,
      })
    }
    case 'get_state': {
      ...
      const stateRow = (await db
        ...
        .first()) as Record<string, unknown> | null
      if (!stateRow)
        return err(`No production state for world ${a.worldId} — call advance_day at least once`)
      const { results: activePrizes } = await db
        .prepare('SELECT * FROM crate_drops WHERE world_id = ? AND claimed = 0 ORDER BY day DESC')
        .bind(a.worldId)
        .all()
      const { results: upcomingEvents } = await db
        .prepare(
        ...
        .bind(a.worldId, stateRow.production_day)
        .all()
      return ok({
        success: true,
        actionType: 'get_state',
        worldId: a.worldId,
        day: stateRow.production_day,
        perimeterRadius: stateRow.perimeter_radius,
        weather: stateRow.weather,
        hazardLevel: stateRow.hazard_level,
        encounterModifier: stateRow.encounter_modifier,
        extractionWindow: stateRow.extraction_window,
        productionMood: stateRow.production_mood,
        activePrizes,
        upcomingEvents,
      })
    }
    case 'update_state': {
      ...
      if (!a.fields || Object.keys(a.fields).length === 0)
        return err('"fields" must be a non-empty object')
      ...
      const { applied: fieldsApplied, rejected: fieldsRejected } = applyDynamicFields(
        a.fields,
        WORLD_STATE_FIELDS_BLACKLIST,
        sets,
        vals,
      )
      if (sets.length === 0)
        return err('No valid fields to update', { fields_rejected: fieldsRejected })
      vals.push(a.worldId)
      await db
        .prepare(`UPDATE world_state SET ${sets.join(', ')} WHERE world_id = ?`)
        .bind(...vals)
        .run()
      return ok({
        success: true,
    ...
    case 'set_schedule': {
      ...
      for (const event of a.events) {
        ...
        await db
          .prepare(
            'INSERT INTO production_calendar (id, world_id, day, event_type, event_data, triggered, resolved) VALUES (?, ?, ?, ?, ?, 0, 0) ON CONFLICT(world_id, day, event_type) DO UPDATE SET event_data = excluded.event_data',
          )
          .bind(
            id,
            a.worldId,
      ...
      return ok({
        success: true,
        actionType: 'set_schedule',
        worldId: a.worldId,
        scheduled: a.events.length,
      })
    }
    case 'list_events': {
      if (!a.worldId) return err('"worldId" is required')
      const { results } = await db
        .prepare('SELECT * FROM production_calendar WHERE world_id = ? AND day >= ? ORDER BY day')
        .bind(a.worldId, a.fromDay)
        .all()
      return ok({
        success: true,
