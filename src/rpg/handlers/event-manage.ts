...
export async function handleEventManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'emit': {
      ...
      const result = await db
        .prepare(
          'INSERT INTO event_inbox (event_type, payload, source_type, source_id, priority) VALUES (?, ?, ?, ?, ?)',
        )
        .bind(
          a.eventType,
    ...
    case 'poll': {
      ...
      const { results } = await db
        ...
        .bind(...binds)
        .all()
      const events = (results as Array<Record<string, unknown>>).map((r) => ({
        ...r,
    ...
    case 'ack': {
      ...
      for (const id of ids)
        await db.prepare('UPDATE event_inbox SET consumed_at = ? WHERE id = ?').bind(now, id).run()
    ...
    case 'list_types': {
      return ok({
        ...
        note: 'eventType accepts any string; these are known types but custom types are permitted',
      })
    }
  }
}
