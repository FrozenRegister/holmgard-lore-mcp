...
const InputSchema = z.object({
  ...
  terrain: z.array(z.object({ x: z.number().int(), y: z.number().int(), type: z.string() })).optional(),
...
function renderGrid(gridData: { width: number; height: number; terrain?: Array<{ x: number; y: number; type: string }>; tokens?: Array<{ id: string; x: number; y: number; symbol?: string }> }): string {
  ...
  for (const t of (gridData.terrain ?? [])) { if (t.x < width && t.y < height) grid[t.y][t.x] = '#' }
  for (const t of (gridData.tokens ?? [])) { if (t.x < width && t.y < height) grid[t.y][t.x] = t.symbol ?? '@' }
...
export async function handleCombatMap(env: AppBindings, args: Record<string, unknown>): Promise<McpResponse> {
  ...
  switch (match.matched) {
    ...
    case 'get': {
      ...
      const row = a.id
        ...
        : await db.prepare('SELECT * FROM battlefield WHERE encounter_id = ? LIMIT 1').bind(id).first()
      if (!row) return err(`Battlefield not found`)
      return ok({ success: true, actionType: 'get', map: { ...(row as Record<string, unknown>), grid_data: JSON.parse((row as any).grid_data ?? '{}') } })
    }
    case 'update': {
    ...
    case 'move_token': {
      if (!a.id || !a.tokenId || a.x === undefined || a.y === undefined) return err('"id", "tokenId", "x", and "y" are required')
      const row = await db.prepare('SELECT grid_data FROM battlefield WHERE id = ?').bind(a.id).first() as { grid_data: string } | null
      if (!row) return err(`Battlefield not found: ${a.id}`)
      const gridData = JSON.parse(row.grid_data)
      if (!gridData.tokens) gridData.tokens = []
      const tokenIdx = gridData.tokens.findIndex((t: { id: string }) => t.id === a.tokenId)
      if (tokenIdx >= 0) { gridData.tokens[tokenIdx].x = a.x; gridData.tokens[tokenIdx].y = a.y }
      else gridData.tokens.push({ id: a.tokenId, x: a.x, y: a.y })
      await db.prepare('UPDATE battlefield SET grid_data = ?, updated_at = ? WHERE id = ?').bind(JSON.stringify(gridData), now, a.id).run()
      return ok({ success: true, actionType: 'move_token', mapId: a.id, tokenId: a.tokenId, x: a.x, y: a.y })
    }
    case 'render': {
      const id = a.id ?? a.encounterId
      if (!id) return err('"id" or "encounterId" is required')
