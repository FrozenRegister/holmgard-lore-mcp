...
export async function handleNarrativeManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'create': {
      ...
      await db
        .prepare(
          'INSERT INTO narrative_notes (id, world_id, type, content, metadata, visibility, tags, entity_id, entity_type, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(
          id,
          a.worldId,
    ...
    case 'get': {
      if (!a.id)
        return err(
          '"id" (the note UUID returned by create, "noteId") is required. Use "list" with worldId to find narrative note IDs. Example: { action: "get", id: "uuid-of-the-note", worldId: "..." }',
        )
    ...
    case 'list': {
      let query =
        'SELECT id, type, content, status, visibility, created_at FROM narrative_notes WHERE world_id = ?'
      const binds: unknown[] = [a.worldId ?? '']
      if (a.filter?.type) {
        query += ' AND type = ?'
        binds.push(a.filter.type)
      }
      if (a.filter?.status) {
        query += ' AND status = ?'
        binds.push(a.filter.status)
      }
      if (a.filter?.entityId) {
        query += ' AND entity_id = ?'
        binds.push(a.filter.entityId)
      }
    ...
    case 'update': {
      ...
      if (a.visibility) {
        ...
        vals.push(a.visibility)
      }
      if (a.tags) {
        sets.push('tags = ?')
        vals.push(JSON.stringify(a.tags))
      }
      if (a.status) {
        sets.push('status = ?')
        vals.push(a.status)
      ...
      if (a.metadata) {
        sets.push('metadata = ?')
        vals.push(JSON.stringify(a.metadata))
      }
      vals.push(a.id)
      await db
        .prepare(`UPDATE narrative_notes SET ${sets.join(', ')} WHERE id = ?`)
        .bind(...vals)
        .run()
    ...
    case 'archive': {
      if (!a.id) return err('"id" is required')
      await db
        .prepare("UPDATE narrative_notes SET status = 'archived', updated_at = ? WHERE id = ?")
        .bind(now, a.id)
        .run()
    ...
    case 'resolve': {
      if (!a.id) return err('"id" is required')
      await db
        .prepare("UPDATE narrative_notes SET status = 'resolved', updated_at = ? WHERE id = ?")
        .bind(now, a.id)
        .run()
