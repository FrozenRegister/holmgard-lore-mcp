...
const ALIASES: Record<string, CombatActionType> = {
  ...
  add_condition: 'apply_condition',
  afflict: 'apply_condition',
  cure_condition: 'remove_condition',
  end_condition: 'remove_condition',
  ability: 'use_ability',
  special: 'use_ability',
  skill: 'use_ability',
  log: 'get_log',
  history: 'get_log',
  summary: 'get_turn_summary',
...
async function toggleCondition(
  ...
  const row = (await db
    .prepare('SELECT conditions FROM characters WHERE id = ?')
    .bind(characterId)
    .first()) as { conditions: string } | null
  if (!row) return
  ...
  await db
    .prepare('UPDATE characters SET conditions = ? WHERE id = ?')
    .bind(JSON.stringify(next), characterId)
    .run()
}
...
export async function handleCombatAction(
  ...
  async function logAction(
    ...
    summary: string,
    detail: unknown = null,
    damageDealt: number | null = null,
    healingDone: number | null = null,
    hpChanges: unknown = null,
  ) {
    if (!a.encounterId) return
    await db
      .prepare(
        'INSERT INTO combat_action_log (encounter_id, round, turn_index, actor_id, actor_name, action_type, target_ids, result_summary, result_detail, damage_dealt, healing_done, hp_changes, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      )
      .bind(
        a.encounterId,
        a.round,
        ...
        a.actorName ?? 'Unknown',
        actionType,
        a.targetIds ? JSON.stringify(a.targetIds) : null,
        summary,
        detail ? JSON.stringify(detail) : null,
        damageDealt,
        healingDone,
        hpChanges ? JSON.stringify(hpChanges) : null,
        now,
      )
      .run()
  }
  ...
  switch (match.matched) {
    case 'attack': {
      ...
      // must not touch.
      const { results: stagedTargets } = (await db
        .prepare(
          `SELECT id, name FROM characters WHERE id IN (${a.targetIds.map(() => '?').join(',')}) AND death_mode = 'staged'`,
        )
        .bind(...a.targetIds)
        .all()) as { results: Array<{ id: string; name: string }> }
      if (stagedTargets.length > 0) {
        return err(
          `Cannot attack staged-dissolution character(s): ${stagedTargets.map((t) => t.name).join(', ')}`,
        )
      }
      ...
      const attackRollResult =
        a.attackRoll !== undefined
          ? {
              total: a.attackRoll,
              rolls: [a.attackRoll],
              steps: [`Supplied roll: ${a.attackRoll}`],
              critical:
                a.attackRoll === 20
                  ? ('success' as const)
                  : a.attackRoll === 1
      ...
      const summary = hit
        ? `${a.actorName ?? a.actorId} hit for ${damage} ${a.damageType ?? 'damage'}${isCrit ? ' (CRITICAL!)' : ''}`
        : `${a.actorName ?? a.actorId} missed${isFumble ? ' (FUMBLE!)' : ''}`
      await logAction(
        'attack',
        summary,
        { hit, attackRoll, isCrit, isFumble, damageRoll },
        hit ? damage : null,
        null,
        null,
      )
    ...
    case 'apply_damage': {
      if (!a.targetIds?.length || a.damage === undefined)
        return err('"targetIds" and "damage" are required')
      ...
      for (const targetId of a.targetIds) {
        ...
        const char = (await db
          .prepare(
            'SELECT hp, resistances, vulnerabilities, immunities, concentrating_on FROM characters WHERE id = ?',
          )
          .bind(hostId)
          .first()) as {
          hp: number
        ...
        if (char) {
          ...
          const vulnerabilities: string[] = char.vulnerabilities
            ? JSON.parse(char.vulnerabilities)
            : []
          let effectiveDamage = a.damage
          if (a.damageType && immunities.includes(a.damageType)) effectiveDamage = 0
          else if (a.damageType && resistances.includes(a.damageType))
            effectiveDamage = Math.floor(effectiveDamage / 2)
          else if (a.damageType && vulnerabilities.includes(a.damageType))
          ...
          if (char.concentrating_on && effectiveDamage > 0)
            concentrationChecks[targetId] = Math.max(10, Math.floor(effectiveDamage / 2))
        }
      }
    ...
    case 'heal': {
      if (!a.targetIds?.length || a.healAmount === undefined)
        return err('"targetIds" and "healAmount" are required')
      const hpChanges: Record<string, number> = {}
      for (const targetId of a.targetIds) {
        ...
        const hostId = resolution?.hostBodyId ?? targetId
        const char = (await db
          .prepare('SELECT hp, max_hp FROM characters WHERE id = ?')
          .bind(hostId)
          .first()) as { hp: number; max_hp: number } | null
        if (char) {
    ...
    case 'apply_condition': {
      if (!a.targetIds?.length || !a.conditionName)
        return err('"targetIds" and "conditionName" are required')
      for (const targetId of a.targetIds) {
        const char = (await db
          .prepare('SELECT conditions FROM characters WHERE id = ?')
          .bind(targetId)
        ...
        if (char) {
          ...
          await db
            .prepare('UPDATE characters SET conditions = ? WHERE id = ?')
            .bind(JSON.stringify(conditions), targetId)
            .run()
        }
      }
    ...
    case 'remove_condition': {
      ...
      for (const targetId of a.targetIds) {
        const char = (await db
          ...
          .first()) as { conditions: string } | null
        if (char) {
          const conditions = JSON.parse(char.conditions ?? '[]').filter(
            (c: string) => c !== a.conditionName,
          )
          await db
            .prepare('UPDATE characters SET conditions = ? WHERE id = ?')
            .bind(JSON.stringify(conditions), targetId)
    ...
    case 'get_log': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM combat_action_log WHERE encounter_id = ? ORDER BY round, turn_index DESC LIMIT ?',
        )
        .bind(a.encounterId, a.limit)
        .all()
      return ok({
        success: true,
        actionType: 'get_log',
    ...
    case 'get_turn_summary': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM combat_action_log WHERE encounter_id = ? AND round = ? ORDER BY turn_index',
        )
        .bind(a.encounterId, a.round)
        .all()
      return ok({
        success: true,
        actionType: 'get_turn_summary',
        encounterId: a.encounterId,
        round: a.round,
        actions: results,
      })
    ...
    case 'ready': {
      if (!a.actorId || !a.description)
        return err('"actorId" and "description" (the trigger + held action) are required')
