...
const ALIASES: Record<string, TurnAction> = {
  ...
  post_actions: 'submit_actions',
  ready: 'mark_ready',
  confirm: 'mark_ready',
  done: 'mark_ready',
  poll: 'poll_results',
  results: 'poll_results',
  get_results: 'poll_results',
}
...
export async function handleTurnManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'init': {
      ...
      const existing = await db
        .prepare('SELECT world_id FROM turn_state WHERE world_id = ?')
        .bind(a.worldId)
        .first()
      if (existing)
        return err(
          `Turn state already initialized for world ${a.worldId}. Use get_status to check current state.`,
        )
      await db
        .prepare(
          'INSERT INTO turn_state (world_id, current_turn, turn_phase, phase_started_at, nations_ready, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(a.worldId, 1, 'planning', now, '[]', now, now)
        .run()
      return ok({
        success: true,
        actionType: 'init',
    ...
    case 'get_status': {
      ...
      return ok({
        ...
        worldId: a.worldId,
        currentTurn: ts.current_turn,
        turnPhase: ts.turn_phase,
        phaseStartedAt: ts.phase_started_at,
        nationsReady: JSON.parse((ts.nations_ready as string) ?? '[]'),
      })
    }
    case 'submit_actions': {
      if (!a.worldId) return err('"worldId" is required')
      ...
      const ts = (await db
        .prepare('SELECT current_turn FROM turn_state WHERE world_id = ?')
        .bind(a.worldId)
        .first()) as { current_turn: number } | null
      if (!ts) return err(`No turn state for world ${a.worldId}.`)
      await db
        .prepare('INSERT INTO event_logs (type, payload, timestamp) VALUES (?, ?, ?)')
        .bind(
          'turn_action_submitted',
          JSON.stringify({
            worldId: a.worldId,
            turn: ts.current_turn,
            entityId,
            actions: a.actions,
          }),
          now,
    ...
    case 'mark_ready': {
      ...
      if (!entityId) return err('"nationId" or "partyId" is required')
      const ts = (await db
        .prepare('SELECT nations_ready FROM turn_state WHERE world_id = ?')
        .bind(a.worldId)
        .first()) as { nations_ready: string } | null
      if (!ts) return err(`No turn state for world ${a.worldId}.`)
      const ready = JSON.parse(ts.nations_ready ?? '[]') as string[]
      if (!ready.includes(entityId)) ready.push(entityId)
      await db
        .prepare('UPDATE turn_state SET nations_ready = ?, updated_at = ? WHERE world_id = ?')
        .bind(JSON.stringify(ready), now, a.worldId)
        .run()
      return ok({
        success: true,
    ...
    case 'poll_results': {
      ...
      const { results: recentEvents } = await db
        .prepare(
          "SELECT * FROM event_logs WHERE type = 'turn_action_submitted' AND json_extract(payload, '$.worldId') = ? ORDER BY timestamp DESC LIMIT 20",
        )
        .bind(a.worldId)
        .all()
      return ok({
