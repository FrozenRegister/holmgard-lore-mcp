...
const InputSchema = z.object({
  ...
  filter: z
    .object({ revealed: z.boolean().optional(), linkedEntityId: z.string().optional() })
    .optional(),
...
export async function handleSecretManage(
  env: AppBindings,
  ...
  switch (match.matched) {
    case 'create': {
      if (!a.worldId || !a.name || !a.publicDescription || !a.secretDescription)
        return err('"worldId", "name", "publicDescription", and "secretDescription" are required')
      const id = crypto.randomUUID()
      await db
        .prepare(
          'INSERT INTO secrets (id, world_id, type, category, name, public_description, secret_description, linked_entity_id, linked_entity_type, reveal_conditions, sensitivity, leak_patterns, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        )
        .bind(
          id,
          a.worldId,
          ...
          a.linkedEntityType ?? null,
          JSON.stringify(a.revealConditions ?? []),
          a.sensitivity,
          '[]',
          now,
          now,
    ...
    case 'get': {
      if (!a.id)
        return err(
          '"id" (the secret UUID returned by create, e.g. "uuid-of-the-secret") is required. Use "list" with worldId to find secret IDs, or filter by linkedEntityId',
        )
      ...
      return ok({
        ...
        secret: {
          ...row,
          reveal_conditions: JSON.parse((row as any).reveal_conditions ?? '[]'),
          leak_patterns: JSON.parse((row as any).leak_patterns ?? '[]'),
        },
      })
    }
    case 'list': {
      let query =
        'SELECT id, name, type, category, sensitivity, revealed, created_at FROM secrets WHERE world_id = ?'
      const binds: unknown[] = [a.worldId ?? '']
      if (a.filter?.revealed !== undefined) {
        query += ' AND revealed = ?'
        binds.push(a.filter.revealed ? 1 : 0)
      }
      if (a.filter?.linkedEntityId) {
        query += ' AND linked_entity_id = ?'
        binds.push(a.filter.linkedEntityId)
      }
    ...
    case 'update': {
      ...
      if (a.publicDescription) {
        ...
        vals.push(a.publicDescription)
      }
      if (a.secretDescription) {
        sets.push('secret_description = ?')
        vals.push(a.secretDescription)
      }
      if (a.sensitivity) {
        sets.push('sensitivity = ?')
        vals.push(a.sensitivity)
      }
      if (a.revealConditions) {
        sets.push('reveal_conditions = ?')
        vals.push(JSON.stringify(a.revealConditions))
      }
      const { applied: fieldsApplied, rejected: fieldsRejected } = applyDynamicFields(
        a.fields,
        SECRET_FIELDS_BLACKLIST,
        sets,
        vals,
      )
      vals.push(a.id)
    ...
    case 'reveal': {
      ...
      await db
        .prepare(
          'UPDATE secrets SET revealed = 1, revealed_at = ?, revealed_by = ?, updated_at = ? WHERE id = ?',
        )
        .bind(now, a.revealedBy ?? null, now, a.id)
        .run()
    ...
    case 'check_reveal': {
      if (!a.id) return err('"id" is required')
      const row = await db
        .prepare('SELECT revealed, sensitivity, reveal_conditions, notes FROM secrets WHERE id = ?')
        .bind(a.id)
        .first()
      if (!row) return err(`Secret not found: ${a.id}`)
      ...
      return ok({
        success: true,
        actionType: 'check_reveal',
        id: a.id,
        alreadyRevealed: !!(row as any).revealed,
        revealConditions: conditions,
        sensitivity: (row as any).sensitivity,
      })
    }
  }
}
