...
export async function snapshotWorldState(db: D1Database, worldId: string): Promise<WorldSnapshot> {
  const [ws, parties, characters, encounters] = await Promise.all([
    ...
    db.prepare('SELECT * FROM encounters WHERE world_id = ? LIMIT 1000').bind(worldId).all() as Promise<{ results: any[] }>,
  ])
  ...
  return {
    ...
    encounters: new Map((encounters?.results || []).map((e: any) => [e.id, e])),
