  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        name: { type: 'string', description: 'Character name (required for create; used for get/find_by_name)' },
        characterType: {
          type: 'string',
        ...
        level: { type: 'integer', minimum: 1, maximum: 20, description: 'Character level (default: 1)' },
        ...
        currentRoomId: { type: 'string', nullable: true, description: 'Current room ID (null to clear)' },
        hostBodyId: { type: 'string', nullable: true, description: 'Host body ID for co-habitation' },
        ...
        perceptionBonus: { type: 'integer', description: 'Perception bonus (auto-derived from WIS if omitted)' },
        stealthBonus: { type: 'integer', description: 'Stealth bonus (auto-derived from DEX if omitted)' },
        ...
        xpAmount: { type: 'integer', minimum: 0, description: 'XP amount to add (alias for amount)' },
        limit: { type: 'integer', minimum: 1, maximum: 200, description: 'Max results for list/search' },
        characterTypeFilter: {
          type: 'string',
          enum: ['pc', 'npc', 'enemy', 'neutral'],
        ...
        preparedSpells: { type: 'array', items: { type: 'string' }, description: 'Prepared spells' },
        cantripsKnown: { type: 'array', items: { type: 'string' }, description: 'Known cantrips' },
        maxSpellLevel: { type: 'integer', minimum: 0, maximum: 9, description: 'Maximum spell level' },
        ...
        triggerProductionPulse: { type: 'boolean', description: 'Trigger production pulse on death' },
        killedBy: { type: 'string', description: 'Alias for killerId' },
        concentratingOn: { type: 'string', nullable: true, description: 'Currently concentrating on spell' },
        legendaryActions: { type: 'integer', minimum: 0, description: 'Legendary actions per round' },
        legendaryActionsRemaining: { type: 'integer', minimum: 0, description: 'Legendary actions remaining' },
        legendaryResistances: { type: 'integer', minimum: 0, description: 'Legendary resistances per day' },
        legendaryResistancesRemaining: { type: 'integer', minimum: 0, description: 'Legendary resistances remaining' },
        ...
        currency: { type: 'object', description: 'Currency (e.g. { gold: 0, silver: 0, copper: 0 })' },
        spellName: { type: 'string', description: 'Spell name (cast_spell action)' },
        slotLevel: { type: 'integer', minimum: 0, maximum: 9, description: 'Spell slot level to expend' },
        usePactMagic: { type: 'boolean', description: 'Use pact magic slot instead of spell slot' },
        requiresConcentration: { type: 'boolean', description: 'Whether spell requires concentration' },
        ...
        dissolutionTerminal: { type: 'string', nullable: true, description: 'Terminal dissolution state' },
        dissolutionId: { type: 'string', nullable: true, description: 'Dissolution process ID' },
        fields: {
          type: 'object',
          description: 'Arbitrary D1 column passthrough for update (blacklist: id, created_at, updated_at, world_id)',
          additionalProperties: {
