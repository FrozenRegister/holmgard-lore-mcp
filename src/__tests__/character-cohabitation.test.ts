...
describe('Character Co-Habitation (#226 Phase 2)', () => {
  ...
  it('create accepts hostBodyId/active directly', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Katerina Sloane' }))
    const hostId = (hostRes as any).characterId

    const passengerRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Bellona Keel', hostBodyId: hostId, active: false,
    }))
    expect(passengerRes).toHaveProperty('success', true)
    const passengerId = (passengerRes as any).characterId

    const getRes = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: passengerId }))
  ...
  it('update sets hostBodyId/active as a raw single-row PATCH that does not cascade', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Host Body' }))
    const hostId = (hostRes as any).characterId

    const aRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Consciousness A', hostBodyId: hostId }))
    const aId = (aRes as any).characterId
    const bRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Consciousness B', hostBodyId: hostId }))
    ...
    const aAfter = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: aId }))
    const bAfter = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: bId }))
  ...
  it('activate atomically activates the target and deactivates its siblings', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Katerina Sloane 2' }))
    const hostId = (hostRes as any).characterId

    const cordeliaRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Cordelia Keel 2', hostBodyId: hostId, active: true,
    }))
    const cordeliaId = (cordeliaRes as any).characterId
    const bellonaRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Bellona Keel 2', hostBodyId: hostId, active: false,
    }))
    const bellonaId = (bellonaRes as any).characterId

    const activateRes = parseResponse(await handleCharacterManage(testEnv, { action: 'activate', characterId: bellonaId }))
    ...
    const cordeliaAfter = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: cordeliaId }))
    const bellonaAfter = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: bellonaId }))
  ...
  it('activate syncs KV projections for the activated character and every deactivated sibling', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Katerina Sloane 3' }))
    const hostId = (hostRes as any).characterId
    const cordeliaRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Cordelia Keel 3', hostBodyId: hostId, active: true,
    }))
    const cordeliaId = (cordeliaRes as any).characterId
    await handleCharacterManage(testEnv, { action: 'create', name: 'Bellona Keel 3', hostBodyId: hostId, active: false })
  ...
  it('activate on a character with no host_body_id is a harmless solo activation', async () => {
    ...
    const soloRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Solo Character' }))
    const soloId = (soloRes as any).characterId

    const activateRes = parseResponse(await handleCharacterManage(testEnv, { action: 'activate', characterId: soloId }))
    expect(activateRes).toHaveProperty('success', true)
  ...
  it('activate accepts an explicit hostBodyId to assign-and-activate in one call', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Katerina Sloane 4' }))
    const hostId = (hostRes as any).characterId
    // Created with no host_body_id at all
    const freshRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Fresh Consciousness' }))
    const freshId = (freshRes as any).characterId

    const activateRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'activate', characterId: freshId, hostBodyId: hostId,
    }))
    expect((activateRes as any).hostBodyId).toBe(hostId)

    const freshAfter = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: freshId }))
  ...
  it('activate reassigning to a different host group leaves the old group untouched', async () => {
    ...
    const hostARes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Host A' }))
    const hostAId = (hostARes as any).characterId
    const hostBRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Host B' }))
    const hostBId = (hostBRes as any).characterId

    const siblingRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Group A Sibling', hostBodyId: hostAId, active: true,
    }))
    const siblingId = (siblingRes as any).characterId
    const movingRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Moving Consciousness', hostBodyId: hostAId, active: false,
    }))
    const movingId = (movingRes as any).characterId

    // Reassign "Moving Consciousness" to Host B instead
    await handleCharacterManage(testEnv, { action: 'activate', characterId: movingId, hostBodyId: hostBId })

    const siblingAfter = parseResponse(await handleCharacterManage(testEnv, { action: 'get', characterId: siblingId }))
  ...
  it('activate returns an error for a non-existent character', async () => {
    const testEnv = env as unknown as AppBindings
    const res = parseResponse(await handleCharacterManage(testEnv, { action: 'activate', characterId: 'non-existent-id' }))
  ...
  it('activate supports switch/take_control/possess aliases', async () => {
    const testEnv = env as unknown as AppBindings
    const soloRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Alias Test Char' }))
    const soloId = (soloRes as any).characterId

    const switchRes = parseResponse(await handleCharacterManage(testEnv, { action: 'switch', characterId: soloId }))
    expect((switchRes as any).actionType).toBe('activate')
    const possessRes = parseResponse(await handleCharacterManage(testEnv, { action: 'possess', characterId: soloId }))
  ...
  it('list_passengers lists dormant passengers and the active consciousness via hostBodyId', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Katerina Sloane 5' }))
    const hostId = (hostRes as any).characterId
    const activeRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Active One', hostBodyId: hostId, active: true,
    }))
    const activeId = (activeRes as any).characterId
    const passengerRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Passenger One', hostBodyId: hostId, active: false,
    }))
    const passengerId = (passengerRes as any).characterId
  ...
  it('list_passengers resolves hostBodyId from a character id when not passed directly', async () => {
    ...
    const hostRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Katerina Sloane 6' }))
    const hostId = (hostRes as any).characterId
    const activeRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Active Two', hostBodyId: hostId, active: true,
    }))
    const activeId = (activeRes as any).characterId
    const passengerRes = parseResponse(await handleCharacterManage(testEnv, {
      action: 'create', name: 'Passenger Two', hostBodyId: hostId, active: false,
    }))
    const passengerId = (passengerRes as any).characterId

    const listRes = parseResponse(await handleCharacterManage(testEnv, { action: 'list_passengers', characterId: passengerId }))
  ...
  it('list_passengers returns an empty result for a character with no host_body_id', async () => {
    const testEnv = env as unknown as AppBindings
    const soloRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'No Group Character' }))
    const soloId = (soloRes as any).characterId

    const listRes = parseResponse(await handleCharacterManage(testEnv, { action: 'list_passengers', characterId: soloId }))
    expect(listRes).toHaveProperty('success', true)
  ...
  it('list_passengers returns an error for a non-existent character', async () => {
    const testEnv = env as unknown as AppBindings
    const res = parseResponse(await handleCharacterManage(testEnv, { action: 'list_passengers', characterId: 'non-existent-id' }))
    expect(res).toHaveProperty('error', true)
    expect((res as any).message).toContain('not found')
  })
  ...
  it('list_passengers supports passengers/list_dormant/co_habitants aliases', async () => {
    const testEnv = env as unknown as AppBindings
    const soloRes = parseResponse(await handleCharacterManage(testEnv, { action: 'create', name: 'Alias Passenger Char' }))
    const soloId = (soloRes as any).characterId

    const passengersRes = parseResponse(await handleCharacterManage(testEnv, { action: 'passengers', characterId: soloId }))
    expect((passengersRes as any).actionType).toBe('list_passengers')
    const dormantRes = parseResponse(await handleCharacterManage(testEnv, { action: 'list_dormant', characterId: soloId }))
