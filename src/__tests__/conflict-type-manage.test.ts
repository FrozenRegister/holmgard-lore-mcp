...
describe('rpg conflict_type sub', () => {
  ...
  it('create adds a new custom conflict type', async () => {
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Ritual', description: 'A ceremonial contest of wills', resolver: 'drama' })
    expect(r.success).toBe(true)
  ...
  it('create requires name and resolver', async () => {
    const noName = await callTool('rpg', { sub: 'conflict_type', action: 'create', resolver: 'combat' })
    expect(noName.error).toBe(true)
    const noResolver = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Hunt' })
    expect(noResolver.error).toBe(true)
  })

  it('create rejects a duplicate name', async () => {
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Physical', resolver: 'combat' })
  ...
  it('update changes description and resolver', async () => {
    const created = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Chase', resolver: 'combat' })
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'update', id: created.conflictTypeId, description: 'A pursuit', resolver: 'both' })
    expect(r.success).toBe(true)
  ...
  it('update rejects a nonexistent id', async () => {
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'update', id: 'nonexistent', description: 'x' })
    expect(r.error).toBe(true)
  })

  it('update with no fields returns an error', async () => {
    const created = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Empty Update', resolver: 'combat' })
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'update', id: created.conflictTypeId })
    expect(r.error).toBe(true)
  })
  ...
  it('delete removes an unreferenced custom conflict type', async () => {
    const created = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Disposable', resolver: 'combat' })
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'delete', id: created.conflictTypeId })
    expect(r.success).toBe(true)

    const list = await callTool('rpg', { sub: 'conflict_type', action: 'list' })
    expect(list.conflictTypes.some((c: { id: string }) => c.id === created.conflictTypeId)).toBe(false)
  })
  ...
  it('update requires id', async () => {
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'update', description: 'no id given' })
    expect(r.error).toBe(true)
  })
  ...
  it('rejects an invalid resolver value (zod validation failure)', async () => {
    const r = await callTool('rpg', { sub: 'conflict_type', action: 'create', name: 'Bad Resolver', resolver: 'not-a-real-resolver' })
    expect(r.error).toBe(true)
  })
  ...
  it('delete rejects a conflict type still referenced by a scene', async () => {
    ...
    await env.RPG_DB.prepare('INSERT INTO worlds (id, name, seed, width, height, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
      .bind('world:ct-ref', 'World', 'seed', 10, 10, now, now).run()
    const scene = await callTool('rpg', { sub: 'scene', action: 'create', worldId: 'world:ct-ref', title: 'Referenced', narration: 'Tagged.' })
    await callTool('rpg', { sub: 'scene', action: 'set_conflict_type', id: scene.sceneId, conflictTypeId: 'physical' })
