...
describe('handleProductionManage', () => {
  ...
  it('advance_day supports advancing multiple days at once', async () => {
    ...
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 7 })
    const body = JSON.parse(r.content[0].text)
  ...
  it('advance_day computes every hazard band across the 30-day arc', async () => {
    await createWorld()
    const expectations: Array<[number, string]> = [[6, 'standard'], [13, 'elevated'], [20, 'high'], [27, 'severe'], [30, 'critical']]
    for (const [day, level] of expectations) {
      await env.RPG_DB.prepare('DELETE FROM world_state WHERE world_id = ?').bind(WORLD).run()
      const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: day })
  ...
  it('advance_day computes perimeter contraction down to the day-25 floor of 8 and holds there', async () => {
    await createWorld()
    const r25 = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 25 })
    expect(JSON.parse(r25.content[0].text).perimeterRadius).toBe(8)
    const r30 = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 5 })
  ...
  it('advance_day opens the extraction window on day 28-30 and closes it permanently after day 30', async () => {
    await createWorld()
    const r28 = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 28 })
    expect(JSON.parse(r28.content[0].text).extractionWindow).toBe('open')
    const r31 = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 3 })
    expect(JSON.parse(r31.content[0].text).extractionWindow).toBe('closed_final')
  })

  it('advance_day can skip the crate drop', async () => {
    await createWorld()
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, skipCrateDrop: true })
    const body = JSON.parse(r.content[0].text)
  ...
  it('advance_day rolls every weather band deterministically (storm/rain/overcast/clear)', async () => {
    await createWorld()
    const bands: Array<[number, string, number]> = [[0.1, 'storm', -8], [0.3, 'rain', -5], [0.6, 'overcast', 0], [0.9, 'clear', 3]]
  ...
  it('advance_day rolls fog with the nearBog-boosted chance', async () => {
    ...
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, nearBog: true })
    const body = JSON.parse(r.content[0].text)
  ...
  it('advance_day opens a pending audience vote every 3rd day', async () => {
    await createWorld()
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 3 })
    const body = JSON.parse(r.content[0].text)
  ...
  it('advance_day does not open a vote on a non-multiple-of-3 day', async () => {
    await createWorld()
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD, daysToAdvance: 2 })
    const body = JSON.parse(r.content[0].text)
  ...
  it('advance_day ticks resource degradation for every owner holding resources in the world', async () => {
    ...
    await env.RPG_DB.prepare('INSERT INTO resource_inventory (id, world_id, owner_type, owner_id, item_name, category, quantity, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)')
      .bind(crypto.randomUUID(), WORLD, 'character', 'char-1', 'Standard Ration Pack', 'food', now, now).run()
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD })
    const body = JSON.parse(r.content[0].text)
    expect(body.resourceDegradation.some((res: { ownerId: string }) => res.ownerId === 'char-1')).toBe(true)
  })
  ...
  it('advance_day passes through interventionSignals to production_intervene', async () => {
    ...
    const r = await handleProductionManage(db(), {
      action: 'advance_day', worldId: WORLD,
      interventionSignals: { noEncounterIn24h: true, allYieldsStationary: true, moraleStableDays: 5 },
    })
    const body = JSON.parse(r.content[0].text)
  ...
  it('advance_day marks matching scheduled production_calendar events as triggered', async () => {
    await createWorld()
    await handleProductionManage(db(), { action: 'set_schedule', worldId: WORLD, events: [{ day: 1, eventType: 'broadcast_event', eventData: { note: 'special' } }] })
    const r = await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD })
    const body = JSON.parse(r.content[0].text)
  ...
  it('get_state reflects the current clock, active prizes, and upcoming scheduled events', async () => {
    await createWorld()
    await handleProductionManage(db(), { action: 'advance_day', worldId: WORLD })
    await handleProductionManage(db(), { action: 'set_schedule', worldId: WORLD, events: [{ day: 10, eventType: 'broadcast_event' }] })
    const r = await handleProductionManage(db(), { action: 'get_state', worldId: WORLD })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
    expect(body.day).toBe(1)
  ...
  it('set_schedule requires worldId', async () => {
    const r = await handleProductionManage(db(), { action: 'set_schedule', events: [{ day: 1, eventType: 'broadcast_event' }] })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  })
  ...
  it('set_schedule requires a non-empty events array', async () => {
    await createWorld()
    const r = await handleProductionManage(db(), { action: 'set_schedule', worldId: WORLD, events: [] })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  })
  ...
  it('set_schedule inserts and can update an existing (world, day, eventType) row', async () => {
    await createWorld()
    await handleProductionManage(db(), { action: 'set_schedule', worldId: WORLD, events: [{ day: 5, eventType: 'broadcast_event', eventData: { a: 1 } }] })
    const r = await handleProductionManage(db(), { action: 'set_schedule', worldId: WORLD, events: [{ day: 5, eventType: 'broadcast_event', eventData: { a: 2 } }] })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
  ...
  it('list_events filters by fromDay', async () => {
    await createWorld()
    await handleProductionManage(db(), { action: 'set_schedule', worldId: WORLD, events: [{ day: 2, eventType: 'broadcast_event' }, { day: 10, eventType: 'broadcast_event' }] })
    const r = await handleProductionManage(db(), { action: 'list_events', worldId: WORLD, fromDay: 5 })
    const body = JSON.parse(r.content[0].text)
