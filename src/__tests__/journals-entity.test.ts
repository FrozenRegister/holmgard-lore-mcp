...
describe('Journals entity reads', () => {
  ...
  async function seedJournal(overrides: Partial<{
    id: string; name: string; entry: string; date_year: number; date_month: number; date_day: number
    calendar_id: string | null; is_private: number
  }> = {}): Promise<string> {
    ...
    await env.RPG_DB.prepare(
      ...
      id, overrides.name ?? 'Session 1', overrides.entry ?? 'The party arrived at the tavern.',
      overrides.date_year ?? 2184, overrides.date_month ?? 7, overrides.date_day ?? 1,
      overrides.calendar_id ?? null, overrides.is_private ?? 0, now, now
    ).run()
    return id
  }
  ...
  async function addParticipant(journalId: string, entityType: string, entityId: string) {
    ...
    ).bind(crypto.randomUUID(), journalId, entityType, entityId, new Date().toISOString()).run()
  }
  ...
  innerDescribe('GET /api/entities/journals/:id/participants', () => {
    it('returns an empty list when a journal has no participants', async () => {
      ...
      const res = await SELF.fetch(`http://example.com/api/entities/journals/${journalId}/participants`)
      const body = await res.json() as Record<string, any>
    ...
    it('resolves character, location, quest, and nation participant names', async () => {
      ...
      await env.RPG_DB.prepare(
        `INSERT INTO room_nodes (id, name, base_description, biome_context, atmospherics, exits, entity_ids, created_at, updated_at, visited_count)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(roomId, 'The Rusty Tankard', 'A dim, smoky tavern.', 'dungeon', '[]', '[]', '[]', now, now, 0).run()

      const world = await callTool('rpg', { sub: 'world', action: 'create', name: `World ${crypto.randomUUID()}`, theme: 'fantasy' })
      const quest = await callTool('rpg', { sub: 'quest', action: 'create', worldId: world.worldId, name: 'Find the Amulet', description: 'A quest.' })

      const nationId = crypto.randomUUID()
      await env.RPG_DB.prepare(
        `INSERT INTO nations (id, world_id, name, leader, ideology, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`
      ).bind(nationId, world.worldId, 'The Sunlit Kingdom', 'Queen Elowen', 'monarchy', now, now).run()
      ...
      const byType = Object.fromEntries(body.participants.map((p: any) => [p.entity_type, p.entity_name]))
