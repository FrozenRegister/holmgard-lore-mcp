...
// The entry-parsing regex was: /^(.+?)\s*:\s*(\d+)$/  
// \d+ only matched integers, so decimal weights (0.03, 0.55, etc.) failed.  
// Fix: change (\d+) to ([\d.]+) in the regex inside the roll_encounter handler.
