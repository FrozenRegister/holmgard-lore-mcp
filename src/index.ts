...
const SUB_SCHEMAS: SubSchemaEntry[] = [
  ...
  { sub: 'combat_action', description: 'In-combat action log. Actions: attack (roll to hit + damage against one or more targets), apply_damage, heal, apply_condition, remove_condition (toggle a condition tag on the target character), use_ability, get_log (encounter action history), get_turn_summary, dash, dodge, disengage, help, ready. Aliases: hit/strike/swing->attack, damage/hurt/wound->apply_damage, restore/cure/recover->heal, condition/add_condition/afflict->apply_condition, cure_condition/end_condition->remove_condition, ability/special/skill->use_ability, log/history->get_log, summary/turn_summary->get_turn_summary, sprint/move_action->dash, evade->dodge, retreat/withdraw->disengage, assist->help, prepare/hold_action/delay->ready.',
    schema: { type: 'object', properties: { action: { type: 'string' }, encounterId: { type: 'string', description: 'Required to log the action; several actions no-op the log without it.' }, actorId: { type: 'string' }, actorName: { type: 'string' }, targetIds: { type: 'array', items: { type: 'string' }, description: 'Required for attack.' }, round: { type: 'integer', minimum: 1, description: 'default 1' }, turnIndex: { type: 'integer', minimum: 0, description: 'default 0' }, attackRoll: { type: 'integer' }, damage: { type: 'integer', minimum: 0 }, damageExpression: { type: 'string', description: 'Dice notation, e.g. "2d6+3"' }, damageType: { type: 'string' }, healAmount: { type: 'integer', minimum: 0 }, conditionName: { type: 'string', description: 'apply_condition/remove_condition' }, abilityName: { type: 'string', description: 'use_ability' }, description: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 100, description: 'get_log, default 20' } }, required: ['action'] } },
  ...
  { sub: 'character', description: 'Character CRUD and management. Actions: create, get, list, update, delete, search, find_by_name, add_xp, get_progression, level_up, cast_spell, snapshot, activate, list_passengers, recompute_derived, kill, move_to_location (#313 — narrative location_key), move_to_tile (#313 — hex-axial q/r/mapId; independent of move_to_location, a character can be dual-mode).',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string' }, characterId: { type: 'string' }, name: { type: 'string' }, worldId: { type: 'string' }, characterClass: { type: 'string' }, race: { type: 'string' }, level: { type: 'number' }, hp: { type: 'number' }, maxHp: { type: 'number' }, ac: { type: 'number' }, stats: { type: 'object' }, query: { type: 'string', description: 'Search query for search action' }, xp: { type: 'number' }, spellName: { type: 'string' }, slotLevel: { type: 'number' }, limit: { type: 'number' }, killerId: { type: 'string' }, causeOfDeath: { type: 'string' }, locationKey: { type: 'string', description: 'Required for move_to_location.' }, q: { type: 'integer', description: 'Required for move_to_tile — hex-axial column.' }, r: { type: 'integer', description: 'Required for move_to_tile — hex-axial row.' }, mapId: { type: 'string', description: 'move_to_tile, default "main".' }, fields: { type: 'object', additionalProperties: true, description: '#425 — arbitrary D1 column passthrough for `update`, e.g. columns with no dedicated param yet. Blacklist: id, created_at, updated_at, world_id.' } }, required: ['action'] } },
  ...
  { sub: 'math', description: 'Dice rolling, probability, and projectile physics. Actions: roll, probability, projectile, get_history, solve, simplify (solve/simplify are algebra stubs — no nerdamer/CAS dependency in Workers — they return success: false with an explanatory message; use roll for dice math).',
  ...
  { sub: 'party', description: 'Party management — creation, membership, trust, morale, march, cohesion. Actions: create, get, list, update, delete, add_member, remove_member, set_leader, trust_shift, resolve_conflict, betrayal_check, morale_roll, watch_rotation, begin_march, get_march_status, cohesion_check (d20 roll + modifiers vs. cohesion_score), group_break (dissolve the party — abandonment/betrayal/death/mutual), cohesion_shift (apply a named cohesion event).',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string' }, partyId: { type: 'string' }, name: { type: 'string' }, worldId: { type: 'string' }, characterId: { type: 'string' }, role: { type: 'string', enum: ['leader', 'member', 'companion', 'hireling', 'prisoner', 'mount'] }, fromCharacterId: { type: 'string' }, towardCharacterId: { type: 'string' }, status: { type: 'string', enum: ['active', 'dormant', 'archived'] }, stressModifier: { type: 'number', description: 'cohesion_check' }, cooperationModifier: { type: 'number', description: 'cohesion_check' }, reason: { type: 'string', description: 'group_break' }, method: { type: 'string', enum: ['abandonment', 'betrayal', 'death', 'mutual'], description: 'Required for group_break.' }, eventType: { type: 'string', description: 'Required for cohesion_shift and (as an alternative to delta) trust_shift — a key into the handler\'s cohesion/trust event tables.' }, fields: { type: 'object', additionalProperties: true, description: '#425 — arbitrary D1 column passthrough for `update` (e.g. formation, current_location, current_quest_id, current_poi, last_played_at). Blacklist: id, created_at, updated_at, world_id.' } }, required: ['action'] } },
  ...
  { sub: 'theft', description: 'Stolen-item ledger — record thefts, track heat level, fence or recover items. Actions: steal (record a theft, heat starts "burning"), fence (mark sold/fenced), recover, cool_heat (step heat down one level toward "cold"), report (mark reported to guards), get, list (filter by thief/heat/recovered). Aliases: pick_pocket/pickpocket/theft->steal, sell_stolen/fence_item->fence, retrieve/find->get, all/search->list, restore/returned->recover, reduce_heat/heat_down->cool_heat, guards/report_theft->report.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string', description: 'Stolen-item record UUID from steal. Required for fence/get/recover/cool_heat/report.' }, itemId: { type: 'string', description: 'Required for steal.' }, stolenFrom: { type: 'string', description: 'Required for steal.' }, stolenBy: { type: 'string', description: 'Required for steal.' }, stolenLocation: { type: 'string' }, witnesses: { type: 'array', items: { type: 'string' } }, bounty: { type: 'integer', minimum: 0, description: 'default 0' }, fencedTo: { type: 'string', description: 'fence' }, filter: { type: 'object', properties: { thief: { type: 'string' }, heat: { type: 'string', enum: ['burning', 'hot', 'warm', 'cool', 'cold'] }, recovered: { type: 'boolean' } }, description: 'list' }, limit: { type: 'integer', minimum: 1, maximum: 200, description: 'list, default 50' } }, required: ['action'] } },
  ...
  { sub: 'improvisation', description: 'Custom-effect ledger — boons, curses, and other ad-hoc buffs/debuffs a DM applies mid-session, with duration tracking and per-round ticking. Actions: apply (create an effect on a target), get, list (all active), remove, tick (advance rounds-remaining and expire), list_by_target. Aliases: create/add_effect/improvise->apply, fetch/find->get, all/active->list, delete/end/dispel->remove, advance/round/next_round->tick, for/on/affecting->list_by_target.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'integer', description: 'Effect ID from apply. Required for get/remove/tick.' }, targetId: { type: 'string', description: 'Required for apply. Character/NPC UUID.' }, targetType: { type: 'string', enum: ['character', 'npc'], description: 'default character' }, name: { type: 'string', description: 'Required for apply.' }, description: { type: 'string' }, sourceType: { type: 'string', enum: ['divine', 'arcane', 'natural', 'cursed', 'psionic', 'unknown'], description: 'default unknown' }, sourceEntityId: { type: 'string' }, sourceEntityName: { type: 'string' }, category: { type: 'string', enum: ['boon', 'curse', 'neutral', 'transformative'], description: 'default neutral' }, powerLevel: { type: 'integer', minimum: 1, maximum: 5, description: 'default 1' }, mechanics: { type: 'array', items: { type: 'string' } }, durationType: { type: 'string', enum: ['rounds', 'minutes', 'hours', 'days', 'permanent', 'until_removed'], description: 'default rounds' }, durationValue: { type: 'integer' }, triggers: { type: 'array', items: { type: 'string' } }, removalConditions: { type: 'array', items: { type: 'string' } }, stackable: { type: 'boolean', description: 'default false' }, rounds: { type: 'integer', minimum: 1, description: 'tick — how many rounds to advance, default 1' } }, required: ['action'] } },
  ...
  { sub: 'npc', description: 'NPC creation, relationships, memories, and location. Actions: create, get, list, update, get_full_context (character + relationships + recent memories), get_relationship, update_relationship (familiarity/disposition/notes), record_memory, get_history (conversation memories between a character and NPC), get_recent (a character\'s recent NPC interactions), get_context (lightweight NPC + recent memories), interact (records/bumps a relationship, optionally logs a memory), assign_to_location (locationKey or hexQ/hexR). Aliases: new_npc/spawn_npc->create, context->get_context, full->get_full_context, relationship/relation->get_relationship, update_rel/set_relationship->update_relationship, memory/remember->record_memory, history/conversations->get_history, recent/recent_interactions->get_recent, talk/speak->interact, all_npcs/browse_npcs->list, place/relocate/move->assign_to_location.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string', description: 'NPC characterId — used by get_full_context/get_context.' }, worldId: { type: 'string' }, name: { type: 'string', description: 'Required for create.' }, class: { type: 'string', description: 'create, default Commoner' }, race: { type: 'string', description: 'create, default Human' }, background: { type: 'string', description: 'create, default Folk Hero' }, alignment: { type: 'string' }, stats: { type: 'object', description: '{ str, dex, con, int, wis, cha }, each default 10' }, hp: { type: 'integer' }, maxHp: { type: 'integer' }, ac: { type: 'integer', description: 'default 10' }, level: { type: 'integer', minimum: 1, maximum: 20, description: 'default 1' }, factionId: { type: 'string' }, characterId: { type: 'string', description: 'The PC/observer side of a relationship — required for get_relationship/update_relationship/record_memory/get_history/get_recent/interact.' }, npcId: { type: 'string', description: 'Alternate key for id on get/update/assign_to_location; required alongside characterId for get_relationship/update_relationship/record_memory/get_history/interact.' }, familiarity: { type: 'string', enum: ['stranger', 'acquaintance', 'friend', 'close_friend', 'rival', 'enemy'], description: 'update_relationship, default stranger' }, disposition: { type: 'string', enum: ['hostile', 'unfriendly', 'neutral', 'friendly', 'helpful'], description: 'update_relationship, default neutral' }, notes: { type: 'string', description: 'update_relationship' }, summary: { type: 'string', description: 'Required for record_memory.' }, importance: { type: 'string', enum: ['low', 'medium', 'high', 'critical'], description: 'record_memory, default medium' }, topics: { type: 'array', items: { type: 'string' }, description: 'record_memory' }, context: { type: 'string', description: 'interact — optional memory text to log alongside the interaction.' }, limit: { type: 'integer', minimum: 1, maximum: 50, description: 'list/get_history/get_recent, default 10' }, locationKey: { type: 'string', description: 'assign_to_location (one of locationKey or hexQ/hexR required).' }, hexQ: { type: 'integer', description: 'assign_to_location' }, hexR: { type: 'integer', description: 'assign_to_location' } }, required: ['action'] } },
  ...
  { sub: 'session', description: 'Session bootstrap and narrative context. Actions: initialize (find an existing world/party to play in, or make one with createNew), get_context (aggregated party/quests/world/narrative/combat snapshot for an AI narrator). Aliases: init/start/setup/initialize_session/start_session->initialize, context/narrative/narrative_context/get_narrative/summary->get_context.',
    schema: { type: 'object', properties: { action: { type: 'string' }, worldId: { type: 'string' }, partyId: { type: 'string' }, createNew: { type: 'boolean', description: 'initialize, default false — create a new world/party instead of reusing the most recent existing one' }, worldName: { type: 'string' }, partyName: { type: 'string' }, includeParty: { type: 'boolean', description: 'get_context, default true' }, includeQuests: { type: 'boolean', description: 'get_context, default true' }, includeWorld: { type: 'boolean', description: 'get_context, default true' }, includeNarrative: { type: 'boolean', description: 'get_context, default true' }, includeCombat: { type: 'boolean', description: 'get_context, default true' }, narrativeLimit: { type: 'integer', minimum: 1, maximum: 50, description: 'get_context, default 10' } }, required: ['action'] } },
  // Discovered while wiring up the #468 drift guard — corrected to match
  ...
  { sub: 'combat_map', description: 'Square-grid tactical battlefield — terrain, tokens, ASCII rendering, AoE templates. Actions: create, get (by id or encounterId), update (terrain/dimensions), move_token, render (ASCII grid), delete, get_terrain, set_terrain, calculate_aoe (circle/square/line template from an origin). Aliases: new_map/setup_map/init_battlefield->create, fetch/show/load->get, edit/modify/patch->update, move/reposition->move_token, display/ascii/view->render, remove/destroy/clear->delete, terrain/read_terrain->get_terrain, update_terrain/paint_terrain->set_terrain, aoe/area_of_effect/blast_radius->calculate_aoe.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string', description: 'Battlefield UUID from create. Required for update/move_token/delete/set_terrain.' }, encounterId: { type: 'string', description: 'Required for create; fallback lookup key for get/render/get_terrain when id is omitted.' }, width: { type: 'integer', minimum: 1, maximum: 50, description: 'create/update, default 10' }, height: { type: 'integer', minimum: 1, maximum: 50, description: 'create/update, default 10' }, terrain: { type: 'array', items: { type: 'object', properties: { x: { type: 'integer' }, y: { type: 'integer' }, type: { type: 'string' } } }, description: 'update/set_terrain' }, tokenId: { type: 'string', description: 'Required for move_token.' }, x: { type: 'integer', description: 'move_token' }, y: { type: 'integer', description: 'move_token' }, origin: { type: 'object', properties: { x: { type: 'integer' }, y: { type: 'integer' } }, description: 'Required for calculate_aoe.' }, target: { type: 'object', properties: { x: { type: 'integer' }, y: { type: 'integer' } }, description: 'calculate_aoe — required when shape is line.' }, shape: { type: 'string', enum: ['circle', 'square', 'line'], description: 'calculate_aoe, default circle' }, size: { type: 'integer', minimum: 1, maximum: 30, description: 'calculate_aoe, default 1' } }, required: ['action'] } },
  ...
  { sub: 'spawn', description: 'Entity spawning — characters, encounters, locations, and placing an existing character on the hex map. Actions: spawn_character, spawn_encounter, spawn_location, add_to_encounter (place a token on a tactical encounter grid), list_spawned, place_character (position an existing character on a world hex map — #340; requires the character to already exist, unlike spawn_character). Aliases: character/spawn_npc/create_character->spawn_character, encounter/new_encounter/setup_encounter->spawn_encounter, location/populate->spawn_location, add/join/insert->add_to_encounter, list/show_all->list_spawned, place/place_npc->place_character.',
    schema: { type: 'object', properties: { action: { type: 'string' }, name: { type: 'string' }, characterType: { type: 'string', enum: ['pc', 'npc', 'enemy', 'neutral'], description: 'default enemy' }, characterClass: { type: 'string', description: 'default Fighter' }, race: { type: 'string', description: 'default Human' }, level: { type: 'integer', minimum: 1, maximum: 20, description: 'default 1' }, hp: { type: 'integer' }, maxHp: { type: 'integer' }, ac: { type: 'integer', description: 'default 10' }, stats: { type: 'object', description: '{ str, dex, con, int, wis, cha }, each default 10' }, encounterId: { type: 'string', description: 'Required for add_to_encounter.' }, regionId: { type: 'string' }, count: { type: 'integer', minimum: 1, maximum: 20, description: 'default 1' }, initiative: { type: 'number', description: 'add_to_encounter' }, position: { type: 'object', properties: { x: { type: 'integer' }, y: { type: 'integer' } }, description: 'add_to_encounter tactical-grid position' }, characterId: { type: 'string', description: 'Required for add_to_encounter and place_character.' }, worldId: { type: 'string' }, q: { type: 'integer', description: 'Required for place_character — hex-axial column.' }, r: { type: 'integer', description: 'Required for place_character — hex-axial row.' }, mapId: { type: 'string', description: 'place_character, default "main"' }, limit: { type: 'integer', minimum: 1, maximum: 100, description: 'list_spawned, default 20' } }, required: ['action'] } },
  ...
  { sub: 'turn', description: 'World-level turn phase tracking — nations/parties submit actions and mark ready each turn. Actions: init (start turn 1 for a world), get_status, submit_actions (log an action batch for a nation/party this turn), mark_ready, poll_results (status + recently submitted actions). Aliases: initialize/setup/start/create->init, status/state/check->get_status, submit/action/post_actions->submit_actions, ready/confirm/done->mark_ready, poll/results/get_results->poll_results.',
    schema: { type: 'object', properties: { action: { type: 'string' }, worldId: { type: 'string', description: 'Required for all actions.' }, nationId: { type: 'string', description: 'submit_actions/mark_ready — one of nationId or partyId required.' }, partyId: { type: 'string', description: 'submit_actions/mark_ready — one of nationId or partyId required.' }, actions: { type: 'array', items: { type: 'object', properties: { type: { type: 'string' }, targetId: { type: 'string' }, description: { type: 'string' } }, required: ['type'] }, description: 'submit_actions' } }, required: ['action'] } },
  // Discovered while wiring up the #468 drift guard — corrected to match
  ...
  // world_map instead).
  { sub: 'spatial', description: 'Room-graph (room_nodes) locations — look/generate/update rooms, exits, room-to-room movement, and node networks. Actions: look (room detail, increments visit count), generate (create a room; validates biome against the per-world registry when worldId is set), update, get_exits, move (follow an exit by direction), list, network_create, network_get, network_list. Aliases: describe/observe/inspect->look, create/new_room/spawn->generate, edit/modify/patch->update, exits/doors->get_exits, go/travel/walk->move, rooms/all_rooms->list, create_network/new_network->network_create, get_network/fetch_network->network_get, list_networks/networks->network_list.',
    schema: { type: 'object', properties: { action: { type: 'string' }, roomId: { type: 'string', description: 'Required for look/update/get_exits/move.' }, name: { type: 'string', description: 'Required for generate/network_create.' }, description: { type: 'string', description: 'generate/update — ignored if under 10 chars.' }, biome: { type: 'string', description: 'generate/update, default dungeon — validated against the world biome registry when worldId is set and the registry is non-empty.' }, atmosphere: { type: 'array', items: { type: 'string' } }, exits: { type: 'array', items: { type: 'object', properties: { direction: { type: 'string' }, targetRoomId: { type: 'string' }, label: { type: 'string' } }, required: ['direction', 'targetRoomId'] } }, entityIds: { type: 'array', items: { type: 'string' } }, direction: { type: 'string', description: 'Required for move.' }, networkId: { type: 'string', description: 'Required for network_get.' }, worldId: { type: 'string', description: 'generate/update/network_create — also gates biome validation.' }, networkType: { type: 'string', enum: ['cluster', 'linear'], description: 'network_create, default cluster' }, limit: { type: 'integer', minimum: 1, maximum: 100, description: 'list/network_list, default 20' }, worldIdFilter: { type: 'string', description: 'list' } }, required: ['action'] } },
  ...
  { sub: 'batch', description: 'Bulk character creation, item distribution, and starter templates. Actions: batch_create_characters, batch_create_npcs (same handler, different default characterType), batch_distribute_items, execute_workflow (records a sequence of tool-call steps — does not execute them; individual tool calls must still be made separately), list_templates, get_template (by templateId or a name substring match). Aliases: bulk_characters/create_characters/many_characters->batch_create_characters, bulk_npcs/create_npcs/many_npcs->batch_create_npcs, distribute/give_items/bulk_items->batch_distribute_items, workflow/run_workflow->execute_workflow, templates/all_templates->list_templates, template/fetch_template->get_template.',
    schema: { type: 'object', properties: {
      action: { type: 'string' },
      characters: { type: 'array', description: 'Required (non-empty) for batch_create_characters/batch_create_npcs. Each item: { name (required), level, characterClass, race, characterType, stats, hp, maxHp, ac }.', items: { type: 'object', properties: { name: { type: 'string' } }, required: ['name'] } },
      distributions: { type: 'array', description: 'Required (non-empty) for batch_distribute_items. Each item: { characterId, itemId, quantity }.', items: { type: 'object', properties: { characterId: { type: 'string' }, itemId: { type: 'string' }, quantity: { type: 'integer', minimum: 1 } }, required: ['characterId', 'itemId'] } },
      steps: { type: 'array', description: 'Required (non-empty) for execute_workflow. Each item: { tool, args }. Recorded only — individual tool calls must still be made separately via tools/call.', items: { type: 'object', properties: { tool: { type: 'string' }, args: { type: 'object', additionalProperties: true } }, required: ['tool', 'args'] } },
      templateId: { type: 'string', description: 'get_template — one of: party-4, dungeon-guards, merchant-caravan, bandit-gang.' },
      templateName: { type: 'string', description: 'get_template, substring match on template name (used if templateId is omitted).' },
      category: { type: 'string', description: 'list_templates filter — party | encounter | npc.' },
    }, required: ['action'] } },
  ...
  { sub: 'perception', description: 'Perception assessments and stealth/opposed checks. Actions: assess (roll vs. DC, persists an assessment record), get_history, get_latest, list_observers (who has assessed a target), stealth_check (#284 — opposed yield-vs-predator stealth roll, same math encounter.resolve/check reuse), perception_contested (#284 — opposed observer-vs-actor roll). Aliases: check/perceive/observe/inspect/roll->assess, history/past->get_history, latest/current/last->get_latest, observers/watchers->list_observers, stealth/sneak/hide_check->stealth_check, contested/opposed_check/spot_check->perception_contested.',
    schema: { type: 'object', properties: { action: { type: 'string' }, observerId: { type: 'string', description: 'Required for assess/get_history/get_latest.' }, targetId: { type: 'string', description: 'Required for assess/list_observers.' }, targetKind: { type: 'string', enum: ['room', 'encounter', 'scene'], description: 'assess, default room' }, rollValue: { type: 'integer', minimum: 1, maximum: 30, description: 'Override the d20 roll (testing) — assess/stealth_check/perception_contested.' }, dc: { type: 'integer', minimum: 1, maximum: 30, description: 'assess, default 12' }, perceptionType: { type: 'string', enum: ['sight', 'hearing', 'smell', 'arcana', 'investigation', 'insight'], description: 'assess, default sight' }, limit: { type: 'integer', minimum: 1, maximum: 50, description: 'get_history/list_observers, default 20' }, stealthMode: { type: 'string', enum: ['active', 'passive', 'rushed', 'hiding'], description: 'stealth_check, default active' }, coverType: { type: 'string', description: 'stealth_check' }, windDirection: { type: 'string', enum: ['toward', 'away', 'crosswind', 'none'], description: 'stealth_check, default none' }, distanceZone: { type: 'string', enum: ['core', 'edge', 'unknown'], description: 'stealth_check, default unknown' }, yieldBleeding: { type: 'boolean', description: 'stealth_check' }, yieldCookingOrFire: { type: 'boolean', description: 'stealth_check' }, isNight: { type: 'boolean', description: 'stealth_check' }, partySize: { type: 'integer', minimum: 1, description: 'stealth_check, default 1' }, yieldStealthBonus: { type: 'number', description: 'stealth_check' }, predatorPerceptionBonus: { type: 'number', description: 'stealth_check' }, observerModifier: { type: 'number', description: 'perception_contested' }, actorModifier: { type: 'number', description: 'perception_contested' } }, required: ['action'] } },
  ...
  { sub: 'scene', description: 'Scene management — narrative scenes with participants. Actions: create, get, list, update, delete, get_latest, state_snapshot, set_conflict_type, get_conflict_type (#316 — link/read a scene\'s conflict-type routing).',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string' }, worldId: { type: 'string' }, title: { type: 'string' }, whenLabel: { type: 'string' }, placeLabel: { type: 'string' }, narration: { type: 'string' }, participants: { type: 'array', items: { type: 'string' } }, previousSceneId: { type: 'string' }, limit: { type: 'number' }, sceneId: { type: 'string' }, include: { type: 'object' }, conflictTypeId: { type: ['string', 'null'], description: 'set_conflict_type (required, pass null to clear) / returned by get_conflict_type' } }, required: ['action'] } },
  ...
  { sub: 'event', description: 'Poll-based event queue — Cloudflare Workers has no long-lived connection to push events over, so this is emit/poll/ack instead of pub/sub. Actions: emit (publish an event), poll (fetch, defaults to unconsumed-only), ack (mark consumed by id or ids), list_types (known eventType/sourceType values — custom eventType strings are also accepted). Aliases: publish/send/notify->emit, subscribe/list/get_events/unsubscribe->poll, consume/mark_read/dismiss->ack, types/topics->list_types.',
    schema: { type: 'object', properties: { action: { type: 'string' }, eventType: { type: 'string', description: 'Required for emit.' }, payload: { type: 'object', additionalProperties: true, description: 'Required for emit.' }, sourceType: { type: 'string', enum: ['npc', 'combat', 'world', 'system', 'scheduler'] }, sourceId: { type: 'string' }, priority: { type: 'integer', description: 'emit, default 0' }, id: { type: 'integer', description: 'ack, single event ID' }, ids: { type: 'array', items: { type: 'integer' }, description: 'ack, multiple event IDs' }, unconsumedOnly: { type: 'boolean', description: 'poll, default true' }, limit: { type: 'integer', minimum: 1, maximum: 200, description: 'poll, default 50' } }, required: ['action'] } },
  ...
  { sub: 'drama', description: 'Ability checks and narrative conflict resolution — opposed checks, group checks, social combat, multi-side dramatic conflicts. Actions: roll_ability (single check, optional advantage/disadvantage), opposed_check (character_a vs character_b), group_check (side_a vs side_b, mode: best|sum|pool), social_combat (multi-round negotiation among participants), dramatic_conflict (multi-side campaign-level conflict over ticks). Aliases: ability/roll->roll_ability, oppose/duel->opposed_check, group/pool->group_check, social/negotiate->social_combat, conflict/campaign->dramatic_conflict.',
    schema: { type: 'object', properties: {
      ...
      side_a: { type: 'array', items: { type: 'object', properties: { character: { type: 'string' }, ability: { type: 'string' } } }, description: 'group_check' },
      side_b: { type: 'array', items: { type: 'object', properties: { character: { type: 'string' }, ability: { type: 'string' } } }, description: 'group_check' },
      mode: { type: 'string', enum: ['best', 'sum', 'pool'], description: 'group_check, default best' },
      participants: { type: 'array', items: { type: 'object', properties: { character: { type: 'string' }, goal: { type: 'string' }, leverage: { type: 'number' } } }, description: 'social_combat' },
      rounds: { type: 'integer', minimum: 1, maximum: 20, description: 'social_combat, default 3' },
      ...
      sides: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, actors: { type: 'array', items: { type: 'string' } }, primary_ability: { type: 'string' }, momentum: { type: 'number' } } }, description: 'dramatic_conflict' },
      ticks: { type: 'integer', minimum: 1, maximum: 20, description: 'dramatic_conflict, default 4' },
      external_factors: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, modifier: { type: 'number' }, affects: { type: 'string' } } }, description: 'dramatic_conflict' },
    }, required: ['action'] } },
  ...
  { sub: 'timeline', description: 'D1-backed narrative timeline — events, branches, perspectives, gap analysis. Actions: get_events (filter by thread/entity/verb/date range/branch), get_gap (canonical events + present characters between two events), get_perspectives (characters present in a date range), create_branch, switch_branch (set the active branch), compare_branches, merge_branch (move events from one branch to another). Aliases: events/list->get_events, gap/between->get_gap, characters/pov->get_perspectives, branch/fork->create_branch, switch->switch_branch, compare/diff->compare_branches, merge->merge_branch. Accepts both "worldId" and snake_case "world_id".',
    schema: { type: 'object', properties: { action: { type: 'string' }, world_id: { type: 'string' }, worldId: { type: 'string', description: 'Alias for world_id' }, thread: { type: 'string' }, from: { type: 'string', description: 'get_events: ISO date lower bound. get_perspectives: required.' }, to: { type: 'string', description: 'get_events: ISO date upper bound. get_perspectives: required.' }, entity_id: { type: 'string' }, verb: { type: 'string' }, canonical_only: { type: 'boolean' }, before_event_id: { type: 'string', description: 'Required for get_gap.' }, after_event_id: { type: 'string', description: 'Required for get_gap.' }, name: { type: 'string', description: 'Required for create_branch.' }, forked_at_event_id: { type: 'string', description: 'Required for create_branch — pivot event.' }, reason: { type: 'string' }, branch_id: { type: 'string', description: 'Required for switch_branch.' }, branch_a: { type: 'string', description: 'Required for compare_branches.' }, branch_b: { type: 'string', description: 'Required for compare_branches.' }, source_branch_id: { type: 'string', description: 'Required for merge_branch.' }, target_branch_id: { type: 'string', description: 'Required for merge_branch.' }, event_ids: { type: 'array', items: { type: 'string' }, description: 'Required (non-empty) for merge_branch.' }, limit: { type: 'integer', minimum: 1, maximum: 500 } }, required: ['action'] } },
  ...
  { sub: 'encounter', description: 'Encounter resolution engine (#280) — resolves a 1d100 threat check (biome + zone threat + contextual modifiers) at a hex, optionally with a stealth/perception opposed check first, and can assign injuries. Actions: resolve (full roll, selects an encounter type and persists injuries), check (lightweight — just the trigger roll, no type selection/injury persistence), list_types, add_type (register a predator/encounter type for a world), check_infection (infection staging for a prior injury). Aliases: encounter/trigger/roll_encounter->resolve, check_encounter/peek/threat_check->check, types/list_encounter_types->list_types, register_type/new_type/create_type->add_type, infection/infection_check->check_infection.',
    schema: { type: 'object', properties: { action: { type: 'string' }, worldId: { type: 'string', description: 'Required for resolve/check/list_types/add_type.' }, q: { type: 'integer', description: 'Required for resolve/check.' }, r: { type: 'integer', description: 'Required for resolve/check.' }, partySize: { type: 'integer', minimum: 1, description: 'default 1' }, timeOfDay: { type: 'string', enum: ['dawn', 'dusk', 'night', 'midday', 'day'] }, noiseLevel: { type: 'string', enum: ['loud', 'moderate', 'silent'] }, scentModifiers: { type: 'array', items: { type: 'string', enum: ['blood', 'cooking', 'fire'] } }, partyInjuries: { type: 'array', items: { type: 'string' } }, weather: { type: 'string', enum: ['clear', 'rain', 'snow', 'fog'] }, includeInjuries: { type: 'boolean', description: 'resolve, default true' }, characterIds: { type: 'array', items: { type: 'string' } }, stealthCheck: { type: 'boolean', description: '#284 — opposed stealth/perception check before the threat roll, default false' }, stealthMode: { type: 'string', enum: ['active', 'passive', 'rushed', 'hiding'], description: 'default active' }, coverType: { type: 'string' }, windDirection: { type: 'string', enum: ['toward', 'away', 'crosswind', 'none'], description: 'default none' }, distanceZone: { type: 'string', enum: ['core', 'edge', 'unknown'], description: 'default unknown' }, yieldBleeding: { type: 'boolean' }, yieldCookingOrFire: { type: 'boolean' }, isNight: { type: 'boolean' }, yieldStealthBonus: { type: 'number' }, predatorPerceptionBonus: { type: 'number' }, yieldStealthRoll: { type: 'integer', minimum: 1, maximum: 20, description: 'Override the yield-side stealth d20 (testing).' }, predatorName: { type: 'string', description: 'add_type' }, category: { type: 'string', enum: ['predator', 'environmental', 'system', 'passive'], description: 'add_type (required)' }, aggression: { type: 'string', enum: ['curious', 'hunting', 'territorial', 'starving', 'fleeing'], description: 'add_type, default curious' }, baseWeight: { type: 'number', minimum: 0, description: 'add_type, default 1.0' }, minThreat: { type: 'number', minimum: 0, maximum: 100, description: 'add_type, default 0' }, requiresCore: { type: 'boolean', description: 'add_type — only selectable when the hex falls within this predator\'s own zone' }, description: { type: 'string', description: 'add_type' }, categoryFilter: { type: 'string', enum: ['predator', 'environmental', 'system', 'passive'], description: 'list_types' }, characterId: { type: 'string', description: 'check_infection' }, injuryId: { type: 'string', description: 'Required for check_infection.' }, hoursSinceInjury: { type: 'number', minimum: 0, description: 'check_infection, default 0' }, treatmentReceived: { type: 'string', enum: ['none', 'basic', 'professional'], description: 'check_infection, default none' } }, required: ['action'] } },
  ...
  { sub: 'conflict_type', description: 'Conflict type taxonomy (#316) — physical/social/hybrid scene routing for dual-agent (combat/drama) resolution. Actions: list, create, update, delete. Aliases: register/add/new/make/insert->create, remove/destroy/erase/drop->delete, all/query/browse->list, modify/edit/patch/change/set->update.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string', description: 'Required for update/delete. Slugified from name on create.' }, name: { type: 'string', description: 'Required for create.' }, description: { type: 'string' }, resolver: { type: 'string', enum: ['combat', 'drama', 'both'], description: 'Required for create.' } }, required: ['action'] } },
]
