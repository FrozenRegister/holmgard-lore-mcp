...
app.post('/mcp', async (c) => {
  ...
  try {
    ...
    if (method === 'tools/call') {
      ...
      // lookup). Response shape (JSON-RPC envelope) stays transport-local — see issue #546
      // judgment call (3).
      ...
      if (dispatchResult.kind === 'not-found') {
        return c.json(
          makeError(id, -32601, `Method not found: tool "${dispatchResult.toolName}"`),
          200,
        )
      }
      ...
      return dispatchResult.handler({ c, id, args, isAuthenticated })
    }
