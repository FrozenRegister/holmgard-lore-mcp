...
const SUB_SCHEMAS: Array<{ sub: string; description: string; schema: Record<string, unknown> }> = [
  ...
  { sub: 'world', description: 'World generation and management. Actions: create, get, list, delete, update, generate, get_state. "id" and "worldId" are interchangeable aliases for the world UUID. Accepts snake_case "world_id" as alias for "worldId" (cross-tool compatibility).',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string', description: 'World UUID (alias for worldId)' }, worldId: { type: 'string', description: 'World UUID (alias for id)' }, world_id: { type: 'string', description: 'Snake_case alias for worldId (cross-tool compatibility with non-RPG tools)' }, name: { type: 'string' }, seed: { type: 'string' }, width: { type: 'number' }, height: { type: 'number' }, landRatio: { type: 'number' }, environment: { type: 'object' }, theme: { type: 'string' } }, required: ['action'] } },
  ...
  { sub: 'time', description: 'In-game time and calendar. Actions: get_current, advance, set_time, get_calendar, check_event, get_phase.',
    schema: { type: 'object', properties: { action: { type: 'string' }, worldId: { type: 'string' }, hours: { type: 'number' }, days: { type: 'number' }, targetTime: { type: 'string' } }, required: ['action'] } },
  ...
  { sub: 'biome', description: 'Biome registry and terrain rules. Actions: register, list, get, update, delete, validate, seed_defaults.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string' }, biomeId: { type: 'string' }, worldId: { type: 'string' }, name: { type: 'string' }, terrainType: { type: 'string' }, movementCost: { type: 'number' }, description: { type: 'string' } }, required: ['action'] } },
