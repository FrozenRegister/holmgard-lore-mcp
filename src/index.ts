...
const SUB_SCHEMAS: Array<{ sub: string; description: string; schema: Record<string, unknown> }> = [
  ...
  { sub: 'world', description: 'World generation and management. Actions: create, get, list, delete, update, generate, get_state.',
    schema: { type: 'object', properties: { action: { type: 'string' }, id: { type: 'string' }, worldId: { type: 'string' }, name: { type: 'string' }, seed: { type: 'string' }, width: { type: 'number' }, height: { type: 'number' }, landRatio: { type: 'number' }, environment: { type: 'object' }, theme: { type: 'string' } }, required: ['action'] } },
