...
import {
  ...
  activateSceneSchema,
  handle_present_choices,
  presentChoicesSchema,
  handle_commit_choice,
  commitChoiceSchema,
  handle_get_choice_history,
  getChoiceHistorySchema,
  handle_scene_brief,
  sceneBriefSchema,
  handle_render_pov,
  renderPovSchema,
  handle_render_with_rolls,
  renderWithRollsSchema,
} from './scene'
...
const ACTION_MAP: Record<string, ActionSpec> = {
  ...
  brief: defineAction(sceneBriefSchema, handle_scene_brief, {
    ...
    include: { events: 5, open_setups: true },
  }),
  render_pov: defineAction(renderPovSchema, handle_render_pov, {
    pov_entity_key: 'character:eira-holt',
    location_key: 'location:marsh-end',
  }),
