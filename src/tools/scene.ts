...
export async function handle_activate_scene({
  c,
  id,
  args,
}: TypedToolContext<typeof activateSceneSchema>): Promise<Response> {
  ...
  const entitiesRaw =
    extractRawField(sceneText, 'Entities') ?? extractRawField(sceneText, 'Present-Entities')
  const locationRef = extractRawField(sceneText, 'Location')
  const timelineRef = extractRawField(sceneText, 'Timeline')
  const description =
    extractRawField(sceneText, 'Description') ??
    sceneText
      .split('\n')
  ...
  const inlineChoices = choicesRaw
    ? choicesRaw
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean)
  ...
  if (keysToFetch.length > 0) {
    ...
    keysToFetch.forEach((k, i) => {
      batchData[k] = rawValues[i] ? parseKvEntry(rawValues[i]!).text : null
    })
  }
  ...
  return c.json(
    makeResult(id, {
      content: [
        {
          type: 'text',
          text: `Scene "${sceneKey}" activated. ${entityKeys.length} entity/entities hydrated, ${availableChoices.length} choice branch(es) available.`,
        },
      ],
      ...
      present_entities: entityKeys,
      entity_data: batchData,
...
export async function handle_present_choices({
  c,
  id,
  args,
}: TypedToolContext<typeof presentChoicesSchema>): Promise<Response> {
  const sceneKey = args.scene_key.trim().toLowerCase()
  ...
  const entityInvRaw =
    extractRawField(entityText, 'Inventory') ?? extractRawField(entityText, 'Items') ?? ''
  ...
  for (const line of sceneText.split('\n').filter((l) => /^\s*-\s+\S+:/.test(l))) {
    const m = line.match(
      /^\s*-\s+(\S+):\s*(.+?)(?:\s*\[requires:\s*([^\]]+)\])?(?:\s*\[min-weight:\s*([\d.]+)\])?$/,
    )
    if (!m) continue
    ...
    if (requires && !entityInvRaw.toLowerCase().includes(requires.toLowerCase())) {
      ...
      minWeight !== null &&
      typeof entityWeight1 === 'number' &&
      entityWeight1 < minWeight
    ) {
      blockedReason = `Requires Weight-1 ≥ ${minWeight} (entity has ${entityWeight1})`
    }
    if (blockedReason)
      blockedChoices.push({ id: choiceId, description: desc.trim(), blocked_reason: blockedReason })
  ...
  return c.json(
    makeResult(id, {
      content: [
        {
          type: 'text',
          text: `${validChoices.length} valid choice(s) for "${entityKey}" in "${sceneKey}". ${blockedChoices.length} blocked.`,
        },
      ],
      ...
      blocked_choices: blockedChoices,
      entity_status: entityStatus,
      entity_weight_1: typeof entityWeight1 === 'number' ? entityWeight1 : null,
    }),
    200,
  )
}

export const commitChoiceSchema = z.object({
  choice_id: z.string().min(1),
  entity_key: z.string().min(1),
})

export async function handle_commit_choice({
  c,
  id,
  args,
}: TypedToolContext<typeof commitChoiceSchema>): Promise<Response> {
  ...
  const outcomeSeed =
    extractRawField(choiceText, 'Outcome-Seed') ??
    extractRawField(choiceText, 'Narrative-Seed') ??
    ''
  const stateChange = extractRawField(choiceText, 'State-Change')
  const nextChoicesRaw =
    extractRawField(choiceText, 'Next-Choices') ?? extractRawField(choiceText, 'Unlocks')
  const nextChoices = nextChoicesRaw
    ? nextChoicesRaw
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean)
  ...
  const now = new Date().toISOString()
  let newEntityText = stateChange
    ? updateFieldInText(entityText, 'Status', stateChange)
    : entityText
  const existingHistory = extractRawField(newEntityText, 'Choice-History') ?? ''
  const historyEntry = `${choiceId}@${now}`
  newEntityText = updateFieldInText(
    newEntityText,
    'Choice-History',
    existingHistory ? `${existingHistory}, ${historyEntry}` : historyEntry,
  )
  ...
  await kvPut(
    ...
    JSON.stringify({
      ...
      meta: {
        ...
        updatedAt: now,
        createdAt: entityMeta.createdAt ?? now,
  ...
  try {
    ...
    const characterId = await resolveEntityToCharacterId(
      c.env.RPG_DB,
      entityMeta,
      entityText,
      entityKey,
    ...
    if (characterId) {
      const charRow = (await c.env
        .RPG_DB!.prepare('SELECT world_id FROM characters WHERE id = ?')
        .bind(characterId)
        .first()) as { world_id: string | null } | null
      if (charRow?.world_id) {
  ...
  return c.json(
    makeResult(id, {
      content: [
        {
          type: 'text',
          text: `Choice "${choiceId}" committed for "${entityKey}".${stateChange ? ` State → ${stateChange}.` : ''} ${nextChoices.length} new choice(s) unlocked.`,
        },
      ],
      metadata: { retrieved: 2, written: 1 },
      choice_id: choiceId,
      entity_key: entityKey,
      outcome_seed: outcomeSeed,
...
export async function handle_get_choice_history({
  c,
  id,
  args,
}: TypedToolContext<typeof getChoiceHistorySchema>): Promise<Response> {
  const entityKey = args.entity_key.trim().toLowerCase()
      ...
      content: [
        {
          ...
          text:
            history.length > 0
              ? `Choice history for "${entityKey}": ${history.length} committed choice(s).`
              : `No choice history found for "${entityKey}".`,
        },
      ],
      metadata: { retrieved: 1, written: 0 },
      entity_key: entityKey,
      history,
      raw_history: historyRaw ?? null,
    }),
    200,
  )
}
...
export async function handle_scene_brief({
  c,
  id,
  args,
}: TypedToolContext<typeof sceneBriefSchema>): Promise<Response> {
  ...
  if (!baseKey)
    return c.json(makeError(id, -32602, 'Either location_key or scene_key is required', null), 200)
  ...
  for (const ek of entityKeys) {
    ...
    if (kv && eventsCount > 0) {
      try {
        const evRaw = await kv.get(`events:${ek}`)
        if (evRaw) recentEvents = (JSON.parse(evRaw) as any[]).slice(0, eventsCount)
      } catch {
  ...
  if (includeSetups) {
    ...
    for (let i = 0; i < setupKeys.length; i++) {
      ...
      const actors = (extractRawField(sText, 'Actors') ?? '')
        .split(',')
        .map((s: string) => s.trim())
        .filter(Boolean)
      if (actors.length === 0 || actors.some((a: string) => actorSet.has(a.toLowerCase()))) {
        openSetups.push({
          ...
          tension: (() => {
            const v = extractFieldFromText(sText, 'Tension')
    ...
    for (let i = 0; i < Math.min(entityKeys.length, 4); i++) {
      for (let j = i + 1; j < Math.min(entityKeys.length, 4); j++) {
        ...
        if (affinity || debt || threat) {
          relationships.push({
            entity_a: entityKeys[i],
            entity_b: entityKeys[j],
            affinity,
            debt,
            threat_level: threat,
          })
  ...
  return c.json(
    makeResult(id, {
      content: [
        {
          type: 'text',
          text: `Scene brief for "${baseKey}": ${occupants.length} entity/entities present. ${openSetups.length} open setup(s). ${relationships.length} relationship(s).`,
        },
      ],
      metadata: {
        location: baseKey,
        entity_count: occupants.length,
        setup_count: openSetups.length,
      },
...
async function buildPovRenderData(
  c: HonoCtx,
  args: PovRenderArgs,
): Promise<{ error: string } | { data: PovRenderData }> {
  ...
  const perception = (() => {
    const v = extractFieldFromText(povText, 'Perception')
    return typeof v === 'number' ? normalizeWeight(v) : 0.5
  })()
  ...
  const baseKey = (
    args.location_key ??
    args.scene_key ??
    extractRawField(povText, 'Location') ??
    ''
  )
    .trim()
    .toLowerCase()
  if (!baseKey)
    return { error: 'scene_key or location_key required, or entity must have a Location field' }
  ...
  if (args.include_voice_hints) {
    ...
    const ownFixations =
      extractRawField(povText, 'Fixations') ?? extractRawField(povText, 'Preoccupations')
    ...
    if (!hasOwnVoiceData) {
      const rawSpeciesField =
        (extractRawField(povText, 'Species') ?? extractRawField(povText, 'Type'))
          ?.trim()
          .toLowerCase() ?? null
      speciesKey = rawSpeciesField
    ...
    voiceSource = hasOwnVoiceData
      ? 'entity'
      : speciesText
        ? `species fallback (${speciesKey})`
        : 'none'
  }
...
export async function handle_render_pov({
  c,
  id,
  args,
}: TypedToolContext<typeof renderPovSchema>): Promise<Response> {
  ...
  return c.json(
    makeResult(id, {
      content: [
        {
          type: 'text',
          text: `POV render for "${povKey}" at "${baseKey}": ${visibleEntities.length} visible entity/entities. Perception: ${threshold.toFixed(2)}.`,
        },
      ],
      metadata: {
        pov: povKey,
...
export async function handle_render_with_rolls({
  c,
  id,
  args,
}: TypedToolContext<typeof renderWithRollsSchema>): Promise<Response> {
  const result = await buildPovRenderData(c, args)
  if ('error' in result) return c.json(makeError(id, -32602, result.error, null), 200)
  const {
    povKey,
  ...
  const rolls: Array<
    ...
    | { label: string; expression: string; error: string }
  > = []
  for (const spec of args.rolls) {
    const resp = await handleMathManage(c.env, {
      action: 'roll',
    ...
    if (data.error) {
      rolls.push({
        label: spec.label,
        expression: spec.expression,
        error: String(data.message ?? 'roll failed'),
      })
      continue
    }
    rolls.push({
  ...
  return c.json(
    makeResult(id, {
      content: [
        {
          type: 'text',
          text: `POV render for "${povKey}" at "${baseKey}": ${visibleEntities.length} visible entity/entities, ${rolls.length} roll(s) resolved.`,
        },
      ],
      metadata: {
        pov: povKey,
