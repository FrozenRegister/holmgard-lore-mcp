...
// Schema exports for typed handlers
...
export const getFactionStandingSchema = z.object({ entity_key: z.string().min(1), faction_key: z.string().min(1) })
...
export const learnFromEventSchema = z.object({ entity_id: z.string().min(1), event_id: z.string().min(1) })
export const migrateKnowledgeSchema = z.object({ world_id: z.string().min(1) })
export const getLocationOccupantsSchema = z.object({ location_key: z.string().min(1) })
export const getReachableLocationsSchema = z.object({ origin_key: z.string().min(1) })
export const senseEnvironmentSchema = z.object({ location_key: z.string().min(1), entity_key: z.string().min(1) })
...
export async function handle_thread_tick({ c, id, args }: TypedToolContext<typeof threadTickSchema>): Promise<Response> {
...
export async function handle_get_relationship({ c, id, args }: TypedToolContext<typeof getRelationshipSchema>): Promise<Response> {
...
export async function handle_get_faction_standing({ c, id, args }: TypedToolContext<typeof getFactionStandingSchema>): Promise<Response> {
...
export async function handle_get_entity_knowledge({ c, id, args }: TypedToolContext<typeof getEntityKnowledgeSchema>): Promise<Response> {
...
export async function handle_set_entity_knowledge({ c, id, args }: TypedToolContext<typeof setEntityKnowledgeSchema>): Promise<Response> {
...
export async function handle_learn_from_event({ c, id, args }: TypedToolContext<typeof learnFromEventSchema>): Promise<Response> {
  if (!c.env.RPG_DB) return c.json(makeError(id, -32603, 'D1 database unavailable', null), 200)
...
export async function handle_migrate_knowledge({ c, id, args }: TypedToolContext<typeof migrateKnowledgeSchema>): Promise<Response> {
  if (!c.env.RPG_DB) return c.json(makeError(id, -32603, 'D1 database unavailable', null), 200)
...
export async function handle_get_location_occupants({ c, id, args }: TypedToolContext<typeof getLocationOccupantsSchema>): Promise<Response> {
...
export async function handle_get_reachable_locations({ c, id, args }: TypedToolContext<typeof getReachableLocationsSchema>): Promise<Response> {
...
export async function handle_sense_environment({ c, id, args }: TypedToolContext<typeof senseEnvironmentSchema>): Promise<Response> {
...
export async function handle_get_thread_comparison({ c, id, args }: TypedToolContext<typeof getThreadComparisonSchema>): Promise<Response> {
...
export async function handle_check_convergence({ c, id, args }: TypedToolContext<typeof checkConvergenceSchema>): Promise<Response> {
  const threadA = args.thread_a.trim()
  const threadB = args.thread_b.trim()
  const { keys: keysA, rawValues: rawValuesA } = await resolveIndexedEntities(c, `_idx:thread:${threadA}`, 'Thread', threadA)
  const { keys: keysB, rawValues: rawValuesB } = await resolveIndexedEntities(c, `_idx:thread:${threadB}`, 'Thread', threadB)
