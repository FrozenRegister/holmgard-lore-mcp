  ...
  oneOf: [
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get',
          description: 'Get a lore entry by key or fuzzy query',
        },
        query: {
          type: 'string',
          minLength: 1,
          description: 'Exact key or fuzzy search query for the lore entry',
        },
      },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_batch',
          description: 'Get multiple lore entries by key',
        },
        keys: {
          type: 'array',
          items: { type: 'string', minLength: 1 },
          minItems: 1,
          description: 'Array of lore keys to fetch',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_section',
          description: 'Get specific sections from a lore entry',
        },
        key: { type: 'string', minLength: 1, description: 'Lore entry key' },
        sections: {
          type: 'array',
          items: { type: 'string', minLength: 1 },
          description: 'Section names to extract',
        },
        mode: {
          type: 'string',
          enum: ['strict', 'loose'],
          description: 'Match mode — loose allows partial section name matches (default: loose)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        ...
        prefix: {
          ...
          minLength: 1,
          description:
            'Only return keys starting with this prefix (e.g. "location", "item") — uses the prefix index when available instead of a full scan',
        },
        limit: {
          type: 'number',
          minimum: 1,
          maximum: 1000,
          description: 'Max number of keys to return (default: 1000)',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'search',
          description: 'Full-text search across lore entries',
        },
        ...
        match_mode: {
          ...
          enum: ['any', 'all', 'exact'],
          description:
            'Match mode — "any" (default, tokenized-OR: match if any token is found), "all" (AND: all tokens must be present, not necessarily contiguous), "exact" (full contiguous substring match)',
        },
        prefix: {
          type: 'string',
          minLength: 1,
          description:
            'Key prefix filter (e.g. "character", "location") — scopes the scan to entries whose key starts with this prefix',
        },
        max_results: {
          type: 'number',
          minimum: 1,
        ...
        scan_limit: {
          ...
          minimum: 1,
          maximum: 2000,
          description: 'Maximum entries to scan (default: 500)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'validate',
          description: 'Validate a topic key exists, with fuzzy did_you_mean suggestions',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'patch',
          description: 'Patch a lore entry with a targeted operation',
        },
        key: { type: 'string', minLength: 1, description: 'Lore entry key' },
        operation: {
          type: 'string',
          enum: ['replace', 'append', 'delete_field'],
          description: 'Patch operation type',
        },
        target: {
          type: 'string',
          description: 'Substring or field name to target (required for replace and delete_field)',
        },
        value: {
          type: 'string',
          description: 'Replacement or appended text (required for replace and append)',
        },
        dry_run: {
          type: 'boolean',
          description: 'Preview what would change without writing (default: false)',
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'batch_set',
          description: 'Write multiple lore entries in parallel',
        },
        entries: {
          type: 'array',
          minItems: 1,
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'batch_mutate',
          description: 'Apply increment or patch mutations to multiple entries sequentially',
        },
        mutations: {
          type: 'array',
          minItems: 1,
          items: {
            ...
            properties: {
              ...
              field_path: {
                type: 'string',
                description: 'Required for increment — markdown field name to increment',
              },
              increment: {
                type: 'number',
                description: 'Amount to increment (for increment action)',
              },
              reason: { type: 'string', description: 'Optional reason note for increment' },
              operation: {
                type: 'string',
                enum: ['replace', 'append', 'delete_field'],
                description: 'Patch operation type (for patch action)',
              },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'restore',
          description: 'Restore a lore entry from its most recent history snapshot',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'history',
          description: 'Get edit history for one or more lore entries',
        },
        keys: {
          type: 'array',
          items: { type: 'string', minLength: 1 },
          minItems: 1,
          description: 'Array of lore keys to fetch history for',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'increment',
          description: 'Increment a numeric markdown field in a lore entry',
        },
        key: { type: 'string', minLength: 1, description: 'Lore entry key' },
        field_path: {
          type: 'string',
          minLength: 1,
          description: 'Markdown field name to increment (e.g. "Count")',
        },
        increment: { type: 'number', description: 'Amount to increment by (default: 1)' },
        reason: {
          type: 'string',
          description: 'Optional reason label for the change (default: "system-update")',
        },
        dry_run: {
          type: 'boolean',
          description: 'Preview what would change without writing (default: false)',
    ...
    },
    {
      ...
      properties: {
        action: {
          ...
          description: 'Append text to a named section within a lore entry',
        },
        key: { type: 'string', minLength: 1, description: 'Lore entry key' },
        ...
        position: {
          type: 'string',
          enum: ['end', 'start'],
          description: 'Where to insert text within the section (default: end)',
        },
        auto_create: {
          type: 'boolean',
          description: 'Create the section if it does not exist (default: true)',
        },
      },
      additionalProperties: false,
    },
  ...
  oneOf: [
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'generate',
          description: 'Generate a new entity from an archetype',
        },
        archetype_key: {
          type: 'string',
          minLength: 1,
          description: 'Archetype lore key to generate the entity from',
        },
    ...
    },
    {
      ...
      properties: {
        ...
        new_location_key: {
          type: 'string',
          minLength: 1,
          description: 'Destination location lore key',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'roll_encounter',
          description: 'Roll for a random encounter at a location',
        },
        ...
        threat_level: {
          ...
          minimum: 1,
          maximum: 10,
          description: 'Threat level 1–10 (default: 5)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'advance_stage',
          description: 'Advance an entity to its next lifecycle stage',
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'batch_stage',
          description: 'Advance all entities at a location to their next stage',
        },
        location_key: { type: 'string', minLength: 1, description: 'Location lore key' },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_inventory',
          description: "Get an entity's inventory",
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
      },
      additionalProperties: false,
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_sensory_profile',
          description: "Get an entity's sensory profile",
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'set_sensory_profile',
          description: "Set an entity's sensory profile",
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_compatibility',
          description: 'Get compatibility score between two entities for an interaction type',
        },
        ...
        interaction_type: {
          type: 'string',
          minLength: 1,
          description: 'Type of interaction to evaluate',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'analyze_utility',
          description: 'Analyze an entity along a utility vector',
        },
        ...
        entity_role: {
          type: 'string',
          enum: ['subject', 'actor'],
          description: 'Role of the entity in the interaction (default: subject)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'list_consumption_timelines',
          description: 'List entity consumption timelines, optionally filtered by status',
        },
        status_filter: {
          type: 'string',
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'create_consumption_timeline',
          description: 'Create a new consumption timeline binding an entity to a predator',
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key being consumed' },
        predator_key: {
          type: 'string',
          minLength: 1,
          description: 'Predator entity lore key doing the consuming',
        },
        stages: {
          type: 'number',
          minimum: 1,
          maximum: 20,
          description: 'Total number of dissolution stages',
        },
        stage_timer: {
          type: 'number',
          minimum: 1,
          description: 'Timer ticks per stage (decrements toward 0)',
        },
        terminal_state: {
          type: 'string',
          minLength: 1,
          description: 'Final conversion state (e.g. consumed-nutrient, vessel, ornament)',
        },
        current_stage: {
          type: 'number',
          minimum: 0,
          description: 'Starting stage index (default: 0)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'set_consumption_timeline',
          description: 'Update an existing consumption timeline with new parameters',
        },
        entity_key: {
          type: 'string',
          minLength: 1,
          description: 'Entity lore key whose timeline to update',
        },
        ...
        terminal_state: {
          type: 'string',
          minLength: 1,
          description: 'New terminal conversion state',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'list_active_threads',
          description: 'List all active world threads',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'resolve_interaction',
          description: 'Resolve an interaction between two entities',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'destroy',
          description: 'Destroy an entity, removing it from the world',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          ...
          description:
            "Get an entity's D1-backed interaction attributes (Weight-1, Weight-2, or any campaign-defined numeric field)",
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          ...
          description:
            "Set an entity's D1-backed interaction attributes; becomes the primary source of truth over KV markdown for resolve_interaction/analyze_utility/get_compatibility",
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
        attributes: {
          type: 'object',
          ...
          minProperties: 1,
          description:
            'Arbitrary numeric attributes, e.g. {"weight-1": 0.3, "tenderness-index": 0.7} — campaigns own their own attribute schema',
        },
        merge: {
          type: 'boolean',
          description: 'Merge into existing attributes (default: true) vs. replace wholesale',
        },
      },
      additionalProperties: false,
    },
  ...
  oneOf: [
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'thread_tick',
          description: 'Advance a world thread by one tick',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_relationship',
          description: 'Get the relationship state between two entities',
        },
        entity_a: { type: 'string', minLength: 1, description: 'First entity lore key' },
        entity_b: { type: 'string', minLength: 1, description: 'Second entity lore key' },
      },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_faction_standing',
          description: "Get an entity's standing with a faction",
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
        entity_name: { type: 'string', minLength: 1, description: 'Alias for entity_key' },
        faction_key: { type: 'string', minLength: 1, description: 'Faction lore key' },
        faction_name: { type: 'string', minLength: 1, description: 'Alias for faction_key' },
      },
      allOf: [
        { anyOf: [{ required: ['entity_key'] }, { required: ['entity_name'] }] },
        { anyOf: [{ required: ['faction_key'] }, { required: ['faction_name'] }] },
      ],
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_entity_knowledge',
          description: 'Get what an entity knows about a topic',
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
        entity_name: { type: 'string', minLength: 1, description: 'Alias for entity_key' },
        topic: {
          type: 'string',
          minLength: 1,
          description: 'Topic or lore key to query knowledge about',
        },
      },
      anyOf: [{ required: ['entity_key'] }, { required: ['entity_name'] }],
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_location_occupants',
          description: 'Get all entities currently at a location',
        },
        location_key: { type: 'string', minLength: 1, description: 'Location lore key' },
        location_id: { type: 'string', minLength: 1, description: 'Alias for location_key' },
      },
      anyOf: [{ required: ['location_key'] }, { required: ['location_id'] }],
      additionalProperties: false,
    },
    {
      type: 'object',
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_reachable_locations',
          description: 'Get all locations reachable from an origin',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'sense_environment',
          description: "Sense the environment from an entity's perspective at a location",
        },
        location_key: { type: 'string', minLength: 1, description: 'Location lore key' },
        entity_key: {
          type: 'string',
          minLength: 1,
          description: 'Entity lore key doing the sensing',
        },
        entity_name: { type: 'string', minLength: 1, description: 'Alias for entity_key' },
        radius: {
          type: 'string',
          description:
            'Accepted but currently unused — sense_environment does not compute a radius.',
        },
      },
      anyOf: [{ required: ['entity_key'] }, { required: ['entity_name'] }],
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_thread_comparison',
          description: 'Compare the state of two world threads',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'check_convergence',
          description: 'Check whether two threads are converging',
        },
        thread_a: { type: 'string', minLength: 1, description: 'First thread identifier' },
        thread_b: { type: 'string', minLength: 1, description: 'Second thread identifier' },
      },
  ...
  oneOf: [
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'activate',
          description: 'Activate a scene as the current active scene',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          ...
          description: 'Present available choices to an entity in a scene',
        },
        scene_key: { type: 'string', minLength: 1, description: 'Scene lore key' },
        entity_key: {
          type: 'string',
          minLength: 1,
          description: 'Entity lore key being presented choices',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'commit_choice',
          description: 'Commit a choice made by an entity',
        },
        choice_id: { type: 'string', minLength: 1, description: 'Choice identifier to commit' },
        entity_key: {
          type: 'string',
          minLength: 1,
          description: 'Entity lore key making the choice',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_history',
          description: 'Get the choice history for an entity',
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'brief',
          description: 'Get a scene brief summarising the current state of a location or scene',
        },
        location_key: {
          type: 'string',
          description: 'Location lore key (use location_key or scene_key)',
        },
        scene_key: {
          type: 'string',
          description: 'Scene lore key (use location_key or scene_key)',
        },
        include: {
          type: 'object',
          properties: {
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'render_pov',
          description: "Render a scene from an entity's point of view",
        },
        pov_entity_key: {
          type: 'string',
          minLength: 1,
          description: 'Entity whose POV to render from',
        },
        scene_key: {
          type: 'string',
          description: 'Scene lore key (optional if location_key given)',
        },
        location_key: {
          type: 'string',
          description: 'Location lore key (optional if scene_key given)',
        },
        include_voice_hints: {
          type: 'boolean',
          description: 'Include narrative voice hints in the output',
        },
        reveal_threshold: {
          type: 'number',
          minimum: 0,
          maximum: 1,
          description: 'Knowledge threshold 0–1 for revealing hidden details',
        },
      },
      additionalProperties: false,
    },
  ...
  oneOf: [
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'append_event',
          description: 'Append an event to the event log for an entity',
        },
        entity_key: {
          type: 'string',
          minLength: 1,
          description: 'Entity lore key the event belongs to',
        },
        ...
        description: { type: 'string', description: 'Alias for detail' },
        at: { type: 'string', description: 'ISO datetime of the event (defaults to now)' },
        date: { type: 'string', description: 'Alias for at' },
        source: {
          type: 'string',
          description: 'Accepted but currently unused — informational provenance note.',
        },
        world_id: {
          type: 'string',
          description:
            'D1 world ID — when supplied, the event is also written to the timeline_events D1 table (dual-write alongside the KV event log), returned as d1_event_id',
        },
        entity_id: {
          type: 'string',
          description:
            'D1 character ID the event refers to — validated as a foreign key against the characters table when world_id is supplied',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'get_event_log',
          description: 'Get the event log for one or more entities',
        },
        entity_key: {
          oneOf: [
        ...
        until: {
          type: 'string',
          description: 'ISO datetime — only return events before this time',
        },
        ...
        tier: {
          type: 'string',
          description:
            'Filter by signal tier from event_verb_taxonomy — "high", or comma-separated e.g. "high,medium". Requires D1 (errors if RPG_DB unavailable).',
        },
        limit: {
          type: 'number',
          minimum: 1,
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'taxonomy_list',
          description: 'List event verbs and their signal tier/category, optionally filtered',
        },
        tier: { type: 'string', enum: ['high', 'medium', 'low'], description: 'Filter by tier' },
        category: {
          type: 'string',
          description: 'Filter by category (e.g. combat, narrative, social, production)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          ...
          const: 'taxonomy_set',
          description:
            'Upsert a verb into the event taxonomy (runtime-updatable, no code deploy needed)',
        },
        ...
        category: {
          type: 'string',
          minLength: 1,
          description: 'Category, e.g. combat, narrative, social, production',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'taxonomy_delete',
          description: 'Remove a verb from the event taxonomy',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          ...
          description: 'List recent lore changes across the store',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'tag_topic',
          description: 'Add or remove tags on a lore topic',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'list_tags',
          description: 'List all tags in use, optionally filtered by prefix',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          ...
          description: 'Bookmark the current world state with a name',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'world_diff',
          description: 'Diff the world state between two bookmarks or timepoints',
        },
        ...
        detail: {
          type: 'string',
          enum: ['summary', 'fields', 'text'],
          description: 'Level of diff detail (default: summary)',
        },
    ...
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'plant_setup',
          description: 'Plant a narrative setup (Chekhov gun) for later payoff',
        },
        id: { type: 'string', minLength: 1, description: 'Unique setup identifier' },
        setup_id: { type: 'string', minLength: 1, description: 'Alias for id' },
        ...
        tension: {
          ...
          minimum: 1,
          maximum: 5,
          description: 'Narrative tension level 1–5',
        },
        expected_in: { type: 'string', description: 'Scene or chapter where payoff is expected' },
        actors: {
          type: 'array',
          items: { type: 'string' },
          description: 'Entity keys involved in this setup',
        },
        payoff_type: {
          type: 'string',
          description:
            'Accepted but currently unused — informational note on the expected payoff category.',
        },
      },
      anyOf: [{ required: ['id'] }, { required: ['setup_id'] }],
      additionalProperties: false,
    },
    {
      type: 'object',
      ...
      properties: {
        action: {
          type: 'string',
          const: 'pay_off_setup',
          description: 'Pay off a previously planted setup',
        },
        ...
        status: {
          type: 'string',
          enum: ['paid', 'abandoned', 'deferred'],
          description: 'Payoff status (default: paid)',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'list_unpaid_setups',
          description: 'List all setups that have not yet been paid off',
        },
        actor: { type: 'string', description: 'Filter by entity key involved as an actor' },
        scope: {
          type: 'string',
          enum: ['scene', 'chapter', 'story'],
          description: 'Filter by expected payoff scope',
        },
        min_tension: {
          type: 'number',
          minimum: 1,
          maximum: 5,
          description: 'Minimum tension level to include',
        },
      },
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'set_goal',
          description: 'Set or update a goal for an entity',
        },
        entity_key: { type: 'string', minLength: 1, description: 'Entity lore key' },
        entity_name: { type: 'string', minLength: 1, description: 'Alias for entity_key' },
        goal_id: {
          type: 'string',
          minLength: 1,
          description: 'Unique goal identifier for this entity',
        },
        goal_name: { type: 'string', minLength: 1, description: 'Alias for goal_id' },
        description: { type: 'string', minLength: 1, description: 'Goal description' },
        goal_description: { type: 'string', minLength: 1, description: 'Alias for description' },
        parent: { type: 'string', description: 'Parent goal ID if this is a sub-goal' },
        status: {
          type: 'string',
          enum: ['active', 'blocked', 'achieved', 'abandoned'],
      ...
      allOf: [
        { anyOf: [{ required: ['entity_key'] }, { required: ['entity_name'] }] },
        { anyOf: [{ required: ['goal_id'] }, { required: ['goal_name'] }] },
        { anyOf: [{ required: ['description'] }, { required: ['goal_description'] }] },
      ],
      additionalProperties: false,
    },
    {
      ...
      properties: {
        action: {
          type: 'string',
          const: 'check_continuity',
          description: 'Run continuity checks across the world state',
        },
        ...
        severity_floor: {
          ...
          description:
            'Minimum severity to report (default: info). low/medium/moderate/high/critical are accepted aliases for info/warn/warn/error/error.',
        },
      },
      additionalProperties: false,
    },
