...
import {
  ...
  handle_analyze_utility,
  analyzeUtilitySchema,
  handle_map_integration,
  mapIntegrationSchema,
  handle_generate_entity,
  generateEntitySchema,
  handle_roll_encounter,
  rollEncounterSchema,
  handle_advance_state_stage,
  advanceStateStageSchema,
  handle_process_stage_batch,
  processStageBatchSchema,
  handle_get_sensory_profile,
  getSensoryProfileSchema,
  handle_get_compatibility,
  getCompatibilitySchema,
  handle_get_inventory,
  getInventorySchema,
  handle_transfer_item,
  transferItemSchema,
  handle_list_consumption_timelines,
  listConsumptionTimelinesSchema,
  handle_list_active_threads,
  handle_destroy_entity,
  destroyEntitySchema,
  ...
  handle_set_sensory_profile,
  setSensoryProfileSchema,
  handle_get_entity_attributes,
  getEntityAttributesSchema,
  handle_set_entity_attributes,
  setEntityAttributesSchema,
} from './entity'
...
const ACTION_MAP: Record<string, ActionSpec | ToolHandler> = {
  ...
  transfer_item: defineAction(transferItemSchema, handle_transfer_item, {
    from_entity: 'character:eira-holt',
    to_entity: 'character:gerent',
    item_key: 'sword',
    quantity: 1,
  }),
  ...
  resolve_interaction: defineAction(resolveInteractionSchema, handle_resolve_interaction, {
    entity_a_id: 'character:eira-holt',
    entity_b_id: 'character:gerent',
    action_type: 'confront',
  }),
  destroy: defineAction(destroyEntitySchema, handle_destroy_entity, {
    entity_key: 'entity:temp-encounter-12345',
  }),
  create_consumption_timeline: defineAction(
    createConsumptionTimelineSchema,
    handle_create_consumption_timeline,
    {
      entity_key: 'character:eira-holt',
      predator_key: 'character:gerent',
      stages: 5,
      stage_timer: 10,
  ...
  set_consumption_timeline: defineAction(
    setConsumptionTimelineSchema,
    handle_set_consumption_timeline,
    {
      entity_key: 'character:eira-holt',
  ...
  set_attributes: defineAction(setEntityAttributesSchema, handle_set_entity_attributes, {
    entity_key: 'character:eira-holt',
    attributes: { 'weight-1': 0.3, 'weight-2': 0.15 },
  }),
}
