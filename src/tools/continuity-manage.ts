...
import {
  ...
  handle_bookmark_state, bookmarkStateSchema,
  handle_world_diff, worldDiffSchema,
  handle_plant_setup, plantSetupSchema,
  handle_pay_off_setup, payOffSetupSchema,
  handle_list_unpaid_setups, listUnpaidSetupsSchema,
  handle_set_goal, setGoalSchema,
  handle_check_continuity, checkContinuitySchema,
} from './meta'
...
const ACTION_MAP: Record<string, ActionSpec | ToolHandler> = {
  ...
  tag_topic: defineAction(tagTopicSchema, handle_tag_topic, {
    key: 'character:eira-holt', add: ['needs-review']
  }),
  find_by_tag: defineAction(findByTagSchema, handle_find_by_tag, {
    tags: ['needs-review'], mode: 'any', limit: 20
  }),
  list_tags: defineAction(listTagsSchema, handle_list_tags, {
    prefix: 'needs', with_counts: true, limit: 200
  }),
  bookmark_state: defineAction(bookmarkStateSchema, handle_bookmark_state, {
    name: 'phase-9-complete', note: 'End of phase 9'
  }),
  world_diff: defineAction(worldDiffSchema, handle_world_diff, {
    from: 'phase-9-complete', detail: 'summary'
  }),
  plant_setup: defineAction(plantSetupSchema, handle_plant_setup, {
    id: 'church-ambush-foreshadow', description: 'Church courier spotted near Marsh-end canal', tension: 3
  }),
  pay_off_setup: defineAction(payOffSetupSchema, handle_pay_off_setup, {
    id: 'church-ambush-foreshadow', resolution: 'Ambush occurred at the canal crossing', status: 'paid'
  }),
  ...
  set_goal: defineAction(setGoalSchema, handle_set_goal, {
    entity_key: 'character:eira-holt', goal_id: 'survive-tribunal', description: 'Survive the Church tribunal in Novigrad on 15 Jun 1264'
  }),
  check_continuity: defineAction(checkContinuitySchema, handle_check_continuity, {
    scope: 'character', severity_floor: 'warn'
  }),
}
