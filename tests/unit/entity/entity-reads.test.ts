...
// Build an env object with the mock as RPG_DB.
// Note: miniflare always provides a real RPG_DB binding from wrangler.test.jsonc;
// passing a truthy mock overrides it, but null does not. So we test the empty-results
// path (real empty D1) rather than the 503 path (no binding).
function makeEnv(db: unknown) {
  return { RPG_DB: db } as any;
}
