...
describe.skipIf(!MCP_API_KEY)('rpg waypoint / party march (#328)', () => {
  ...
  async function createWorld() {
    const world = parseResult(await tool('rpg', { sub: 'world', action: 'create', name: `GotlandWorld ${uid()}`, theme: 'fantasy' }))
    createdWorldIds.push(world.worldId)
    return world.worldId as string
  }

  async function createParty(worldId: string) {
    const party = parseResult(await tool('rpg', { sub: 'party', action: 'create', name: `Test Party ${uid()}`, worldId }))
  ...
  it('seed_defaults seeds the Gotland waypoints and precomputed distances', async () => {
    ...
    const res = parseResult(await tool('rpg', { sub: 'waypoint', action: 'seed_defaults', worldId }))
  ...
  it('a party can march between two seeded Gotland waypoints and arrives after enough days', async () => {
    ...
    const march = parseResult(await tool('rpg', {
      sub: 'party', action: 'begin_march', partyId, fromWaypointName: 'Visby', toWaypointName: 'Roma Kloster',
    }))
    expect(march.success).toBe(true)
    expect(march.blocked).toBe(false)
    expect(march.distanceKm).toBeGreaterThan(0)

    const status = parseResult(await tool('rpg', { sub: 'party', action: 'get_march_status', partyId }))
  ...
  it('begin_march returns a structured blocked (not error) response for a pair with no precomputed distance', async () => {
    ...
    await tool('rpg', { sub: 'waypoint', action: 'register', worldId, name: isolatedName, lat: 57.0, lon: 18.0, q: 100, r: 100 })
    const partyId = await createParty(worldId)

    const march = parseResult(await tool('rpg', {
      sub: 'party', action: 'begin_march', partyId, fromWaypointName: 'Visby', toWaypointName: isolatedName,
    }))
    expect(march.success).toBe(true)
