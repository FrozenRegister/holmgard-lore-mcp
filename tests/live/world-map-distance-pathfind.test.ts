...
describe.skipIf(!MCP_API_KEY)('rpg world_map distance/pathfind (#430)', () => {
  ...
  async function createWorld() {
    const world = parseResult(await tool('rpg', { sub: 'world', action: 'create', name: `DistancePathfindWorld ${uid()}`, theme: 'fantasy' }))
  ...
  it('distance reports hexDistance and null km on a non-geo-calibrated world', async () => {
    ...
    const res = parseResult(await tool('rpg', { sub: 'world_map', action: 'distance', worldId, from: { q: 0, r: 0 }, to: { q: 3, r: 0 } }))
  ...
  it('distance computes straightLineKm once geo-calibrated', async () => {
    const worldId = await createWorld()
    await tool('rpg', { sub: 'waypoint', action: 'calibrate', worldId, originLat: 57.6, originLon: 18.3, kmPerHex: 1 })
    const res = parseResult(await tool('rpg', { sub: 'world_map', action: 'distance', worldId, from: { q: 0, r: 0 }, to: { q: 3, r: 0 } }))
    expect(res.success).toBe(true)
  ...
  it('pathfind routes around an impassable hex and reports the path', async () => {
    ...
    await tool('rpg', { sub: 'biome', action: 'register', worldId, name: biomeName, movementCost: 1.0, modeCosts: { foot: 0 } })
    await tool('rpg', { sub: 'world_map', action: 'patch', worldId, hexes: [{ q: 1, r: 0, biome: biomeName }] })

    const res = parseResult(await tool('rpg', { sub: 'world_map', action: 'pathfind', worldId, from: { q: 0, r: 0 }, to: { q: 2, r: 0 }, mode: 'foot' }))
  ...
  it('pathfind avoids a requested zone_type', async () => {
    const worldId = await createWorld()
    await tool('rpg', { sub: 'world_map', action: 'suggest_poi', worldId, query: `Territory ${uid()}`, q: 1, r: 0, radius: 0, zoneType: 'predator_zone' })

    const res = parseResult(await tool('rpg', { sub: 'world_map', action: 'pathfind', worldId, from: { q: 0, r: 0 }, to: { q: 2, r: 0 }, avoid: ['predator_zone'] }))
    expect(res.routable).toBe(true)
    expect(res.path.some((p: { q: number; r: number }) => p.q === 1 && p.r === 0)).toBe(false)
  })
})
