...
describe.skipIf(!MCP_API_KEY)('rpg waypoint register — optional lat/lon (#399)', () => {
  it('registers without lat/lon on a non-geo-calibrated world', async () => {
    const worldRes = parseResult(
      await tool('rpg', { sub: 'world', action: 'create', name: `Waypoint Test World ${uid()}` }),
    )
  ...
  it('requires lat/lon once the world is geo-calibrated', async () => {
    const worldRes = parseResult(
      await tool('rpg', {
        sub: 'world',
        action: 'create',
        name: `Calibrated Waypoint World ${uid()}`,
      }),
    )
    const worldId = worldRes.worldId
    await tool('rpg', {
    ...
    const missingLatLon = parseResult(
      await tool('rpg', {
        sub: 'waypoint',
        action: 'register',
        worldId,
