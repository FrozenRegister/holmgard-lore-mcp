...
describe.skipIf(!MCP_API_KEY)('Append to Section', () => {
  ...
  it('appends to end of section', async () => {
    const res = await tool('append_to_section', { key, section: 'Personality', text: ' Loyal to companions.' })
  ...
  it('section not found with auto_create=false returns error', async () => {
    const res = await tool('append_to_section', {
      key, section: 'NonExistentSection', text: 'Some text.', auto_create: false,
    })
    expect(res.result.error).toBe('section_not_found')
  })

  it('auto_create=true creates new section', async () => {
    const res = await tool('append_to_section', { key, section: 'Notes', text: 'First note.' })
  ...
  it('empty text returns empty_text error', async () => {
    const res = await tool('append_to_section', { key, section: 'Personality', text: '' })
    expect(res.result.error).toBe('empty_text')
  })

  it('non-existent key returns key_not_found', async () => {
    const res = await tool('append_to_section', {
      key: `character:ats-does-not-exist-${uid()}`, section: 'Personality', text: 'Text.',
    })
