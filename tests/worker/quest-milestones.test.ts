...
describe('Quest Milestones', () => {
  ...
  async function seedQuest(): Promise<string> {
    const world = await callTool('rpg', { sub: 'world', action: 'create', name: `World ${crypto.randomUUID()}`, theme: 'fantasy' })
    const quest = await callTool('rpg', {
      sub: 'quest', action: 'create', worldId: world.worldId, name: 'Test Quest', description: 'A quest for milestone tests.',
    })
  ...
  innerDescribe('POST /admin/quests/:questId/milestones', () => {
    it('creates a milestone with sort_order 0 for the first one', async () => {
      ...
      const res = await adminPost(`/admin/quests/${questId}/milestones`, { title: 'Find the map', secret: ADMIN_SECRET })
    ...
    it('auto-increments sort_order for subsequent milestones', async () => {
      const questId = await seedQuest()
      await adminPost(`/admin/quests/${questId}/milestones`, { title: 'First', secret: ADMIN_SECRET })
      const res = await adminPost(`/admin/quests/${questId}/milestones`, { title: 'Second', secret: ADMIN_SECRET })
      const body = await res.json() as Record<string, any>
    ...
    it('rejects whitespace-only title', async () => {
      const questId = await seedQuest()
      const res = await adminPost(`/admin/quests/${questId}/milestones`, { title: '   ', secret: ADMIN_SECRET })
      expect(res.status).toBe(400)
    })

    it('rejects unauthorized requests', async () => {
      const questId = await seedQuest()
      const res = await adminPost(`/admin/quests/${questId}/milestones`, { title: 'X', secret: 'wrong' })
    ...
    it('accepts all optional fields', async () => {
      ...
      await adminPost(`/admin/quests/${questId}/milestones`, {
        title: 'Defeat the boss', notes: 'Bring potions', status: 'in_progress',
        linked_entity_type: 'character', linked_entity_id: 'char-1', color: '#ff0000', is_private: true,
        secret: ADMIN_SECRET,
      })
  ...
  innerDescribe('PATCH /admin/quests/:questId/milestones/:milestoneId', () => {
    async function seedMilestone(questId: string): Promise<string> {
      const res = await adminPost(`/admin/quests/${questId}/milestones`, { title: 'Original', secret: ADMIN_SECRET })
      const body = await res.json() as Record<string, any>
      return body.id
    }
    ...
    it('updates every patchable field', async () => {
      ...
      const res = await adminPatch(`/admin/quests/${questId}/milestones/${milestoneId}`, {
        title: 'Updated', status: 'completed', notes: 'done', linked_entity_type: 'location',
        linked_entity_id: 'loc-1', color: '#00ff00', is_private: true, sort_order: 5, secret: ADMIN_SECRET,
      })
      expect(res.status).toBe(200)
      const listBody = await getMilestones(questId)
    ...
    it('supports partial updates (only touches provided fields)', async () => {
      ...
      const milestoneId = await seedMilestone(questId)
      const res = await adminPatch(`/admin/quests/${questId}/milestones/${milestoneId}`, {
        status: 'failed', secret: ADMIN_SECRET,
      })
      expect(res.status).toBe(200)
      const listBody = await getMilestones(questId)
    ...
    it('rejects unauthorized requests', async () => {
      const questId = await seedQuest()
      const milestoneId = await seedMilestone(questId)
      const res = await adminPatch(`/admin/quests/${questId}/milestones/${milestoneId}`, { title: 'X', secret: 'wrong' })
      expect(res.status).toBe(401)
    })
    ...
    it('500 responses never expose internal details', async () => {
      const questId = await seedQuest()
      const milestoneId = await seedMilestone(questId)
      const res = await SELF.fetch(`http://example.com/admin/quests/${questId}/milestones/${milestoneId}`, {
  ...
  innerDescribe('DELETE /admin/quests/:questId/milestones/:milestoneId', () => {
    it('deletes a milestone', async () => {
      const questId = await seedQuest()
      const createRes = await adminPost(`/admin/quests/${questId}/milestones`, { title: 'To delete', secret: ADMIN_SECRET })
      const { id } = await createRes.json() as Record<string, any>
      const res = await adminDelete(`/admin/quests/${questId}/milestones/${id}`, { secret: ADMIN_SECRET })
      expect(res.status).toBe(200)
      const listBody = await getMilestones(questId)
    ...
    it('rejects unauthorized requests', async () => {
      const questId = await seedQuest()
      const createRes = await adminPost(`/admin/quests/${questId}/milestones`, { title: 'X', secret: ADMIN_SECRET })
      const { id } = await createRes.json() as Record<string, any>
      const res = await adminDelete(`/admin/quests/${questId}/milestones/${id}`, { secret: 'wrong' })
      expect(res.status).toBe(401)
    })

    it('500 responses never expose internal details', async () => {
      const questId = await seedQuest()
      const res = await SELF.fetch(`http://example.com/admin/quests/${questId}/milestones/whatever`, {
  ...
  innerDescribe('GET /api/entities/quests/:id/milestones', () => {
    it('returns milestones ordered by sort_order', async () => {
      const questId = await seedQuest()
      await adminPost(`/admin/quests/${questId}/milestones`, { title: 'First', secret: ADMIN_SECRET })
      await adminPost(`/admin/quests/${questId}/milestones`, { title: 'Second', secret: ADMIN_SECRET })
