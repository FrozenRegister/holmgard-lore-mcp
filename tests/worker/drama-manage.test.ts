...
describe('handleDramaManage', () => {
  ...
  it('opposed_check: errors when character_b not found', async () => {
    ...
    const r = await handleDramaManage(db(), {
      action: 'opposed_check',
      character_a: 'char-oc-a',
      ability_a: 'cha',
      character_b: 'missing-b',
      ability_b: 'wis',
    })
    const body = JSON.parse(r.content[0].text) as { error: boolean; message: string }
    expect(body.error).toBe(true)
  ...
  it('opposed_check: accepts advantage/disadvantage side flags', async () => {
    ...
    const r = await handleDramaManage(db(), {
      action: 'opposed_check',
      character_a: 'char-adv-a',
      ability_a: 'cha',
      character_b: 'char-adv-b',
      ability_b: 'cha',
      advantage: 'a',
      disadvantage: 'b',
    })
  ...
  it('group_check: requires non-empty side_a', async () => {
    const r = await handleDramaManage(db(), {
      action: 'group_check',
      side_a: [],
      side_b: [{ character: 'x', ability: 'cha' }],
    })
    const body = JSON.parse(r.content[0].text) as { error: boolean }
    expect(body.error).toBe(true)
  })
  ...
  it('group_check: requires non-empty side_b', async () => {
    const r = await handleDramaManage(db(), {
      action: 'group_check',
      side_a: [{ character: 'x', ability: 'cha' }],
      side_b: [],
    })
    const body = JSON.parse(r.content[0].text) as { error: boolean }
    expect(body.error).toBe(true)
  ...
  it('group_check: best mode — resolves group vs group', async () => {
    ...
    const r = await handleDramaManage(db(), {
      ...
      mode: 'best',
      side_a: [
        { character: 'char-ga1', ability: 'cha' },
        { character: 'char-ga2', ability: 'cha' },
      ],
    ...
    const body = JSON.parse(r.content[0].text) as {
      success: boolean
      winner: string
      pooled_score_a: number
      pooled_score_b: number
      rolls_a: unknown[]
      rolls_b: unknown[]
    }
    expect(body.success).toBe(true)
  ...
  it('group_check: sum mode aggregates modifiers', async () => {
    ...
    const r = await handleDramaManage(db(), {
      ...
      mode: 'sum',
      side_a: [
        { character: 'char-gs1', ability: 'cha' },
        { character: 'char-gs2', ability: 'cha' },
      ],
  ...
  it('group_check: pool mode applies helper bonus', async () => {
    ...
    const r = await handleDramaManage(db(), {
      ...
      mode: 'pool',
      side_a: [
        { character: 'char-gp1', ability: 'cha' },
        { character: 'char-gp2', ability: 'cha' },
      ],
  ...
  it('group_check: notFound character produces notFound entry', async () => {
    ...
    const r = await handleDramaManage(db(), {
      ...
      mode: 'best',
      side_a: [
        { character: 'char-gf1', ability: 'cha' },
        { character: 'not-in-db', ability: 'cha' },
      ],
      side_b: [{ character: 'char-gf2', ability: 'cha' }],
    })
    const body = JSON.parse(r.content[0].text) as {
      success: boolean
      rolls_a: Array<{ notFound: boolean }>
    }
    expect(body.success).toBe(true)
  ...
  it('social_combat: requires at least 2 participants', async () => {
    const r = await handleDramaManage(db(), {
      ...
      participants: [{ character: 'x', leverage: 0 }],
    })
    const body = JSON.parse(r.content[0].text) as { error: boolean }
    expect(body.error).toBe(true)
  })
  ...
  it('social_combat: works without arena/stakes and with unknown character (uses defaults)', async () => {
    ...
    const r = await handleDramaManage(db(), {
      action: 'social_combat',
      participants: [
        { character: 'char-sc3', leverage: 0 },
        { character: 'unknown-sc', leverage: 0 },
      ],
      rounds: 1,
    })
  ...
  it('dramatic_conflict: resolves multi-tick campaign', async () => {
    ...
    const body = JSON.parse(r.content[0].text) as {
      ...
      ticks: Array<{
        tick: number
        side_results: Array<{ side: string; external_bonus: number; total: number }>
        winner_name: string
  ...
  it("roll_ability: a mental check on the host id uses the active passenger's score, not the host's own", async () => {
    ...
    await seedChar(
      ...
      'Bellona Keel',
      { cha: 18 },
      { hostBodyId: 'kat-host', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
    await seedChar(
      'kat-fork',
      'Cordelia Keel',
      { cha: 15 },
      { hostBodyId: 'kat-host', active: 0, updatedAt: now },
    )
  ...
  it("roll_ability: a physical check on the host id always uses the host's own score, even while a passenger drives", async () => {
    ...
    await seedChar(
      ...
      'Bellona Keel 2',
      { str: 20 },
      { hostBodyId: 'kat-host2', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
  ...
  it("roll_ability: calling directly on the passenger's own id resolves the same group", async () => {
    ...
    await seedChar(
      ...
      'Bellona Keel 3',
      { cha: 18 },
      { hostBodyId: 'kat-host3', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )

    const r = await handleDramaManage(db(), {
      action: 'roll_ability',
      character: 'kat-general3',
      ability: 'cha',
    })
    const body = JSON.parse(r.content[0].text) as {
      success: boolean
  ...
  it('opposed_check: co-habitating driver resolves correctly against a solo opponent', async () => {
    ...
    await seedChar(
      ...
      'Cordelia Keel 4',
      { cha: 18 },
      { hostBodyId: 'kat-host4', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
    await seedChar('rival', 'Rival Diplomat', { cha: 10 })

    const r = await handleDramaManage(db(), {
      action: 'opposed_check',
      character_a: 'kat-host4',
      ability_a: 'cha',
      character_b: 'rival',
      ability_b: 'cha',
    })
    const body = JSON.parse(r.content[0].text) as {
      success: boolean
      a: { character: string; score: number }
      b: { character: string; score: number }
    }
    expect(body.success).toBe(true)
