...
async function seedCharacter(name: string): Promise<string> {
  ...
  await env.RPG_DB.prepare(
    ...
    .bind(id, name, '{}', 10, 10, 10, 1, now, now)
    .run()
  return id
}
...
describe('entity_manage get_attributes / set_attributes', () => {
  ...
  it('set_attributes: creates a new row and get_attributes reads it back', async () => {
    ...
    const setRes = await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:fresh-attrs',
      attributes: { 'weight-1': 0.3, 'tenderness-index': 0.7 },
    })
  ...
  it('set_attributes: resolves and stores character_id via name match', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:linked-attrs-subject',
      attributes: { 'weight-2': 0.15 },
    })
  ...
  it('set_attributes: merge:true (default) folds new attributes into existing row', async () => {
    ...
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:merge-attrs',
      attributes: { 'weight-1': 0.2, 'weight-2': 0.4 },
    })
    const res = await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:merge-attrs',
      attributes: { 'weight-2': 0.9, 'cortisol-level': 0.1 },
    })
  ...
  it('set_attributes: merge:false replaces the stored attribute set wholesale', async () => {
    ...
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:replace-attrs',
      attributes: { 'weight-1': 0.2, 'weight-2': 0.4 },
    })
    const res = await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:replace-attrs',
      attributes: { 'weight-2': 0.9 },
      merge: false,
    })
  ...
  it('set_attributes: falls through to overwrite when existing stored JSON is corrupt', async () => {
    ...
    const now = new Date().toISOString()
    await env.RPG_DB.prepare(
      'INSERT INTO entity_attributes (id, lore_key, character_id, attributes, created_at, updated_at) VALUES (?, ?, NULL, ?, ?, ?)',
    )
      .bind(crypto.randomUUID(), 'character:corrupt-attrs', 'not valid json{{', now, now)
      .run()
    const res = await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:corrupt-attrs',
      attributes: { 'weight-1': 0.5 },
      merge: true,
    })
  ...
  it('set_attributes: rejects an empty attributes object', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:empty-attrs',
      attributes: {},
    })
    expect(res.error).toBeDefined()
    expect(res.error.code).toBe(-32602)
  ...
  it('get_attributes: returns source none when the stored attributes JSON is not an object', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind(crypto.randomUUID(), 'character:non-object-attrs', '[1,2,3]', now, now)
      .run()
    const res = await callTool('entity_manage', {
      action: 'get_attributes',
      entity_key: 'character:non-object-attrs',
    })
    expect(res.result.source).toBe('none')
    expect(res.result.attributes).toEqual({})
  })
...
describe('resolve_interaction — D1 attribute priority (#410)', () => {
  ...
  it('uses D1 weight-1/weight-2 over conflicting KV markdown values', async () => {
    ...
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:d1-attacker',
      attributes: { 'weight-1': 1.0 },
    })
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:d1-defender',
      attributes: { 'weight-2': 0.0 },
    })

    const res = await callTool('entity_manage', {
      action: 'resolve_interaction',
      entity_a_id: 'character:d1-attacker',
      entity_b_id: 'character:d1-defender',
      action_type: 'consume',
    })
  ...
  it('falls back to kv source when no D1 attributes exist', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'resolve_interaction',
      entity_a_id: 'character:kv-only-a',
      entity_b_id: 'character:kv-only-b',
      action_type: 'test',
    })
...
describe('analyze_utility — D1 attribute priority (#410)', () => {
  ...
  it('D1 attributes override matching KV fields and mark breakdown source', async () => {
    await seedKV(
      'character:d1-utility',
      ['**Tenderness-Index:** 0.10', '**Fat-Marbling-Index:** 0.80'].join('\n'),
    )
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:d1-utility',
      attributes: { 'tenderness-index': 0.95 },
    })

    const res = await callTool('entity_manage', {
      action: 'analyze_utility',
      entity_id: 'character:d1-utility',
  ...
  it('D1 attributes can add a field absent from KV markdown', async () => {
    ...
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:d1-only-field',
      attributes: { 'compliance-potential': 0.6 },
    })
    const res = await callTool('entity_manage', {
      action: 'analyze_utility',
      entity_id: 'character:d1-only-field',
      utility_vector: 'GASTRIC',
    })
  ...
  it('d1_attributes_used is false when no D1 attributes exist', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'analyze_utility',
      entity_id: 'character:kv-only-utility',
      utility_vector: 'GASTRIC',
    })
  ...
  it('d1_attributes_used is false on the grade-F empty-breakdown path', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'analyze_utility',
      entity_id: 'character:blank-utility',
      utility_vector: 'GASTRIC',
    })
...
describe('get_compatibility — D1 attribute priority (#410)', () => {
  ...
  it('uses D1 weight-1/weight-2 over KV and reports source', async () => {
    ...
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:compat-a',
      attributes: { 'weight-1': 0.05 },
    })
    await callTool('entity_manage', {
      action: 'set_attributes',
      entity_key: 'character:compat-b',
      attributes: { 'weight-2': 0.95 },
    })

    const res = await callTool('entity_manage', {
      action: 'get_compatibility',
      entity_a: 'character:compat-a',
      entity_b: 'character:compat-b',
  ...
  it('falls back to kv source when no D1 attributes exist', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'get_compatibility',
      entity_a: 'character:compat-kv-a',
      entity_b: 'character:compat-kv-b',
      interaction_type: 'hunt',
    })
