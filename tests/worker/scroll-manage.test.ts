...
describe('scroll_manage', () => {
  ...
  it('create+get_details+get_dc+use+identify round-trip', async () => {
    const scroll = await callTool('rpg', {
      sub: 'scroll',
      action: 'create',
    ...
    const unidentified = await callTool('rpg', {
      sub: 'scroll',
      action: 'get_details',
      id: scroll.scrollId,
    })
    ...
    const details = await callTool('rpg', {
      sub: 'scroll',
      action: 'get_details',
      id: scroll.scrollId,
    })
  ...
  it('use/identify/get_dc/get_details require id and 404 for an unknown scroll', async () => {
    ...
    const notFoundIdentify = await callTool('rpg', {
      sub: 'scroll',
      action: 'identify',
      id: 'nonexistent',
    })
    ...
    const notFoundDetails = await callTool('rpg', {
      sub: 'scroll',
      action: 'get_details',
      id: 'nonexistent',
    })
