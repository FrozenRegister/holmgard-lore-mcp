...
describe('handlePerceptionManage', () => {
  ...
  it('get_history returns assessments for observer', async () => {
    await handlePerceptionManage(db(), {
      action: 'assess',
      observerId: 'obs-6',
      targetId: 'room-6',
      rollValue: 15,
  ...
  it('predatorPerceptionModifier: core distance zone adds +5', () => {
    const { total, breakdown } = predatorPerceptionModifier({
      distanceZone: 'core',
      windDirection: 'none',
      yieldBleeding: false,
      yieldCookingOrFire: false,
    })
    expect(total).toBe(5)
  ...
  it('predatorPerceptionModifier: edge distance zone subtracts -3', () => {
    const { total } = predatorPerceptionModifier({
      distanceZone: 'edge',
      windDirection: 'none',
      yieldBleeding: false,
      yieldCookingOrFire: false,
  ...
  it('yieldStealthModifier: stealth mode modifiers (hiding +2, active +0, passive -5, rushed -8)', () => {
    expect(
      yieldStealthModifier({ stealthMode: 'hiding', isNight: false, partySize: 1 }).total,
    ).toBe(2)
    expect(
      yieldStealthModifier({ stealthMode: 'active', isNight: false, partySize: 1 }).total,
    ).toBe(0)
    expect(
      yieldStealthModifier({ stealthMode: 'passive', isNight: false, partySize: 1 }).total,
    ).toBe(-5)
    expect(
      yieldStealthModifier({ stealthMode: 'rushed', isNight: false, partySize: 1 }).total,
    ).toBe(-8)
  })
  ...
  it('yieldStealthModifier: night adds +2, party size subtracts -2 per member beyond 1', () => {
    expect(yieldStealthModifier({ stealthMode: 'active', isNight: true, partySize: 1 }).total).toBe(
      2,
    )
    expect(
      yieldStealthModifier({ stealthMode: 'active', isNight: false, partySize: 3 }).total,
    ).toBe(-4)
    expect(
      yieldStealthModifier({ stealthMode: 'active', isNight: true, partySize: 3 }).breakdown
        .partySize,
    ).toBe(-4)
  })
  ...
  it('stealthOutcomeFromMargin covers every outcome band', () => {
    ...
    expect(stealthOutcomeFromMargin(100)).toEqual({
      outcome: 'avoided_entirely',
      advantage: 'none',
    })
    ...
    expect(stealthOutcomeFromMargin(0)).toEqual({
      outcome: 'predator_searching',
      advantage: 'none',
    })
