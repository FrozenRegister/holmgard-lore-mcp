...
describe('plant_setup', () => {
  ...
  it('accepts setup_id as an alias for id', async () => {
    const res = await callTool('continuity_manage', {
      action: 'plant_setup',
      setup_id: 'church-ambush',
      description: 'Church courier spotted near the canal',
      payoff_type: 'threat',
    })
    expect(res.error).toBeUndefined()
...
describe('pay_off_setup', () => {
  it('marks a setup as paid', async () => {
    ...
    const res = await callTool('continuity_manage', {
      ...
      id: 'gun-on-wall',
      resolution: 'Fired in chapter 3',
      paid_in: 'scene:climax',
    })
    expect(res.error).toBeUndefined()
