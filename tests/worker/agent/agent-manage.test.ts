...
async function createCharacter(name: string): Promise<string> {
  ...
  await (mockEnv.RPG_DB as D1Database)
    .prepare(
...
describe('agent_manage tool', () => {
  ...
  describe('lifecycle', () => {
    ...
    it('lists agents with status filter', async () => {
      ...
      // pause one
      const updateResult = extractJson(
        await handleAgentManage(mockEnv, {
          action: 'update',
          agentId: create2.agentId,
          status: 'paused',
        }),
      )
      expect(updateResult.success).toBe(true)

      const paused = extractJson(
        await handleAgentManage(mockEnv, { action: 'list', status: 'paused' }),
      )
    ...
    it('updates agent fields and resumes a paused agent', async () => {
      const characterId = await createCharacter('Kara')
      const createResult = extractJson(
        await handleAgentManage(mockEnv, {
          action: 'create',
          characterId,
          provider: 'cloudflare',
  ...
  describe('prompt assembly', () => {
    ...
    it('sets and lists slices', async () => {
      ...
      const list = extractJson(
        await handleAgentManage(mockEnv, { action: 'list_slices', characterId }),
      )
  ...
  describe('mind state', () => {
    ...
    it('adds, lists, and removes secrets', async () => {
      ...
      const list = extractJson(
        await handleAgentManage(mockEnv, { action: 'list_secrets', characterId }),
      )
      expect(list.success).toBe(true)
      expect(list.count).toBe(1)
  ...
  describe('invocation', () => {
    ...
    it('invoke returns error when AI binding is missing', async () => {
      ...
      const characterId = await createCharacter('Kara')
      const createResult = extractJson(
        await handleAgentManage(noAiEnv, {
          action: 'create',
          characterId,
          provider: 'cloudflare',
          model: '@cf/meta/llama-3.1-8b-instruct',
    ...
    it('replay returns dry-mode info for a stored call', async () => {
      ...
      const invokeResult = extractJson(
        await handleAgentManage(mockEnv, {
          action: 'invoke',
          agentId,
          situation: 'go',
