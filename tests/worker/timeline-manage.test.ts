...
describe('handleTimelineManage', () => {
  ...
  it('get_events requires world_id', async () => {
    const body = JSON.parse(
      (await handleTimelineManage(db(), { action: 'get_events' })).content[0].text,
    )
  ...
  it('get_events returns empty list when no events', async () => {
    ...
    const body = JSON.parse(
      (await handleTimelineManage(db(), { action: 'get_events', world_id: 'w-ge-empty' }))
        .content[0].text,
    )
  ...
  it('get_events returns events for world', async () => {
    ...
    const body = JSON.parse(
      (await handleTimelineManage(db(), { action: 'get_events', world_id: 'w-ge' })).content[0]
        .text,
    )
  ...
  it('get_events accepts camelCase worldId as an alias for world_id (#336)', async () => {
    ...
    const body = JSON.parse(
      (await handleTimelineManage(db(), { action: 'get_events', worldId: 'w-ge-camel' })).content[0]
        .text,
    )
  ...
  it('get_events filters by thread', async () => {
    ...
    const body = JSON.parse(
      (
        await handleTimelineManage(db(), {
          action: 'get_events',
          world_id: 'w-ge-thread',
          thread: 'main',
        })
      ).content[0].text,
    )
  ...
  it('get_gap returns error for unknown events', async () => {
    const body = JSON.parse(
      (
        await handleTimelineManage(db(), {
          action: 'get_gap',
          before_event_id: 'no-evt',
          after_event_id: 'also-no',
        })
      ).content[0].text,
    )
    expect(body.error).toBe(true)
  ...
  it('get_gap returns gap with canonical events and characters between', async () => {
    ...
    const midId = await seedEvent('w-gap', {
      verb: 'middle',
      eventAt: '2184-06-01',
      canonical: true,
      entityId: 'c-gap',
  ...
  it('create_branch requires world_id, name, forked_at_event_id', async () => {
    const r = await handleTimelineManage(db(), {
  ...
  it('switch_branch marks branch as active', async () => {
    ...
    const row = (await env.RPG_DB.prepare('SELECT is_active FROM timeline_branches WHERE id = ?')
      .bind(branchId)
      .first()) as { is_active: number }
  ...
  it('merge_branch moves events from source to target branch', async () => {
    ...
    const body = JSON.parse(
      (
        await handleTimelineManage(db(), {
          action: 'merge_branch',
          source_branch_id: 'src-branch',
          target_branch_id: 'tgt-branch',
          event_ids: [evtId],
        })
      ).content[0].text,
    )
    ...
    const row = (await env.RPG_DB.prepare('SELECT branch_id FROM timeline_events WHERE id = ?')
      .bind(evtId)
      .first()) as { branch_id: string }
...
describe('handleTimeManage — get_timeline and jump_to', () => {
  ...
  it('get_timeline requires world_id', async () => {
    const body = JSON.parse(
      (await handleTimeManage(db(), { action: 'get_timeline' })).content[0].text,
    )
    expect(body.error).toBe(true)
  })
  ...
  it('get_timeline returns all events for world ordered by date', async () => {
    ...
    const body = JSON.parse(
      (await handleTimeManage(db(), { action: 'get_timeline', world_id: 'w-tl' })).content[0].text,
    )
    expect(body.success).toBe(true)
  ...
  it('get_timeline filters by thread', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .run()
    const body = JSON.parse(
      (await handleTimeManage(db(), { action: 'get_timeline', world_id: 'w-tl3', thread: 'alpha' }))
        .content[0].text,
    )
    expect(body.count).toBe(1)
  })
  ...
  it('jump_to returns gap with null events when no canonical events exist', async () => {
    ...
    const body = JSON.parse(
      (await handleTimeManage(db(), { action: 'jump_to', world_id: 'w-jump', date: '2184-07-15' }))
        .content[0].text,
    )
    expect(body.success).toBe(true)
  ...
  it('jump_to returns bracketing canonical events', async () => {
    ...
    const body = JSON.parse(
      (await handleTimeManage(db(), { action: 'jump_to', world_id: 'w-jump2', date: '2184-07-15' }))
        .content[0].text,
    )
  ...
  it('jump_to play mode adds constraint when after_event exists', async () => {
    ...
    await seedEvent('w-jump3', {
      verb: 'constraint-trigger',
      eventAt: '2184-12-01',
      canonical: true,
    })
    const body = JSON.parse(
