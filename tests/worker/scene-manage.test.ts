...
describe('scene_manage tool', () => {
  ...
  it('get_conflict_type returns null for a scene with no conflict type set', async () => {
    ...
    const created = await callTool('rpg', {
      sub: 'scene',
      action: 'create',
      worldId: 'world:conflict',
      title: 'Neutral',
      narration: 'Nothing tagged yet.',
    })
    const r = await callTool('rpg', {
      sub: 'scene',
      action: 'get_conflict_type',
      id: created.sceneId,
    })
    expect(r.success).toBe(true)
  ...
  it('set_conflict_type tags a scene as physical and get_conflict_type reflects it', async () => {
    ...
    const created = await callTool('rpg', {
      ...
      worldId: 'world:conflict',
      title: 'Courtyard',
      narration: 'Predator sighted.',
    })
    const setRes = await callTool('rpg', {
      sub: 'scene',
    ...
    const getRes = await callTool('rpg', {
      sub: 'scene',
      action: 'get_conflict_type',
      id: created.sceneId,
    })
  ...
  it('set_conflict_type can clear a scene back to null', async () => {
    ...
    const clearRes = await callTool('rpg', {
      sub: 'scene',
      action: 'set_conflict_type',
      id: created.sceneId,
      conflictTypeId: null,
  ...
  it('set_conflict_type rejects an unknown conflict type id', async () => {
    ...
    const created = await callTool('rpg', {
      ...
      worldId: 'world:conflict',
      title: 'Odd',
      narration: 'A weird scene.',
    })
    const r = await callTool('rpg', {
      sub: 'scene',
      action: 'set_conflict_type',
      id: created.sceneId,
      conflictTypeId: 'nonexistent-type',
    })
    expect(r.error).toBe(true)
  })
