...
describe('handleWorldMap', () => {
  ...
  it('patch defaults waterDepth to null when omitted', async () => {
    ...
    await handleWorldMap(db(), {
      action: 'patch',
      worldId: WORLD,
      hexes: [{ q: 20, r: 20, biome: 'grass' }],
    })
    const row = (await env.RPG_DB.prepare(
      'SELECT water_depth FROM hexes WHERE world_id = ? AND q = ? AND r = ?',
    )
      .bind(WORLD, 20, 20)
      .first()) as any
  ...
  it('patch stores an explicit waterDepth', async () => {
    ...
    await handleWorldMap(db(), {
      action: 'patch',
      worldId: WORLD,
      hexes: [{ q: 21, r: 21, biome: 'grass', waterDepth: 0.9 }],
    })
    const row = (await env.RPG_DB.prepare(
      'SELECT water_depth FROM hexes WHERE world_id = ? AND q = ? AND r = ?',
    )
  ...
  it('patch can update waterDepth back to null on re-patch', async () => {
    ...
    await handleWorldMap(db(), {
      action: 'patch',
      worldId: WORLD,
      hexes: [{ q: 22, r: 22, biome: 'grass', waterDepth: 1.5 }],
    })
    await handleWorldMap(db(), {
      action: 'patch',
      worldId: WORLD,
      hexes: [{ q: 22, r: 22, biome: 'grass' }],
    })
    const row = (await env.RPG_DB.prepare(
      'SELECT water_depth FROM hexes WHERE world_id = ? AND q = ? AND r = ?',
    )
  ...
  it('batch stores explicit waterDepth per hex (#431)', async () => {
    ...
    await handleWorldMap(db(), {
      ...
      worldId: WORLD,
      hexes: [
        { q: 30, r: 30, biome: 'grass', waterDepth: 0.4 },
        { q: 31, r: 30, biome: 'grass' },
      ],
    })
    const rows = (await env.RPG_DB.prepare(
      'SELECT q, water_depth FROM hexes WHERE world_id = ? AND r = 30 ORDER BY q',
    )
      .bind(WORLD)
      .all()) as any
  ...
  it('batch flags unknown biomes as per-hex errors but still writes valid hexes once a registry exists (#274)', async () => {
    ...
    const r = await handleWorldMap(db(), {
      ...
      worldId: WORLD,
      hexes: [
        { q: 0, r: 0, biome: 'limestone_karst' },
        { q: 1, r: 0, biome: 'not_a_real_biome' },
      ],
    })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
    expect(body.hexesInserted).toBe(1)
    expect(body.errors).toEqual([
      { index: 1, q: 1, r: 0, biome: 'not_a_real_biome', error: 'Unknown biome' },
    ])
    const fetched = JSON.parse(
      (
        await handleWorldMap(db(), {
  ...
  it('suggest_poi with zoneType/predatorRef but no shape stores metadata without a queryable zone', async () => {
    ...
    const zones = JSON.parse(
      (await handleWorldMap(db(), { action: 'list_zones', worldId: WORLD })).content[0].text,
    )
  ...
  it('update_poi rejects a polygon with fewer than 3 points', async () => {
    ...
    const r = await handleWorldMap(db(), {
      action: 'update_poi',
      structureId: created.landmarkId,
      polygon: [[0, 0]],
    })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  ...
  it('query_zone sets inPerimeter true for a point within a ring zone band', async () => {
    ...
    const inside = await handleWorldMap(db(), {
      action: 'query_zone',
      worldId: WORLD,
      q: 70,
      r: 50,
    ...
    const outside = await handleWorldMap(db(), {
      action: 'query_zone',
      worldId: WORLD,
      q: 50,
      r: 50,
  ...
  it('preview overlays a perimeter ring glyph and leaves cells outside the band untouched', async () => {
    ...
    await handleWorldMap(db(), {
      ...
      worldId: WORLD,
      hexes: [
        { q: 70, r: 50, biome: 'grass' },
        { q: 50, r: 50, biome: 'grass' },
      ],
    })
    await handleWorldMap(db(), {
      action: 'suggest_poi',
    ...
    const inBand = await handleWorldMap(db(), {
      action: 'preview',
      worldId: WORLD,
      q: 70,
      r: 50,
    ...
    const outOfBand = await handleWorldMap(db(), {
      action: 'preview',
      worldId: WORLD,
      q: 50,
      r: 50,
  ...
  it('render_svg falls back to a default gray for a biome with no known color', async () => {
    ...
    await handleWorldMap(db(), {
      action: 'patch',
      worldId: WORLD,
      hexes: [{ q: 0, r: 0, biome: 'totally_unknown_biome' }],
    })
    const r = await handleWorldMap(db(), {
      action: 'render_svg',
      worldId: WORLD,
  ...
  it('distance computes straightLineKm and a terrain-weighted estimatedTravelDays once calibrated', async () => {
    ...
    await handleWorldMap(db(), {
      ...
      worldId: WORLD,
      hexes: [
        { q: 1, r: 0, biome: 'grass_430' },
        { q: 2, r: 0, biome: 'forest_430' },
        { q: 3, r: 0, biome: 'grass_430' },
      ],
    })
    const r = await handleWorldMap(db(), {
      action: 'distance',
