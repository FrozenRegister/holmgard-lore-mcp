...
describe('Combat depth', () => {
  ...
  it('legendary_action consumes remaining legendary actions and rejects when exhausted', async () => {
    ...
    await env.RPG_DB.prepare('UPDATE characters SET legendary_actions = 3 WHERE id = ?')
      .bind(char.characterId)
      .run()

    const r1 = await callTool('rpg', {
      sub: 'combat',
      action: 'legendary_action',
      characterId: char.characterId,
      actionName: 'Tail Attack',
    })
    ...
    const exhausted = await callTool('rpg', {
      sub: 'combat',
      action: 'legendary_action',
      characterId: char.characterId,
    })
  ...
  it('lair_action triggers for a creature with has_lair_actions and logs to an encounter', async () => {
    ...
    await env.RPG_DB.prepare('UPDATE characters SET has_lair_actions = 1 WHERE id = ?')
      .bind(char.characterId)
      .run()
    ...
    const r = await callTool('rpg', {
      ...
      actionName: 'The floor becomes difficult terrain',
    })
    expect(r.success).toBe(true)
  ...
  it('lair_action rejects a creature without has_lair_actions and requires characterId', async () => {
    ...
    const noLair = await callTool('rpg', {
      sub: 'combat',
      action: 'lair_action',
      characterId: char.characterId,
    })
  ...
  it('help sets the helped condition on the target(s)', async () => {
    ...
    await callTool('rpg', {
      sub: 'combat_action',
      action: 'help',
      actorId: helper.characterId,
      targetIds: [ally.characterId],
    })
    const got = await callTool('rpg', {
      sub: 'character',
      action: 'get',
  ...
  it('ready sets the readying condition and logs the trigger', async () => {
    ...
    const r = await callTool('rpg', {
      ...
      encounterId: enc.encounterId,
      actorId: char.characterId,
      description: 'Attack the first enemy that enters the doorway',
    })
    expect(r.success).toBe(true)
  ...
  it('apply_damage halves damage of a resisted type', async () => {
    ...
    await env.RPG_DB.prepare('UPDATE characters SET resistances = ? WHERE id = ?')
      .bind(JSON.stringify(['fire']), char.characterId)
      .run()
    const r = await callTool('rpg', {
      sub: 'combat_action',
      action: 'apply_damage',
  ...
  it('apply_damage zeroes damage of an immune type', async () => {
    ...
    await env.RPG_DB.prepare('UPDATE characters SET immunities = ? WHERE id = ?')
      .bind(JSON.stringify(['poison']), char.characterId)
      .run()
    const r = await callTool('rpg', {
      sub: 'combat_action',
      action: 'apply_damage',
  ...
  it('apply_damage doubles damage of a vulnerable type', async () => {
    const char = await callTool('rpg', {
      sub: 'character',
      action: 'create',
      name: 'Cold Vulnerable',
      hp: 20,
      maxHp: 20,
    })
    await env.RPG_DB.prepare('UPDATE characters SET vulnerabilities = ? WHERE id = ?')
      .bind(JSON.stringify(['cold']), char.characterId)
      .run()
    const r = await callTool('rpg', {
      sub: 'combat_action',
      action: 'apply_damage',
  ...
  it('apply_damage flags a concentration check when a concentrating character takes damage', async () => {
    ...
    await env.RPG_DB.prepare('UPDATE characters SET concentrating_on = ? WHERE id = ?')
      .bind('Bless', char.characterId)
      .run()
    const r = await callTool('rpg', {
      sub: 'combat_action',
      action: 'apply_damage',
  ...
  it('apply_damage aimed at a passenger consciousness lands on the shared host body HP, not a separate field', async () => {
    ...
    const passenger = await callTool('rpg', {
      sub: 'character',
      action: 'create',
      name: 'Passenger Consciousness',
      hp: 30,
      maxHp: 30,
      hostBodyId: host.characterId,
      active: true,
    })
    ...
    const r = await callTool('rpg', {
      sub: 'combat_action',
      action: 'apply_damage',
      targetIds: [passenger.characterId],
      damage: 12,
    })
    expect(r.hpChanges[passenger.characterId]).toBe(-12)

    const hostAfter = await callTool('rpg', {
      sub: 'character',
      action: 'get',
      characterId: host.characterId,
    })
    const passengerAfter = await callTool('rpg', {
      sub: 'character',
      action: 'get',
      characterId: passenger.characterId,
    })
  ...
  it('heal aimed at a passenger consciousness restores the shared host body HP, not a separate field', async () => {
    ...
    const passenger = await callTool('rpg', {
      sub: 'character',
      action: 'create',
      name: 'Passenger Consciousness 2',
      hp: 10,
      maxHp: 30,
      hostBodyId: host.characterId,
      active: false,
  ...
  it('apply_damage/heal against a nonexistent targetId are a harmless no-op', async () => {
    const dmgRes = await callTool('rpg', {
      sub: 'combat_action',
      action: 'apply_damage',
      targetIds: ['no-such-character'],
      damage: 5,
    })
    expect(dmgRes.hpChanges).toEqual({})
    const healRes = await callTool('rpg', {
      sub: 'combat_action',
      action: 'heal',
      targetIds: ['no-such-character'],
      healAmount: 5,
