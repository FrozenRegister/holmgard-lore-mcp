...
describe('advance_state_stage', () => {
  it('increments State-Stage and writes back', async () => {
    ...
    const res = await callTool('entity_manage', {
      ...
      entity_key: 'character:caterpillar',
    })
  ...
  it('advances from loose plain-colon format (no bold markers)', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:loose-stage',
    })
    expect(res.result.advanced).toBe(true)
  ...
  it('parses stage from embedded Stage-N-of-M narrative status and updates in-place', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:subject-alpha',
    })
    expect(res.result.advanced).toBe(true)
...
describe('advance_state_stage — D1 dissolution_stage mirror (#411)', () => {
  ...
  it('mirrors the new State-Stage into dissolution_stage for a staged character resolved by name', async () => {
    ...
    await seedKV(
      'character:mirror-test-subject',
      '**State-Stage:** 3\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:mirror-test-subject',
    })
    expect(res.result.advanced).toBe(true)
    expect(res.result.new_stage).toBe(4)
    expect(res.result.d1_mirrored).toBe(true)

    const row = (await env.RPG_DB.prepare('SELECT dissolution_stage FROM characters WHERE id = ?')
      .bind(characterId)
      .first()) as { dissolution_stage: number }
  ...
  it('does not mirror for an entity with no matching D1 character', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:no-d1-row',
    })
    expect(res.result.advanced).toBe(true)
  ...
  it('does not mirror for a D1 character whose death_mode is still instant', async () => {
    ...
    await env.RPG_DB.prepare("UPDATE characters SET death_mode = 'instant' WHERE name = ?")
      .bind('Instant Mode Subject')
      .run()
    await seedKV(
      'character:instant-mode-subject',
      '**State-Stage:** 1\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:instant-mode-subject',
    })
    expect(res.result.advanced).toBe(true)
    expect(res.result.d1_mirrored).toBe(false)
  })
...
describe('advance_state_stage — terminal-stage hook (#420)', () => {
  ...
  async function seedStagedCharacter(
    name: string,
    dissolutionStage: number,
    opts: { dissolutionTerminal?: string; worldId?: string } = {},
  ): Promise<string> {
    const id = crypto.randomUUID()
    const now = new Date().toISOString()
    await env.RPG_DB.prepare(
  ...
  it('marks Terminal-Status with a generic fallback for a pure-KV entity with no D1 link', async () => {
    await seedKV(
      'character:pure-kv-terminal',
      '**State-Stage:** 4\n**State-Total:** 5\n**Stage-Timer:** 1',
    )
    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:pure-kv-terminal',
    })
    expect(res.result.is_terminal).toBe(true)
    expect(res.result.terminal_timeline_event_id).toBeNull()

    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:pure-kv-terminal',
    })
  ...
  it("uses the linked D1 character's dissolution_terminal free text for Terminal-Status", async () => {
    await seedStagedCharacter('Descriptor Test Subject', 4, {
      dissolutionTerminal: 'consumed by the Slime-Girl distributed intelligence',
    })
    await seedKV(
      'character:descriptor-test-subject',
      '**State-Stage:** 4\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:descriptor-test-subject',
    })
    expect(res.result.is_terminal).toBe(true)

    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:descriptor-test-subject',
    })
    expect(lore.result.text).toContain(
      '**Terminal-Status:** consumed by the Slime-Girl distributed intelligence',
    )
  })
  ...
  it('logs a discoverable timeline_events row when the D1 character has a world_id', async () => {
    const world = await callTool('rpg', {
      sub: 'world',
      action: 'create',
      name: 'Terminal Hook World',
      theme: 'fantasy',
    })
    const worldPayload = JSON.parse(world.result.content[0].text) as { worldId: string }
    const worldId = worldPayload.worldId
    const characterId = await seedStagedCharacter('World Linked Subject', 4, {
      dissolutionTerminal: 'mycelium-integrated',
      worldId,
    })
    await seedKV(
      'character:world-linked-subject',
      '**State-Stage:** 4\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:world-linked-subject',
    })
    expect(res.result.is_terminal).toBe(true)
    ...
    const row = (await env.RPG_DB.prepare('SELECT * FROM timeline_events WHERE id = ?')
      .bind(res.result.terminal_timeline_event_id)
      .first()) as { world_id: string; verb: string; entity_id: string; detail: string } | null
  ...
  it('does not log a timeline_events row when the D1 character has no world_id', async () => {
    ...
    await seedKV(
      'character:no-world-subject',
      '**State-Stage:** 4\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:no-world-subject',
    })
    expect(res.result.is_terminal).toBe(true)
    expect(res.result.terminal_timeline_event_id).toBeNull()
  })
  ...
  it('does not fire the hook on a non-terminal advance', async () => {
    await seedKV(
      'character:mid-stage-subject',
      '**State-Stage:** 1\n**State-Total:** 5\n**Stage-Timer:** 3',
    )
    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:mid-stage-subject',
    })
    expect(res.result.is_terminal).toBe(false)
    ...
    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:mid-stage-subject',
    })
...
describe('advance_state_stage — dissolution primitives (#441)', () => {
  ...
  async function seedStagedCharacter(
    name: string,
    dissolutionStage: number,
    opts: { dissolutionTerminal?: string; hp?: number; worldId?: string } = {},
  ): Promise<string> {
    const id = crypto.randomUUID()
    const now = new Date().toISOString()
    await env.RPG_DB.prepare(
  ...
  it('writes Dissolution-Scent field on stage 1 advance', async () => {
    ...
    await seedKV(
      'character:scent-test-subject',
      '**State-Stage:** 0\n**State-Total:** 5\n**Stage-Timer:** 3',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:scent-test-subject',
    })
    expect(res.result.advanced).toBe(true)
    ...
    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:scent-test-subject',
    })
  ...
  it('writes Movement-Locked and Communication-Penalty flags on stage 2+', async () => {
    ...
    await seedKV(
      'character:mechanical-test-subject',
      '**State-Stage:** 1\n**State-Total:** 5\n**Stage-Timer:** 3',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:mechanical-test-subject',
    })
    expect(res.result.advanced).toBe(true)

    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:mechanical-test-subject',
    })
  ...
  it('writes Dissolution-Conversion and Dissolution-Conversion-Label on terminal stage', async () => {
    await seedStagedCharacter('Conversion Test Subject', 4, {
      dissolutionTerminal: 'consumed by the mycelium network',
    })
    await seedKV(
      'character:conversion-test-subject',
      '**State-Stage:** 4\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:conversion-test-subject',
    })
    expect(res.result.advanced).toBe(true)
    expect(res.result.is_terminal).toBe(true)
    ...
    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:conversion-test-subject',
    })
  ...
  it('applies HP drain to D1 character atomically via batch', async () => {
    ...
    await seedKV(
      'character:hp-drain-subject',
      '**State-Stage:** 0\n**State-Total:** 5\n**Stage-Timer:** 3',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:hp-drain-subject',
    })
    expect(res.result.advanced).toBe(true)
    // d1_hp_drained is true when a D1 character is linked and batch executes
    expect(res.result.d1_hp_drained).toBe(true)

    const row = (await env.RPG_DB.prepare('SELECT hp FROM characters WHERE id = ?')
      .bind(characterId)
      .first()) as { hp: number }
  ...
  it('applies HP drain on stage 3+ where hp_drain_per_tick > 0', async () => {
    ...
    await seedKV(
      'character:hp-drain-stage-3-subject',
      '**State-Stage:** 2\n**State-Total:** 5\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:hp-drain-stage-3-subject',
    })
    expect(res.result.advanced).toBe(true)
    // d1_hp_drained is true when a D1 character is linked and batch executes
    expect(res.result.d1_hp_drained).toBe(true)
  ...
  it('does not write dissolution fields for non-staged characters', async () => {
    ...
    await seedKV(
      'character:non-staged-subject',
      '**State-Stage:** 0\n**State-Total:** 5\n**Stage-Timer:** 3',
    )
    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:non-staged-subject',
    })
    expect(res.result.advanced).toBe(true)
    ...
    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:non-staged-subject',
    })
...
describe('resolveDissolutionConfig — KV lookup order (#472)', () => {
  ...
  it('prefers a per-instance config over the seeded default when dissolutionId matches', async () => {
    const seededDefault: SerializedConfig = {
      version: 2,
      stages: DEFAULT_DISSOLUTION_CONFIG.stages,
      terminalStage: 5,
      terminalConversions: {},
      migrated_at: new Date().toISOString(),
    }
...
describe('advance_state_stage — config-driven dissolution beyond stage 5 (#471, #472)', () => {
  ...
  async function seedStagedCharacter(
    name: string,
    dissolutionStage: number,
    opts: { dissolutionId?: string; hp?: number } = {},
  ): Promise<string> {
    const id = crypto.randomUUID()
    const now = new Date().toISOString()
    await env.RPG_DB.prepare(
  ...
  it("applies real sensory/mechanical mutations at stage 6, 7, and 8 for a character with an 8-stage per-instance config (previously silently no-op'd)", async () => {
    ...
    const characterId = await seedStagedCharacter('Eight Stage Subject', 5, {
      dissolutionId,
      hp: 20,
    })
    await seedKV(
      'character:eight-stage-subject',
      '**State-Stage:** 5\n**State-Total:** 8\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:eight-stage-subject',
    })
    expect(res.result.advanced).toBe(true)
    ...
    const lore = await callTool('lore_manage', {
      action: 'get',
      query: 'character:eight-stage-subject',
    })
  ...
  it("reaches the custom config's terminal at stage 8, not stage 5", async () => {
    ...
    await seedKV(
      'character:eight-stage-terminal-subject',
      '**State-Stage:** 7\n**State-Total:** 8\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:eight-stage-terminal-subject',
    })
    expect(res.result.advanced).toBe(true)
    expect(res.result.is_terminal).toBe(true)
    expect(res.result.dissolution?.scent_applied).toBe(true)
  ...
  it("surfaces stage_exceeds_config instead of silently no-oping when the resolved config is shorter than the entity's own tracked total", async () => {
    ...
    await seedKV(
      'character:mismatched-config-subject',
      '**State-Stage:** 5\n**State-Total:** 8\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:mismatched-config-subject',
    })
    expect(res.result.advanced).toBe(true)
    expect(res.result.is_terminal).toBe(false)
  ...
  it("neither writes mutations nor sets stage_exceeds_config when the config-exceeding stage happens to also be the entity's own terminal stage", async () => {
    ...
    await seedKV(
      'character:coincident-terminal-subject',
      '**State-Stage:** 5\n**State-Total:** 6\n**Stage-Timer:** 1',
    )

    const res = await callTool('entity_manage', {
      action: 'advance_stage',
      entity_key: 'character:coincident-terminal-subject',
    })
    expect(res.result.advanced).toBe(true)
    expect(res.result.is_terminal).toBe(true)
...
describe('generate_entity', () => {
  ...
  it('returns error for missing archetype', async () => {
    const res = await callTool('entity_manage', {
      action: 'generate',
      archetype_key: 'archetype:no-such',
    })
...
describe('roll_encounter', () => {
  ...
  it('returns rolled=false when no Encounter-Table', async () => {
    ...
    const res = await callTool('entity_manage', {
      action: 'roll_encounter',
      location_key: 'location:empty-field',
    })
