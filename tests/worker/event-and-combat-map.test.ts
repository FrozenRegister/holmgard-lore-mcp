...
describe('event_manage', () => {
  ...
  it('emit+poll+ack round-trip', async () => {
    const emitted = await callTool('rpg', {
      sub: 'event',
      action: 'emit',
      eventType: 'combat_update',
      payload: { encounterId: 'enc-1', round: 2 },
      sourceType: 'combat',
    })
    expect(emitted.success).toBe(true)
    expect(emitted.eventId).toBeTruthy()

    const polled = await callTool('rpg', {
      sub: 'event',
      action: 'poll',
      eventType: 'combat_update',
    })
    ...
    const polledAfterAck = await callTool('rpg', {
      sub: 'event',
      action: 'poll',
      eventType: 'combat_update',
    })
  ...
  it('ack accepts a batch of ids', async () => {
    const e1 = await callTool('rpg', {
      sub: 'event',
      action: 'emit',
      eventType: 'system',
      payload: { msg: 'a' },
    })
    const e2 = await callTool('rpg', {
      sub: 'event',
      action: 'emit',
      eventType: 'system',
      payload: { msg: 'b' },
    })
    const acked = await callTool('rpg', {
      sub: 'event',
      action: 'ack',
      ids: [e1.eventId, e2.eventId],
    })
  ...
  it('emit accepts custom event types (not in the closed enum)', async () => {
    const custom = await callTool('rpg', {
      sub: 'event',
      action: 'emit',
      eventType: 'crate_drop',
      payload: { location: 'hex-1' },
    })
  ...
  it('emit accepts production event types (perimeter_contraction, audience_vote, etc)', async () => {
    const perimeter = await callTool('rpg', {
      sub: 'event',
      action: 'emit',
      eventType: 'perimeter_contraction',
      payload: {},
    })
    expect(perimeter.success).toBe(true)
    const audience = await callTool('rpg', {
      sub: 'event',
      action: 'emit',
...
describe('combat_map: terrain and AoE', () => {
  ...
  it('calculate_aoe requires origin, and target for a line shape', async () => {
    ...
    const noTarget = await callTool('rpg', {
      sub: 'combat_map',
      action: 'calculate_aoe',
      origin: { x: 0, y: 0 },
      shape: 'line',
