// Tests for scene_manage registration via registerTool() (#545)
import { describe } from './support/helpers'
import { SELF, env } from 'cloudflare:test'
import { expect, it } from 'vitest'
import { getToolHandler } from '../../src/tools/register'

describe('scene_manage via registerTool() (Phase 4 #545)', () => {
  ...
  it('getToolHandler resolves scene_manage', () => {
    const handler = getToolHandler('scene_manage')
    expect(handler).toBeDefined()
    expect(typeof handler).toBe('function')
  })

  it('handles get_history action via MCP endpoint', async () => {
    const r = await callTool('scene_manage', {
      action: 'get_history',
  ...
  it('rejects missing action via MCP endpoint', async () => {
    const r = await callTool('scene_manage', { scene_key: 'test' })
    expect(r.error || r.success === false).toBeTruthy()
  })

  it('rejects invalid action via MCP endpoint', async () => {
    const r = await callTool('scene_manage', { action: 'invalid_action_xyz' })
    expect(r.error || r.success === false).toBeTruthy()
  })
})
