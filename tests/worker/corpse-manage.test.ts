...
describe('handleCorpseManage', () => {
  ...
  it('get returns corpse with loot', async () => {
    const c = await handleCorpseManage(db(), {
      action: 'create',
      characterId: 'c2',
      characterName: 'Orc',
    })
  ...
  it('register accepts an explicit deathAt override', async () => {
    const r = await handleCorpseManage(db(), {
      action: 'register',
      characterId: 'adebayo',
      characterName: 'Adebayo',
      deathAt: '2026-01-01T00:00:00.000Z',
    })
    const body = JSON.parse(r.content[0].text)
  ...
  it('decompose computes every stage boundary from hoursSinceDeath', async () => {
    const reg = await handleCorpseManage(db(), {
      action: 'register',
      characterId: 'yune',
      characterName: 'Yune',
    })
  ...
  it('decompose sets isLandmark true at advanced_decay and skeletal, false before', async () => {
    ...
    const { corpseId } = JSON.parse(reg.content[0].text)
    const early = await handleCorpseManage(db(), {
      action: 'decompose',
      id: corpseId,
      hoursSinceDeath: 24,
    })
    expect(JSON.parse(early.content[0].text).isLandmark).toBe(false)
    const advanced = await handleCorpseManage(db(), {
      action: 'decompose',
      id: corpseId,
      hoursSinceDeath: 200,
    })
  ...
  it('decompose falls back to computing elapsed time from death_at when hoursSinceDeath is omitted', async () => {
    const reg = await handleCorpseManage(db(), {
      ...
      characterId: 'yune',
      characterName: 'Yune',
      deathAt: new Date(Date.now() - 10 * 3_600_000).toISOString(),
    })
    const { corpseId } = JSON.parse(reg.content[0].text)
  ...
  async function registerWithItems(
    worldId: string,
    characterId: string,
  ...
  it('loot_corpse at fresh stage returns the full inventory snapshot', async () => {
    const corpseId = await registerWithItems(WORLD, 'yune', [
      { itemName: 'Field Dressing', category: 'medical' },
      { itemName: '50ft Rope', category: 'tools' },
    ])
    const r = await handleCorpseManage(db(), {
      action: 'loot_corpse',
      id: corpseId,
  ...
  it('loot_corpse at bloat stage applies partial (50%) contamination filtering', async () => {
    const corpseId = await registerWithItems(WORLD, 'nia', [
      { itemName: 'Field Dressing', category: 'medical' },
      { itemName: '50ft Rope', category: 'tools' },
    ])
    await handleCorpseManage(db(), { action: 'decompose', id: corpseId, hoursSinceDeath: 30 })
  ...
  it('loot_corpse at active_decay stage applies heavier (20%) contamination filtering', async () => {
    const corpseId = await registerWithItems(WORLD, 'adebayo', [
      { itemName: 'Field Dressing', category: 'medical' },
    ])
  ...
  it('loot_corpse at advanced_decay stage only returns metal (tools/weapon) items', async () => {
    const corpseId = await registerWithItems(WORLD, 'kat', [
      { itemName: 'Field Dressing', category: 'medical' },
      { itemName: 'Survival Knife', category: 'weapon' },
      { itemName: '50ft Rope', category: 'tools' },
    ])
    ...
    const body = JSON.parse(r.content[0].text)
    expect(body.itemsLooted.map((i: { itemName: string }) => i.itemName).sort()).toEqual([
      '50ft Rope',
      'Survival Knife',
    ])
  })
  ...
  it('loot_corpse at skeletal stage returns metal items plus synthetic Bone Fragments', async () => {
    const corpseId = await registerWithItems(WORLD, 'skeletal-1', [
      { itemName: 'Survival Knife', category: 'weapon' },
    ])
  ...
  it('loot_corpse removes looted items from the persisted snapshot', async () => {
    const corpseId = await registerWithItems(WORLD, 'persisted', [
      { itemName: 'Field Dressing', category: 'medical' },
    ])
    await handleCorpseManage(db(), {
      action: 'loot_corpse',
      id: corpseId,
      looterCharacterId: 'looter',
      rollValue: 20,
  ...
  it('loot_corpse reports disease exposure when the disease roll succeeds, and null when it fails', async () => {
    ...
    expect(JSON.parse(exposed.content[0].text).diseaseExposure).toEqual({
  ...
  it('loot_corpse reports null disease exposure at fresh stage (0% disease risk)', async () => {
    const corpseId = await registerWithItems(WORLD, 'clean', [
      { itemName: 'Field Dressing', category: 'medical' },
    ])
    const r = await handleCorpseManage(db(), {
      action: 'loot_corpse',
  ...
  it("psychological_impact matches the issue's given DCs: fresh/stranger=10, fresh/party_member=16, bloat/stranger=12, active_decay/stranger=14", async () => {
    ...
    const fresh = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      rollValue: 20,
    ...
    const freshParty = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
    ...
    await handleCorpseManage(db(), { action: 'decompose', id: corpseId, hoursSinceDeath: 30 })
    const bloat = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      rollValue: 20,
    ...
    await handleCorpseManage(db(), { action: 'decompose', id: corpseId, hoursSinceDeath: 100 })
    const active = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      rollValue: 20,
  ...
  it('psychological_impact applies flat DC overrides for betrayed_them (18) and saved_them (20)', async () => {
    ...
    const { corpseId } = JSON.parse(reg.content[0].text)
    const betrayed = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      relationship: 'betrayed_them',
      rollValue: 20,
    })
    expect(JSON.parse(betrayed.content[0].text).dc).toBe(18)
    const saved = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      relationship: 'saved_them',
      rollValue: 20,
    })
  ...
  it('psychological_impact reports shaken (fail by 1-4), disturbed (5-9), and traumatized (10+)', async () => {
    ...
    const shaken = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      rollValue: 2,
    ...
    const disturbed = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      rollValue: 2,
    ...
    const traumatized = await handleCorpseManage(db(), {
      action: 'psychological_impact',
      id: corpseId,
      observerCharacterId: 'obs',
      rollValue: 2,
  ...
  it('tickAllCorpseDecomposition ticks every non-recovered corpse with a death_at, and skips recovered ones', async () => {
    await createWorld()
    const reg1 = await handleCorpseManage(db(), {
      action: 'register',
      characterId: 'a',
      characterName: 'a',
      worldId: WORLD,
      deathAt: new Date(Date.now() - 30 * 3_600_000).toISOString(),
    })
    const corpseId1 = JSON.parse(reg1.content[0].text).corpseId
    const reg2 = await handleCorpseManage(db(), {
      action: 'register',
      characterId: 'b',
      characterName: 'b',
      worldId: WORLD,
