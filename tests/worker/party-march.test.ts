...
describe('handlePartyManage — waypoint march (#328)', () => {
  ...
  async function setDistance(fromId: string, toId: string, distanceKm: number | null) {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind(
        ...
        toId,
        distanceKm,
        distanceKm === null ? 'osrm_foot_v1_no_route' : 'osrm_foot_v1',
        now,
      )
      .run()
  }
  ...
  it('begin_march errors when the target waypoint does not exist', async () => {
    ...
    const r = await handlePartyManage(db(), {
      action: 'begin_march',
      partyId,
      toWaypointName: 'Nowhere',
      fromWaypointName: 'Nowhere',
    })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  ...
  it('begin_march returns a structured blocked response and does not mutate party state when not precomputed', async () => {
    ...
    const party = (await env.RPG_DB.prepare(
      'SELECT travel_status, travel_target_waypoint_id FROM parties WHERE id = ?',
    )
      .bind(partyId)
      .first()) as any
  ...
  it("begin_march starts a march using the party's current_waypoint_id when fromWaypoint is omitted", async () => {
    ...
    await setDistance(visbyId, romaId, 18.26)
    const partyId = await createParty()
    await env.RPG_DB.prepare('UPDATE parties SET current_waypoint_id = ? WHERE id = ?')
      .bind(visbyId, partyId)
      .run()
    ...
    const party = (await env.RPG_DB.prepare(
      'SELECT travel_status, travel_target_waypoint_id, travel_remaining_km FROM parties WHERE id = ?',
    )
      .bind(partyId)
      .first()) as any
  ...
  it('begin_march accepts an explicit pace override', async () => {
    ...
    const party = (await env.RPG_DB.prepare(
      'SELECT travel_pace_km_per_day FROM parties WHERE id = ?',
    )
      .bind(partyId)
      .first()) as any
  ...
  it('tickAllPartiesMarch resolves exact and overshoot arrival, discarding leftover budget', async () => {
    ...
    expect(results).toEqual([
      { partyId, status: 'arrived', remainingKm: 0, arrivedAtWaypointId: romaId },
    ])

    const party = (await env.RPG_DB.prepare(
      'SELECT current_waypoint_id, travel_target_waypoint_id, travel_remaining_km, travel_status, current_hex_q, current_hex_r FROM parties WHERE id = ?',
    )
      .bind(partyId)
      .first()) as any
  ...
  it('tickAllPartiesMarch keeps a party marching when distance remains after the tick', async () => {
    ...
    const party = (await env.RPG_DB.prepare(
      'SELECT travel_status, travel_remaining_km FROM parties WHERE id = ?',
    )
      .bind(partyId)
      .first()) as any
    expect(party.travel_status).toBe('marching')
  ...
  it('tickAllPartiesMarch leaves non-marching parties untouched', async () => {
    ...
    const results = await tickAllPartiesMarch(env.RPG_DB, WORLD, 1)
    expect(results).toEqual([])
    const party = (await env.RPG_DB.prepare('SELECT travel_status FROM parties WHERE id = ?')
      .bind(partyId)
      .first()) as any
    expect(party.travel_status).toBe('stationary')
  })
  ...
  it('tickAllPartiesMarch supports advancing multiple days in one call', async () => {
    ...
    expect(results).toEqual([
      { partyId, status: 'arrived', remainingKm: 0, arrivedAtWaypointId: farosundId },
    ])
  })
})
