...
import { resolveCohabitation, resolveEffectiveStats } from '@/rpg/utils/cohabitation'
...
vDesc('cohabitation resolution (#315)', () => {
  ...
  it('resolves the active passenger as driver when called on the host id', async () => {
    ...
    await seedChar(
      ...
      'Bellona',
      {},
      { hostBodyId: 'host-1', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
    await seedChar(
      'fork-1',
      'Cordelia',
      {},
      { hostBodyId: 'host-1', active: 0, updatedAt: '2184-01-01T00:00:00.000Z' },
    )
  ...
  it('resolves the same group when called directly on a passenger id', async () => {
    ...
    await seedChar(
      ...
      'Bellona 2',
      {},
      { hostBodyId: 'host-2', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
  ...
  it('tie-breaks on most-recently-updated when more than one row in the group is active', async () => {
    ...
    await seedChar(
      ...
      'Katerina 3b',
      {},
      { active: 1, updatedAt: '2184-01-01T00:00:00.000Z' },
    )
    await seedChar(
      'pass-3b',
      'Passenger 3b',
      {},
      { hostBodyId: 'host-3b', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
  ...
  it('falls back to the host row when nobody in the group is active', async () => {
    ...
    await seedChar(
      ...
      'Passenger 3',
      {},
      { hostBodyId: 'host-3', active: 0, updatedAt: '2184-01-01T00:00:00.000Z' },
    )
  ...
  it('falls back to itself when host_body_id is a dangling reference to a nonexistent host', async () => {
    await seedChar(
      ...
      'Ghost Passenger',
      {},
      { hostBodyId: 'no-such-host', active: 1 },
    )
    const r = await resolveCohabitation(db(), 'ghost-passenger')
    expect(r).toEqual({
      hostBodyId: 'ghost-passenger',
      driverId: 'ghost-passenger',
      isCohabitating: false,
    })
  })
  ...
  it('splits physical (host) vs. mental (driver) stats, and the display name follows the driver', async () => {
    await seedChar(
      'host-4',
      'Katerina 4',
      { str: 12, dex: 14, con: 20, int: 12, wis: 14, cha: 12 },
      { hp: 18, maxHp: 20, ac: 13, updatedAt: '2184-01-01T00:00:00.000Z' },
    )
    await seedChar(
      'gen-4',
      'Bellona 4',
      { str: 30, dex: 30, con: 30, int: 20, wis: 6, cha: 18 },
      { hostBodyId: 'host-4', active: 1, updatedAt: '2184-01-02T00:00:00.000Z' },
    )
