...
import {
  handleBroadcastManage,
  runProductionIntervene,
  createPendingVote,
} from '@/rpg/handlers/broadcast-manage'
...
describe('handleBroadcastManage', () => {
  ...
  async function createWorld(id = WORLD) {
    ...
    await env.RPG_DB.prepare(
      ...
      .run()
    await env.RPG_DB.prepare('INSERT OR IGNORE INTO world_state (world_id) VALUES (?)')
      .bind(id)
      .run()
  }
  ...
  async function createCharacter(id: string, name = 'Yune') {
    const now = new Date().toISOString()
    await env.RPG_DB.prepare(
      "INSERT OR IGNORE INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, '{}', 10, 10, 10, 1, ?, ?)",
    )
  ...
  it('audience_pulse applies the positive or negative branch of a polarizing event', async () => {
    ...
    const pos = await handleBroadcastManage(db(), {
      action: 'audience_pulse',
      worldId: WORLD,
      characterId: 'char-4',
      eventType: 'betrayed_yield',
    ...
    const neg = await handleBroadcastManage(db(), {
      action: 'audience_pulse',
      worldId: WORLD,
      characterId: 'char-4',
      eventType: 'betrayed_yield',
  ...
  it('audience_pulse clamps approval to [0, 100]', async () => {
    ...
    await handleBroadcastManage(db(), {
      action: 'audience_pulse',
      worldId: WORLD,
      characterId: 'char-5',
      eventType: 'reached_extraction',
    })
    await handleBroadcastManage(db(), {
      action: 'audience_pulse',
      worldId: WORLD,
  ...
  it('resolve_vote creates-and-resolves fan_favorite in one call', async () => {
    ...
    const r = await handleBroadcastManage(db(), {
      action: 'resolve_vote',
      worldId: WORLD,
      voteType: 'fan_favorite',
      day: 3,
      winningOption: 'char-1',
    })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
  ...
  it('resolve_vote computes mercy_kill consequences for yes and no', async () => {
    await createWorld()
    const yes = await handleBroadcastManage(db(), {
      action: 'resolve_vote',
      worldId: WORLD,
      voteType: 'mercy_kill',
      winningOption: 'yes',
    ...
    const no = await handleBroadcastManage(db(), {
      action: 'resolve_vote',
      worldId: WORLD,
      voteType: 'mercy_kill',
      winningOption: 'no',
  ...
  it('resolve_vote computes prize_drop_location and showdown consequences', async () => {
    await createWorld()
    const location = await handleBroadcastManage(db(), {
      action: 'resolve_vote',
      worldId: WORLD,
      voteType: 'prize_drop_location',
      winningOption: '12,34',
    })
    expect(JSON.parse(location.content[0].text).consequence.coordinatesRevealed).toBe('12,34')

    const showdown = await handleBroadcastManage(db(), {
      action: 'resolve_vote',
      worldId: WORLD,
      voteType: 'showdown',
      winningOption: 'yield-a, yield-b',
    })
    expect(JSON.parse(showdown.content[0].text).consequence.pairedYields).toEqual([
      'yield-a',
      'yield-b',
    ])
  })
  ...
  it('production_intervene applies all signal boosts and caps the threshold at 95', async () => {
    ...
    const r = await handleBroadcastManage(db(), {
      action: 'production_intervene',
      worldId: WORLD,
      noEncounterIn24h: true,
      allYieldsStationary: true,
      moraleStableDays: 5,
      daysSinceLastIntervention: 100,
  ...
  it('runProductionIntervene is directly usable by production-manage and updates world_state.last_intervention_at', async () => {
    ...
    const state = (await env.RPG_DB.prepare(
      'SELECT last_intervention_at FROM world_state WHERE world_id = ?',
    )
      .bind(WORLD)
      .first()) as { last_intervention_at: string | null }
  ...
  it('celeste_moment returns a templated broadcast for every known eventType', async () => {
    for (const eventType of [
      ...
      const r = await handleBroadcastManage(db(), {
        action: 'celeste_moment',
        eventType,
        characterId: 'yune',
        details: 'a sharpened stick',
      })
      const body = JSON.parse(r.content[0].text)
      expect(body.success).toBe(true)
