// Tests for world_manage registration via registerTool() (#545)
import { describe } from './support/helpers'
import { SELF, env } from 'cloudflare:test'
import { expect, it } from 'vitest'
import { getToolHandler } from '../../src/tools/register'

describe('world_manage via registerTool() (Phase 4 #545)', () => {
  ...
  it('getToolHandler resolves world_manage', () => {
    const handler = getToolHandler('world_manage')
    expect(handler).toBeDefined()
    expect(typeof handler).toBe('function')
  })

  it('handles check_convergence action via MCP endpoint', async () => {
    const r = await callTool('world_manage', {
  ...
  it('rejects missing action via MCP endpoint', async () => {
    const r = await callTool('world_manage', { thread_id: 'test' })
    expect(r.error || r.success === false).toBeTruthy()
  })

  it('rejects invalid action via MCP endpoint', async () => {
    const r = await callTool('world_manage', { action: 'invalid_action_xyz' })
    expect(r.error || r.success === false).toBeTruthy()
  })
})
