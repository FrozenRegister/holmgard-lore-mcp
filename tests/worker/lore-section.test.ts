...
describe('get_lore_section', () => {
  it('exact match returns section content', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:basic', sections: ['Personality'] })
  ...
  it('case-insensitive match in loose mode', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:case', sections: ['personality'] })
    expect(res.result.sections['personality']).toBe('Curious and kind.')
  })

  it('trailing colon stripped in loose mode', async () => {
    await seedKV('section:colon', '## Personality:\nCurious.')
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:colon', sections: ['Personality'] })
  ...
  it('whitespace collapsed in loose mode', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:spaces', sections: ['Physical Profile'] })
  ...
  it('not_found lists missing sections', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:missing', sections: ['Inventory'] })
  ...
  it('empty section returns empty string and empty_section warning', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:empty', sections: ['Personality'] })
  ...
  it('no ## headings: returns not_found and no_sections_found warning', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:flat', sections: ['Personality'] })
    expect(res.result.sections).toEqual({})
  ...
  it('fallback to # headings when no ## headings exist', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:single-hash', sections: ['Title'] })
  ...
  it('### headings are section boundaries alongside ## headings', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:sub', sections: ['Strengths', 'Weaknesses', 'Goals'] })
  ...
  it('special characters in section name match exactly', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:special', sections: ['Weight-1 (Predator Drive)'] })
  ...
  it('substring does not cause false match (Goals vs Goals (Completed))', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:substring', sections: ['Goals'] })
  ...
  it('last section runs to EOF correctly', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:last', sections: ['Section B'] })
  ...
  it('duplicate section: first non-empty wins, duplicate_section warning added', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:dup', sections: ['Notes'] })
  ...
  it('duplicate section: skips empty first occurrence, returns first non-empty, no empty_section warning', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:dup-empty-first', sections: ['Notes'] })
  ...
  it('duplicate section: all empty → returns "", warns both duplicate_section and empty_section', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:dup-all-empty', sections: ['Notes'] })
  ...
  it('zero sections requested: sections={}, not_found=[], no_sections_requested warning', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:zero', sections: [] })
    expect(res.result.sections).toEqual({})
    expect(res.result.not_found).toEqual([])
  ...
  it('unicode and emoji in section name', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:unicode', sections: ["État d'Esprit 😤"] })
  ...
  it('non-existent key returns key_not_found error in result', async () => {
    const res = await callTool('lore_manage', { action: 'get_section', key: 'character:does-not-exist-99999', sections: ['Personality'] })
    expect(res.error).toBeUndefined()
  ...
  it('consecutive empty sections both get empty_section warnings', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:consecutive', sections: ['Section A', 'Section B', 'Section C'] })
  ...
  it('mixed request: found sections returned, missing in not_found', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:mixed', sections: ['Personality', 'Inventory', 'Goals'] })
    expect(res.result.sections['Personality']).toBe('Curious.')
  ...
  it('very long section returns full content without truncation', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:long', sections: ['Notes'] })
  ...
  it('mixed # and ## headings: both are boundaries, sections accessible at any level', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:mixed-hash', sections: ['Section A', 'Title Block'] })
  ...
  it('strict mode does not strip trailing colon', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:strict', sections: ['Personality'], mode: 'strict' })
  ...
  it('strict mode matches when heading and request are identical (case-insensitive)', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:strict-match', sections: ['PERSONALITY'], mode: 'strict' })
    expect(res.result.sections['PERSONALITY']).toBe('Curious.')
  })

  it('result includes version from lore metadata', async () => {
    await seedKV('section:version', '## Notes\nSome notes.')
    const res = await callTool('lore_manage', { action: 'get_section', key: 'section:version', sections: ['Notes'] })
...
describe('append_to_section', () => {
  it('appends to end of populated section (default position)', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:populated', section: 'Personality', text: 'Loyal to companions.' })
  ...
  it('prepends to start of section', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:prepend', section: 'Personality', text: 'A former novice. ', position: 'start' })
  ...
  it('replaced_empty action when section has no content', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:empty-sec', section: 'Notes', text: 'First observation.' })
  ...
  it('creates section when not found and auto_create is true (default)', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:create', section: 'Inventory', text: 'rations×3' })
  ...
  it('returns section_not_found when auto_create is false and section missing', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:no-create', section: 'Inventory', text: 'rations', auto_create: false })
  ...
  it('targets first occurrence for duplicate section, adds duplicate_section warning', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:dup', section: 'Notes', text: 'Additional.' })
    expect(res.result.warnings).toContain('duplicate_section')
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:dup' })).result.text as string
  ...
  it('handles last section running to EOF without trailing heading', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:eof', section: 'Section B', text: 'More B.' })
    expect(res.result.action).toBe('appended')
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:eof' })).result.text as string
  ...
  it('text starting with newline inserts as new paragraph', async () => {
    ...
    await callTool('lore_manage', { action: 'append_section', key: 'ats:newpara', section: 'Notes', text: '\n\nLine three.' })
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:newpara' })).result.text as string
    expect(text).toContain('Line two.\n\nLine three.')
  })

  it('appending a single word adds a space between existing text and new text', async () => {
    await seedKV('ats:word', '## Personality\nBrave.')
    await callTool('lore_manage', { action: 'append_section', key: 'ats:word', section: 'Personality', text: 'Loyal.' })
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:word' })).result.text as string
  ...
  it('empty text returns empty_text error without mutating entry', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:empty-text', section: 'Personality', text: '' })
    expect(res.result.error).toBe('empty_text')
    expect((await callTool('lore_manage', { action: 'get', query: 'ats:empty-text' })).result.text).toBe('## Personality\nCurious.')
  })

  it('whitespace-only text returns empty_text error', async () => {
    await seedKV('ats:ws-text', '## Personality\nCurious.')
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:ws-text', section: 'Personality', text: '   ' })
    expect(res.result.error).toBe('empty_text')
  })
  ...
  it('very long append succeeds without truncation', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:long', section: 'Notes', text: '\n' + longText })
    expect(res.result.action).toBe('appended')
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:long' })).result.text as string
  ...
  it('section name with special characters (parens, hyphens) matches correctly', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:special', section: 'Weight-1 (Predator Drive)', text: ' Updated: 0.90' })
    expect(res.result.action).toBe('appended')
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:special' })).result.text as string
  ...
  it('substring section name does not false-match longer name', async () => {
    ...
    await callTool('lore_manage', { action: 'append_section', key: 'ats:substr', section: 'Goals', text: ' New goal.' })
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:substr' })).result.text as string
  ...
  it('trailing colon on heading is stripped for matching', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:colon', section: 'Personality', text: ' Loyal.' })
    expect(res.result.action).toBe('appended')
    expect((await callTool('lore_manage', { action: 'get', query: 'ats:colon' })).result.text).toContain('Curious. Loyal.')
  })
  ...
  it('no ## headings + auto_create true creates section at end', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:no-headings', section: 'Personality', text: 'Curious.', auto_create: true })
    expect(res.result.action).toBe('created')
    expect((await callTool('lore_manage', { action: 'get', query: 'ats:no-headings' })).result.text).toContain('## Personality\nCurious.')
  })

  it('no ## headings + auto_create false returns section_not_found', async () => {
    await seedKV('ats:no-headings-nc', 'Just flat text.')
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:no-headings-nc', section: 'Personality', text: 'Curious.', auto_create: false })
    expect(res.result.error).toBe('section_not_found')
  })

  it('unicode and emoji in section name match correctly', async () => {
    await seedKV('ats:unicode', "## État d'Esprit 😤\nFrustrated.")
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:unicode', section: "État d'Esprit 😤", text: ' And hopeful.' })
    expect(res.result.action).toBe('appended')
    expect((await callTool('lore_manage', { action: 'get', query: 'ats:unicode' })).result.text).toContain('Frustrated. And hopeful.')
  })
  ...
  it('text containing ## strings is stored as literal content, not parsed as section boundaries', async () => {
    ...
    await callTool('lore_manage', { action: 'append_section', key: 'ats:hash-in-text', section: 'Notes', text: '\n## This is NOT a heading\nJust content.' })
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:hash-in-text' })).result.text as string
  ...
  it('non-existent key returns key_not_found error', async () => {
    const res = await callTool('lore_manage', { action: 'append_section', key: 'character:does-not-exist-ats-99999', section: 'Personality', text: 'Text.' })
    expect(res.result.error).toBe('key_not_found')
  })

  it('consecutive appends accumulate correctly (no stale-cache issue)', async () => {
    await seedKV('ats:consec', '## Notes\nFirst.')
    await callTool('lore_manage', { action: 'append_section', key: 'ats:consec', section: 'Notes', text: ' Second.' })
    await callTool('lore_manage', { action: 'append_section', key: 'ats:consec', section: 'Notes', text: ' Third.' })
    expect((await callTool('lore_manage', { action: 'get', query: 'ats:consec' })).result.text).toContain('First. Second. Third.')
  })
  ...
  it('auto-created section is placed after all existing content including trailing loose text', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:trailing', section: 'NewSection', text: 'New content.' })
    expect(res.result.action).toBe('created')
    const text = (await callTool('lore_manage', { action: 'get', query: 'ats:trailing' })).result.text as string
  ...
  it('response shape has key, section, action, position, new_version, bytes_added, warnings', async () => {
    ...
    const res = await callTool('lore_manage', { action: 'append_section', key: 'ats:shape', section: 'Notes', text: ' More.' })
  ...
  it('mutation is reversible via restore_lore', async () => {
    ...
    await callTool('lore_manage', { action: 'append_section', key: 'ats:restore', section: 'Notes', text: ' Appended.' })
    await callTool('lore_manage', { action: 'restore', key: 'ats:restore' })
    expect((await callTool('lore_manage', { action: 'get', query: 'ats:restore' })).result.text).toBe('## Notes\nOriginal content.')
  })
})
