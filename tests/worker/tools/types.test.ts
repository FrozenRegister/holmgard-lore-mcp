...
describe('makeActionDispatcher', () => {
  ...
  const ACTION_MAP: Record<string, ActionSpec> = {
    echo: defineAction(echoSchema, async ({ c, id, args }) => c.json({ result: { value: args.value, id } }), {
      value: 'hello',
    }),
    echo_no_example: defineAction(echoSchema, async ({ c, id, args }) => c.json({ result: { value: args.value, id } })),
  }

  const dispatch = makeActionDispatcher('test_tool', ACTION_MAP)

  it('rejects missing action', async () => {
    const res = (await dispatch({ c: { json: anyCtx } as any, id: '1', args: {}, isAuthenticated: false })) as any
    expect(res.error).toBeDefined()
    expect(res.error.message).toContain('Missing required param: action')
  })

  it('rejects non-string action', async () => {
    const res = (await dispatch({ c: { json: anyCtx } as any, id: '1', args: { action: 7 }, isAuthenticated: false })) as any
    expect(res.error).toBeDefined()
    expect(res.error.message).toContain('Missing required param: action')
  })

  it('rejects unknown action', async () => {
    const res = (await dispatch({ c: { json: anyCtx } as any, id: '1', args: { action: 'nope' }, isAuthenticated: true })) as any
    expect(res.error).toBeDefined()
    expect(res.error.message).toBe('Unknown action "nope"')
  })

  it('rejects invalid params and includes the example payload', async () => {
    const res = (await dispatch({ c: { json: anyCtx } as any, id: '1', args: { action: 'echo' }, isAuthenticated: true })) as any
    expect(res.error).toBeDefined()
    expect(res.error.data.example).toEqual({ action: 'echo', value: 'hello' })
  })

  it('rejects invalid params with no example configured', async () => {
    const res = (await dispatch({ c: { json: anyCtx } as any, id: '1', args: { action: 'echo_no_example' }, isAuthenticated: true })) as any
    expect(res.error).toBeDefined()
...
describe('defineAction', () => {
  it('pairs a schema and handler into an ActionSpec', () => {
    ...
    const handler = async ({ c, id, args }: { c: any; id: any; args: { n: number } }) => c.json({ n: args.n, id })
