...
import { handleResourceManage, degradeOwnerResources, tickAllOwnersDegradation } from '@/rpg/handlers/resource-manage'
...
describe('handleResourceManage', () => {
  ...
  it('consume requires ownerType, ownerId, and inventoryId/itemName', async () => {
    ...
    const r2 = await handleResourceManage(db(), { action: 'consume', ownerType: 'character', ownerId: 'c1' })
    expect(JSON.parse(r2.content[0].text).error).toBe(true)
  })

  it('consume errors when the resource is not found', async () => {
    const r = await handleResourceManage(db(), { action: 'consume', ownerType: 'character', ownerId: 'c1', itemName: 'Nothing' })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  })

  async function seedInventory(ownerType: string, ownerId: string, itemName: string, opts: Partial<{ quantity: number; category: string; degradationTimer: number | null; expiresOnDay: number | null; day: number }> = {}) {
    const id = crypto.randomUUID()
    const now = new Date().toISOString()
    await env.RPG_DB.prepare('INSERT INTO resource_inventory (id, world_id, owner_type, owner_id, item_name, category, quantity, degradation_timer, expires_on_day, acquired_day, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
      .bind(id, WORLD, ownerType, ownerId, itemName, opts.category ?? 'food', opts.quantity ?? 1, opts.degradationTimer ?? null, opts.expiresOnDay ?? null, opts.day ?? 0, now, now).run()
    return id
  }

  it('consume by itemName decrements quantity and returns the catalog effect text', async () => {
    await createWorld()
    await seedInventory('character', 'char-1', 'Field Dressing', { quantity: 2, category: 'medical' })
    const r = await handleResourceManage(db(), { action: 'consume', ownerType: 'character', ownerId: 'char-1', itemName: 'Field Dressing' })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
  ...
  it('consume by inventoryId works and returns null effect for unknown items', async () => {
    await createWorld()
    const id = await seedInventory('character', 'char-1', 'Homebrew Tonic', { quantity: 1, category: 'custom' })
    const r = await handleResourceManage(db(), { action: 'consume', ownerType: 'character', ownerId: 'char-1', inventoryId: id })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
  ...
  it('degrade requires ownerType, ownerId, and worldId', async () => {
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'c1' })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  })
  ...
  it('degrade spoils an item once its degradation timer runs out', async () => {
    await createWorld()
    await seedInventory('character', 'char-2', 'Standard Ration Pack', { quantity: 1, category: 'food', degradationTimer: 1 })
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-2', worldId: WORLD, days: 2 })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
  ...
  it('degrade decrements a still-viable timer without spoiling', async () => {
    await createWorld()
    await seedInventory('character', 'char-3', 'Standard Ration Pack', { quantity: 1, category: 'food', degradationTimer: 5 })
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-3', worldId: WORLD, days: 1 })
    const body = JSON.parse(r.content[0].text)
  ...
  it('degrade spoils an item once its expiresOnDay is reached', async () => {
    await createWorld()
    await seedInventory('character', 'char-4', 'Antiseptic Vial', { quantity: 1, category: 'medical', expiresOnDay: 25 })
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-4', worldId: WORLD, day: 25 })
    const body = JSON.parse(r.content[0].text)
  ...
  it('degrade resets the starvation streak when the owner holds food', async () => {
    await createWorld()
    await seedInventory('character', 'char-5', 'Standard Ration Pack', { quantity: 1, category: 'food' })
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-5', worldId: WORLD })
    const body = JSON.parse(r.content[0].text)
  ...
  it('degrade increments the starvation streak with no food and reports tiered penalties', async () => {
    await createWorld()
    const r1 = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-6', worldId: WORLD })
    expect(JSON.parse(r1.content[0].text).daysWithoutFood).toBe(1)
    const r2 = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-6', worldId: WORLD })
  ...
  it('degrade reports the day-4+ death-save tier', async () => {
    await createWorld()
    for (let i = 0; i < 4; i++) await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-7', worldId: WORLD })
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-7', worldId: WORLD })
    const body = JSON.parse(r.content[0].text)
  ...
  it('degrade respects an explicit ateToday flag even with no food in inventory', async () => {
    await createWorld()
    await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-8', worldId: WORLD })
    const r = await handleResourceManage(db(), { action: 'degrade', ownerType: 'character', ownerId: 'char-8', worldId: WORLD, ateToday: true })
    const body = JSON.parse(r.content[0].text)
    expect(body.daysWithoutFood).toBe(0)
  })
  ...
  it('degradeOwnerResources and tickAllOwnersDegradation are directly usable by production-manage', async () => {
    await createWorld()
    await seedInventory('party', 'party-1', 'Standard Ration Pack', { quantity: 1, category: 'food' })
  ...
  it('craft errors for an unknown biome', async () => {
    const r = await handleResourceManage(db(), { action: 'craft', biomeName: 'lava_moon', itemName: 'Splint' })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  })

  it('craft errors for an item not craftable in the given biome', async () => {
    const r = await handleResourceManage(db(), { action: 'craft', biomeName: 'Pine Forest', itemName: 'Cave shelter' })
    const body = JSON.parse(r.content[0].text)
    expect(body.error).toBe(true)
  })
  ...
  it('craft succeeds against its DC and stores the item when owner info is given', async () => {
    await createWorld()
    const r = await handleResourceManage(db(), {
      action: 'craft', biomeName: 'pine forest', itemName: 'Splint', abilityModifier: 20, rollValue: 10,
      ownerType: 'character', ownerId: 'char-9', worldId: WORLD, day: 3,
    })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
    ...
    const state = await handleResourceManage(db(), { action: 'get_state', ownerType: 'character', ownerId: 'char-9' })
    const stateBody = JSON.parse(state.content[0].text)
    expect(stateBody.inventory.some((i: { item_name: string }) => i.item_name === 'Splint')).toBe(true)
  })

  it('craft fails when the roll is below the DC and does not store anything', async () => {
    const r = await handleResourceManage(db(), { action: 'craft', biomeName: 'pine forest', itemName: 'Splint', abilityModifier: -20, rollValue: 1 })
    const body = JSON.parse(r.content[0].text)
  ...
  it('improvise applies a break-on-use roll and never bypasses it for craft', async () => {
    ...
    const improviseRes = await handleResourceManage(db(), { action: 'improvise', biomeName: 'pine forest', itemName: 'Improvised spear', abilityModifier: 20, rollValue: 10 })
    ...
    const craftRes = await handleResourceManage(db(), { action: 'craft', biomeName: 'pine forest', itemName: 'Improvised spear', abilityModifier: 20, rollValue: 10 })
  ...
  it('scavenge rolls against a forageable item DC and reports success/failure', async () => {
    const r = await handleResourceManage(db(), { action: 'scavenge', biomeName: 'meadow', abilityModifier: 20, rollValue: 15 })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
  ...
  it('scavenge stores the found item when owner info is given', async () => {
    await createWorld()
    const r = await handleResourceManage(db(), {
      action: 'scavenge', biomeName: 'beach', abilityModifier: 20, rollValue: 15,
      ownerType: 'party', ownerId: 'party-2', worldId: WORLD, day: 1,
    })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
    if (body.succeeded) {
      const state = await handleResourceManage(db(), { action: 'get_state', ownerType: 'party', ownerId: 'party-2' })
      const stateBody = JSON.parse(state.content[0].text)
  ...
  it('scavenge reports failure without storing anything on a low roll', async () => {
    const r = await handleResourceManage(db(), { action: 'scavenge', biomeName: 'limestone karst', abilityModifier: -20, rollValue: 1 })
    const body = JSON.parse(r.content[0].text)
    expect(body.succeeded).toBe(false)
  })
  ...
  it('scavenge works for the remaining biomes (coastal_water, bog)', async () => {
    for (const biome of ['coastal_water', 'bog']) {
      const r = await handleResourceManage(db(), { action: 'scavenge', biomeName: biome, abilityModifier: 20, rollValue: 15 })
      const body = JSON.parse(r.content[0].text)
      expect(body.success).toBe(true)
    }
  })
  ...
  it('get_state returns an empty inventory and zero starvation for a fresh owner', async () => {
    const r = await handleResourceManage(db(), { action: 'get_state', ownerType: 'character', ownerId: 'brand-new' })
    const body = JSON.parse(r.content[0].text)
    expect(body.success).toBe(true)
