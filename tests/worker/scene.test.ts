...
describe('present_choices', () => {
  ...
  it('blocks choices requiring missing item', async () => {
    ...
    const res = await callTool('scene_manage', {
      action: 'present_choices',
      scene_key: 'scene:chest',
      entity_key: 'character:no-key',
    })
...
describe('commit_choice — timeline_events bridge (#350)', () => {
  ...
  it('writes a "chose" timeline_events row when the entity resolves to a D1 character with world_id', async () => {
    ...
    await env.LORE_DB.put(
      'choice:bridge-quest',
      JSON.stringify({
        text: '**Outcome-Seed:** The bridge holds.\n**State-Change:** Crossed',
        meta: { version: 1 },
      }),
    )
    await env.LORE_DB.put(
      'character:bridge-hero',
      JSON.stringify({
        text: '**Status:** Idle\n**Location:** location:riverbank',
        meta: { version: 1, d1_id: 'char-bridge-1' },
      }),
    )

    const res = await callTool('scene_manage', {
      action: 'commit_choice',
      choice_id: 'choice:bridge-quest',
      entity_key: 'character:bridge-hero',
    })
    ...
    const row = (await env.RPG_DB.prepare('SELECT * FROM timeline_events WHERE id = ?')
      .bind(res.result.timeline_event_id)
  ...
  it('returns null timeline_event_id (no error) when the entity has no matching D1 character', async () => {
    ...
    const res = await callTool('scene_manage', {
      action: 'commit_choice',
      choice_id: 'choice:unbridged',
      entity_key: 'character:no-d1-match',
    })
    expect(res.error).toBeUndefined()
  ...
  it('returns null timeline_event_id (no error) when the resolved D1 character has no world_id', async () => {
    ...
    await env.LORE_DB.put(
      'choice:worldless-choice',
      JSON.stringify({ text: '**Outcome-Seed:** Adrift.', meta: { version: 1 } }),
    )
    await env.LORE_DB.put(
      'character:worldless',
      JSON.stringify({
        text: '**Status:** Idle',
        meta: { version: 1, d1_id: 'char-worldless' },
      }),
    )

    const res = await callTool('scene_manage', {
      action: 'commit_choice',
      choice_id: 'choice:worldless-choice',
      entity_key: 'character:worldless',
    })
    expect(res.error).toBeUndefined()
    expect(res.result.timeline_event_id).toBeNull()
  })
  ...
  it('returns null timeline_event_id (no error) when meta.d1_id points to a nonexistent D1 character', async () => {
    ...
    await env.LORE_DB.put(
      ...
      JSON.stringify({
        text: '**Status:** Idle',
        meta: { version: 1, d1_id: 'char-does-not-exist' },
      }),
    )

    const res = await callTool('scene_manage', {
      action: 'commit_choice',
      choice_id: 'choice:stale-id-choice',
      entity_key: 'character:stale-id',
    })
    expect(res.error).toBeUndefined()
    expect(res.result.timeline_event_id).toBeNull()
  })
  ...
  it('records a "chose" event with null location_id when the entity has no Location field', async () => {
    ...
    await env.LORE_DB.put(
      'character:locationless-hero',
      JSON.stringify({
        text: '**Status:** Idle',
        meta: { version: 1, d1_id: 'char-bridge-2' },
      }),
    )

    const res = await callTool('scene_manage', {
      action: 'commit_choice',
      choice_id: 'choice:no-location-choice',
      entity_key: 'character:locationless-hero',
    })
    expect(res.error).toBeUndefined()
    expect(res.result.timeline_event_id).toBeTruthy()

    const row = (await env.RPG_DB.prepare('SELECT location_id FROM timeline_events WHERE id = ?')
      .bind(res.result.timeline_event_id)
      .first()) as Record<string, unknown>
...
describe('get_choice_history', () => {
  it('parses Choice-History into structured entries', async () => {
    await seedKV(
      'character:veteran',
      '**Choice-History:** choice:join-guild@2024-01-01T00:00:00.000Z, choice:betray-ally@2024-06-01T00:00:00.000Z',
    )
    const res = await callTool('scene_manage', {
      action: 'get_history',
...
describe('scene_brief', () => {
  ...
  it('includes open setups for present actors', async () => {
    ...
    await callTool('continuity_manage', {
      action: 'plant_setup',
      id: 'noble-secret',
      description: 'Noble hides a secret',
      tension: 4,
      actors: ['character:noble'],
    })
  ...
  it('returns error for nonexistent location', async () => {
    const res = await callTool('scene_manage', {
      action: 'brief',
      location_key: 'location:nonexistent-9999',
    })
    expect(res.error).toBeDefined()
    expect(res.error.code).toBe(-32602)
  })
...
describe('render_pov', () => {
  ...
  it('keeps [hidden] lines for high-perception POV', async () => {
    ...
    const res = await callTool('scene_manage', {
      action: 'render_pov',
      pov_entity_key: 'character:sharp-eyes',
      location_key: 'location:shadows',
    })
  ...
  it('falls back to species voice hints when the entity has none of its own', async () => {
    ...
    await seedKV(
      'species:lamia',
      '**Diction:** Sibilant, measured, drawn-out consonants\n**Register:** Low, hissing undertone',
    )
    await seedKV(
      'character:kavissa',
      '**Status:** Active\n**Species:** lamia\n**Location:** location:tavern\n**Perception:** 0.8',
    ...
    const res = await callTool('scene_manage', {
      action: 'render_pov',
      pov_entity_key: 'character:kavissa',
      location_key: 'location:tavern',
      include_voice_hints: true,
    })
  ...
  it('returns null voice hints with source "none" when neither entity nor species has data', async () => {
    await seedKV('location:tavern', 'The tavern is warm.')
    await seedKV(
      'character:blank-voice',
      '**Status:** Active\n**Location:** location:tavern\n**Perception:** 0.8',
    )
    const res = await callTool('scene_manage', {
      action: 'render_pov',
      pov_entity_key: 'character:blank-voice',
      location_key: 'location:tavern',
      include_voice_hints: true,
    })
  ...
  it('uses entity Location field when no location_key provided', async () => {
    ...
    const res = await callTool('scene_manage', {
      action: 'render_pov',
      pov_entity_key: 'character:recluse',
    })
    expect(res.error).toBeUndefined()
  ...
  it('falls back to scene_key when location_key is omitted', async () => {
    ...
    const res = await callTool('scene_manage', {
      action: 'render_pov',
      pov_entity_key: 'character:wanderer',
      scene_key: 'scene:campfire',
    })
    expect(res.error).toBeUndefined()
