...
describe('check_continuity', () => {
  it('finds dangling character references', async () => {
    ...
    expect(findings.some((f) => f.check === 'dangling' && f.key === 'character:wanderer')).toBe(
      true,
    )
  })
  ...
  it('accepts "medium" as an alias for severity_floor=warn', async () => {
    ...
    expect(
      findings.some((f) => f.key === 'character:sev-alias-test' && f.severity === 'warn'),
    ).toBe(true)
  })
  ...
  describe('auto_fix', () => {
    it('removes dangling test-key references', async () => {
      ...
      expect(
        fixes.some(
          (f) => f.key === 'character:autofix-test-ref' && f.action === 'removed_test_reference',
        ),
      ).toBe(true)
      const lore = await callTool('lore_manage', {
        action: 'get',
        query: 'character:autofix-test-ref',
      })
    ...
    it('auto-corrects an unambiguous typo against an existing key', async () => {
      ...
      expect(
        fixes.some((f) => f.key === 'character:autofix-typo' && f.action === 'typo_correction'),
      ).toBe(true)
    ...
    it('skips as ambiguous when multiple close-match candidates exist', async () => {
      ...
      expect(
        skips.some(
          (s) => s.key === 'character:autofix-ambiguous' && s.reason.includes('ambiguous'),
        ),
      ).toBe(true)
    })
    ...
    it('falls back an occupancy issue to location:unknown when no close match exists', async () => {
      ...
      const lore = await callTool('lore_manage', {
        action: 'get',
        query: 'character:autofix-occupancy',
      })
    ...
    it('auto-corrects an unambiguous occupancy typo against an existing location', async () => {
      ...
      await seedKV(
        'character:autofix-occupancy-typo',
      ...
      expect(
        fixes.some(
          (f) => f.key === 'character:autofix-occupancy-typo' && f.action === 'typo_correction',
        ),
      ).toBe(true)
      const lore = await callTool('lore_manage', {
        action: 'get',
        query: 'character:autofix-occupancy-typo',
      })
    ...
    it('skips an occupancy issue as ambiguous when multiple close-match locations exist', async () => {
      ...
      await seedKV(
        'character:autofix-occupancy-ambiguous',
      ...
      expect(
        skips.some(
          (s) =>
            s.key === 'character:autofix-occupancy-ambiguous' && s.reason.includes('ambiguous'),
        ),
      ).toBe(true)
    })
    ...
    it('skips a dangling reference with no confident match as unfixable', async () => {
      ...
      expect(
        skips.some(
          (s) => s.key === 'character:autofix-nomatch' && s.reason === 'no confident match found',
        ),
      ).toBe(true)
    })
    ...
    it('skips knowledge and inventory findings as requiring manual review', async () => {
      ...
      expect(
        skips.some(
          (s) =>
            s.key === 'character:autofix-inv' &&
            s.check === 'inventory' &&
            s.reason.includes('entity-by-entity'),
        ),
      ).toBe(true)
    })
    ...
    it('auto-fixed entries are reversible via restore_lore', async () => {
      ...
      const lore = await callTool('lore_manage', {
        action: 'get',
        query: 'character:autofix-restore',
      })
