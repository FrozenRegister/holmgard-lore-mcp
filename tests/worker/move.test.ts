...
describe('move_entity', () => {
  it('updates Location field and returns success', async () => {
    ...
    const res = await callTool('entity_manage', { action: 'move', entity_key: 'character:traveler', new_location_key: 'location:new-city' })
  ...
  it('updates location indexes so get_location_occupants reflects the move', async () => {
    ...
    await callTool('entity_manage', { action: 'move', entity_key: 'character:mover', new_location_key: 'location:room-b' })
    const oldLoc = await callTool('world_manage', { action: 'get_location_occupants', location_key: 'location:room-a' })
    expect(oldLoc.result.occupants).toHaveLength(0)
    const newLoc = await callTool('world_manage', { action: 'get_location_occupants', location_key: 'location:room-b' })
  ...
  it('pushes history before writing', async () => {
    ...
    await callTool('entity_manage', { action: 'move', entity_key: 'character:hist-mover', new_location_key: 'location:end' })
    const restore = await callTool('lore_manage', { action: 'restore', key: 'character:hist-mover' })
  ...
  it('returns error for nonexistent entity', async () => {
    const res = await callTool('entity_manage', { action: 'move', entity_key: 'character:ghost-9999', new_location_key: 'location:void' })
