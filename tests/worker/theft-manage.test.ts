...
describe('handleTheftManage', () => {
  ...
  async function seed() {
    ...
    await env.RPG_DB.prepare(
      'INSERT OR IGNORE INTO items (id, name, description, type, weight, value, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('test-item', 'Gold Ring', null, 'jewelry', 0, 100, now, now)
      .run()
  ...
  it('steal records theft', async () => {
    await seed()
    const r = await handleTheftManage(db(), {
      action: 'steal',
      itemId: 'test-item',
      stolenFrom: 'victim-char',
      stolenBy: 'thief-char',
      stolenLocation: 'market',
      witnesses: ['npc-1'],
      bounty: 50,
    })
    const body = JSON.parse(r.content[0].text)
  ...
  it('get returns stolen item record', async () => {
    await seed()
    const c = await handleTheftManage(db(), {
      action: 'steal',
      itemId: 'test-item',
      stolenFrom: 'victim-char',
      stolenBy: 'thief-char',
