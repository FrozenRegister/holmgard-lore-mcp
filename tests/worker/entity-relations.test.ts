// tests/worker/entity-relations.test.ts
...
describe('POST /admin/relations', () => {
  ...
  it('creates a relation and returns ok:true with an id', async () => {
    ...
    const body = (await res.json()) as RelationCreateResponse
  ...
  it('returns 401 with wrong secret', async () => {
    ...
    const body = (await res2.json()) as SuccessResponse
...
describe('PATCH /admin/relations/:id', () => {
  ...
  it('updates relation_type', async () => {
    ...
    const body = (await patchRes.json()) as SuccessResponse
    expect(body.ok).toBe(true)

    const get = (await (await getRelations('characters', 'c-patch')).json()) as RelationsResponse
...
describe('DELETE /admin/relations/:id', () => {
  ...
  it('deletes an existing relation and returns ok:true', async () => {
    ...
    const body = (await delRes.json()) as SuccessResponse
    expect(body.ok).toBe(true)

    const get = (await (await getRelations('characters', 'c-del')).json()) as RelationsResponse
  ...
  it('returns 401 when secret header is missing', async () => {
    const created = (await (await createRelation({ from_id: 'c-del-noauth' })).json()) as Record<
      string,
      any
    >
    const res = await SELF.fetch(`http://example.com/admin/relations/${created.id}`, {
  ...
  it('accepts X-Api-Key as fallback header (DELETE)', async () => {
    const created = (await (await createRelation({ from_id: 'c-del-apikey' })).json()) as Record<
      string,
      any
    >
    const res = await SELF.fetch(`http://example.com/admin/relations/${created.id}`, {
...
describe('POST /admin/relations — edge cases', () => {
  ...
  it('trims notes to null when it becomes empty after trim', async () => {
    ...
    expect(res.status).toBe(201)
    const relations = (await (await getRelations('characters', 'c-trim-notes')).json()) as Record<
      string,
      any
    >
    const relation = relations.relations[0]
