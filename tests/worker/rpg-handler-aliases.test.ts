...
describe('rpg cross-sub action aliases (#404)', () => {
  ...
  it('npc_dialogue is a working alias for npc', async () => {
    const npc = await callTool('rpg', {
      sub: 'npc_dialogue',
      action: 'create',
      name: 'Merchant Alias Test',
    })
  ...
  it('party.place_character transparently routes to spawn.place_character', async () => {
    const world = await callTool('rpg', {
      sub: 'world',
      action: 'create',
      name: 'Party Place Char World',
      theme: 'fantasy',
    })
    const char = await callTool('rpg', {
      sub: 'character',
      action: 'create',
      name: 'Party Placeable',
      worldId: world.worldId,
    })
