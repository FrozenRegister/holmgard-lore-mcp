...
describe('append_event', () => {
  it('appends an event to an entity chronicle', async () => {
    const res = await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:zira',
      verb: 'sedated',
      object: 'character:predator',
      thread: 'thread-alpha',
  ...
  it('rejects invalid entity_id with FK validation error', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-1', 'Test World', 'seed123', 100, 100, now, now)
      .run()
  ...
  it('successfully inserts event to D1 with valid world_id and entity_id', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-2', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-test-1', 'Test Character', '{}', 10, 10, 15, 1, now, now)
      .run()
    ...
    const row = await env.RPG_DB.prepare('SELECT * FROM timeline_events WHERE id = ?')
      .bind(res.result.metadata.d1_event_id)
      .first()
  ...
  it('accepts date and description as aliases for at and detail', async () => {
    const res = await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:alias-test',
      verb: 'departed',
      date: '1264-05-01T00:00:00Z',
      description: 'Household begins journey',
      source: 'roleplay-session',
      world_id: 'test-world-1',
    })
    expect(res.error).toBeUndefined()
    const log = await callTool('continuity_manage', {
  ...
  it('derives entity_id from entity_key via lore_key lookup', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-lore', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('char-lore-1', 'Lore Character', '{}', 10, 10, 15, 1, now, now, 'character:lore-hero')
      .run()
    ...
    const row = (await env.RPG_DB.prepare('SELECT entity_id FROM timeline_events WHERE id = ?')
      .bind(res.result.metadata.d1_event_id)
      .first()) as { entity_id: string } | null
  ...
  it('skips entity_id derivation when lore_key column is missing (catch path)', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-nolore', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    ...
    const row = (await env.RPG_DB.prepare('SELECT entity_id FROM timeline_events WHERE id = ?')
      .bind(res.result.metadata.d1_event_id)
      .first()) as { entity_id: string | null } | null
  ...
  it('auto-witnesses events to co-located entities via D1', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-witness', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('market-square', 'Market Square', 'A bustling market square', 'urban', now, now)
      .run()
    // Source character
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-source', 'Source Character', '{}', 10, 10, 15, 1, now, now)
      .run()
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('char-witness', 'Witness Character', '{}', 10, 10, 15, 1, now, now, 'market-square')
      .run()
    ...
    const knowledge = await env.RPG_DB.prepare('SELECT * FROM entity_knowledge WHERE entity_id = ?')
      .bind('char-witness')
      .all()
  ...
  it('falls through to KV when timeline_events table is missing (non-FK D1 error)', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-fallback', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-fallback', 'Fallback Character', '{}', 10, 10, 15, 1, now, now)
      .run()
  ...
  it('returns FOREIGN KEY error when INSERT violates FK constraint', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-fk', 'Test World', 'seed123', 100, 100, now, now)
      .run()
  ...
  it('skips FK check when characters table is missing but world exists', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-nochars', 'Test World', 'seed123', 100, 100, now, now)
      .run()
  ...
  it('derives entity_id as null when lore_key column exists but no character matches', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-nomatch', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare('ALTER TABLE characters ADD COLUMN lore_key TEXT').run()
  ...
  it('auto-witnesses with object, no entity_id, and no detail (branch coverage)', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-aw2', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO room_nodes (id, name, base_description, biome_context, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
    )
      .bind('tavern-room', 'Tavern', 'A dimly lit tavern interior', 'urban', now, now)
      .run()
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('char-aw2-src', 'AW2 Source', '{}', 10, 10, 15, 1, now, now, 'character:aw2-source')
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at, current_room_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-aw2-wit', 'AW2 Witness', '{}', 10, 10, 15, 1, now, now, 'tavern-room')
      .run()
  ...
  it('auto-witnesses with no co-located occupants (empty for-loop)', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-aw3', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO room_nodes (id, name, base_description, biome_context, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
    )
      .bind('empty-room', 'Empty Room', 'An empty room with nobody in it', 'urban', now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-aw3-src', 'AW3 Source', '{}', 10, 10, 15, 1, now, now)
      .run()
  ...
  it('auto-witness catches errors when entity_knowledge table is missing', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-aw4', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO room_nodes (id, name, base_description, biome_context, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
    )
      .bind('aw4-room', 'AW4 Room', 'A room for testing', 'urban', now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-aw4-src', 'AW4 Source', '{}', 10, 10, 15, 1, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at, current_room_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-aw4-wit', 'AW4 Witness', '{}', 10, 10, 15, 1, now, now, 'aw4-room')
      .run()
  ...
  it('auto-witness catches errors when characters table is missing (occupants query fails)', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-aw5', 'Test World', 'seed123', 100, 100, now, now)
      .run()
...
describe('get_event_log', () => {
  ...
  it('accepts array of entity keys', async () => {
    await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:aa',
      verb: 'walked',
      world_id: 'test-world-1',
    })
    await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:bb',
      verb: 'ran',
      world_id: 'test-world-1',
    })
    const res = await callTool('continuity_manage', {
      action: 'get_event_log',
      entity_key: ['character:aa', 'character:bb'],
    })
    expect(res.result.metadata.returned).toBe(2)
  })
  ...
  it('returns empty when no events exist', async () => {
    const res = await callTool('continuity_manage', {
      action: 'get_event_log',
      entity_key: 'character:nobody-9999',
    })
...
describe('get_event_log — tier filter (#311)', () => {
  beforeEach(async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-tier', 'Test World Tier', 'seed', 10, 10, now, now)
      .run()
  })
  ...
  it('filters events by a single tier', async () => {
    await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:tier-test',
      verb: 'killed',
      world_id: 'test-world-tier',
    })
    await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:tier-test',
  ...
  it('accepts comma-separated tiers', async () => {
    await callTool('continuity_manage', {
      action: 'append_event',
      entity_key: 'character:tier-test-2',
      verb: 'killed',
      world_id: 'test-world-tier',
    })
    await callTool('continuity_manage', {
...
describe('taxonomy_list / taxonomy_set / taxonomy_delete (#311)', () => {
  ...
  it('lists seeded verbs filtered by category', async () => {
    ...
    expect(res.result.verbs.every((v: { category: string }) => v.category === 'production')).toBe(
      true,
    )
  })
  ...
  it('creates a new verb via taxonomy_set', async () => {
    const res = await callTool('continuity_manage', {
      ...
      tier: 'medium',
      category: 'narrative',
      description: 'A ceremony was performed',
    })
    expect(res.error).toBeUndefined()
    expect(res.result.metadata.updated).toBe(false)
    const list = await callTool('continuity_manage', {
      action: 'taxonomy_list',
      category: 'narrative',
    })
...
describe('#311 — D1-unavailable guards', () => {
  it('get_event_log errors on tier filter when RPG_DB unavailable', async () => {
    const res: any = await handle_get_event_log(
      mockNoDbCtx({ entity_key: 'character:x', tier: 'high', limit: 50 }),
    )
    expect(res.error).toBeDefined()
    expect(res.error.code).toBe(-32603)
  ...
  it('taxonomy_set errors when RPG_DB unavailable', async () => {
    const res: any = await handle_taxonomy_set(
      mockNoDbCtx({ verb: 'x', tier: 'high', category: 'narrative' }),
    )
    expect(res.error).toBeDefined()
    expect(res.error.code).toBe(-32603)
  })
...
describe('find_by_tag', () => {
  ...
  it('returns empty when no topics match', async () => {
    const res = await callTool('continuity_manage', {
      action: 'find_by_tag',
      tags: ['theme:nonexistent-xyz-123'],
    })
...
describe('set_entity_knowledge', () => {
  ...
  it('successfully inserts knowledge with valid entity_id', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('char-test-2', 'Test Character', '{}', 10, 10, 15, 1, now, now)
      .run()
...
describe('learn_from_event', () => {
  it('rejects invalid entity_id with FK validation error', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-3', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('event-source-char', 'Event Source', '{}', 10, 10, 15, 1, now, now)
      .run()
    await env.RPG_DB.prepare(
  ...
  it('rejects invalid event_id', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('char-test-3', 'Test Character', '{}', 10, 10, 15, 1, now, now)
      .run()
  ...
  it('successfully creates knowledge from event with valid IDs', async () => {
    ...
    await env.RPG_DB.prepare(
      ...
      .bind('test-world-4', 'Test World', 'seed123', 100, 100, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('event-source-char-2', 'Event Source', '{}', 10, 10, 15, 1, now, now)
      .run()
    await env.RPG_DB.prepare(
      'INSERT INTO characters (id, name, stats, hp, max_hp, ac, level, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
      .bind('char-test-4', 'Learning Character', '{}', 10, 10, 15, 1, now, now)
      .run()
...
describe('check_continuity', () => {
  it('detects dangling references', async () => {
    ...
    expect(findings.some((f) => f.check === 'dangling' && f.message.includes('missing-9999'))).toBe(
      true,
    )
  })
  ...
  it('world filter does not shrink the existence set used for dangling checks', async () => {
    ...
    expect(
      findings.some(
        (f) => f.key === 'character:cordelia-fork' && f.message.includes('linwood-estate'),
      ),
    ).toBe(false)
  })
