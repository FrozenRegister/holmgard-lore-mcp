...
- Migrated from `npm` to `pnpm` (with exact version `10.15.0`, per `package.json`'s current `packageManager` field)
...
- [x] Already has `packageManager: pnpm@9` — **Update to `pnpm@10.15.0`** (exact version)
...
1. **Exact pnpm version**: Use `10.15.0` to avoid CI warnings
...
✅ Exact pnpm version (`10.15.0`) in package.json
...
3. Update package.json: `pnpm@10.15.0`
