...
**Multiple agents working in parallel — claim files before editing them:** GitHub has no native file-checkout/lock for source files (`.gitattributes` `lockable` + Git LFS locks are for binary assets, not text source, and most agent tooling doesn't honor them anyway). When you're about to start work on an issue, before writing code: (1) post a comment on the issue listing the files you intend to touch, (2) open a **draft PR early** (as soon as you have a first commit) so its Files-changed tab is the live, authoritative list, and (3) before starting *new* work, check open PRs and recent issue comments for claims that overlap files you're about to modify — if you find one, sequence, split scope, or ask the human via `AskUserQuestion` rather than proceeding blind. Full protocol, including how claims release and how to handle a conflict discovered mid-work: [`docs/file-claim-protocol.md`](./docs/file-claim-protocol.md).
...
- **Stale claims are detected reactively; dead-claimant claims are cleared at tick start.** `resolveTickConflicts` treats an expired `claimed_until` as unclaimed the moment it's checked. Separately, `clearDeadPredatorClaims()` (implemented in #445 Phase 3, run at the top of `creature_ai_tick`) proactively releases any `creature:`-namespaced claim whose claimant no longer has a live `creature_ai_state` row — faction/other-namespaced claims are deliberately left untouched, so a creature must be registered with a `creature:`-prefixed `creature_key` for its dangling claims to be reconciled.
...
**`world_state.world_day` is the tick driver's day-count, and it is a different column from `production_day` (#629).** `resource_consume` and `weather_update` need an absolute day-count integer (`tickAllOwnersDegradation`'s `day` param, `weather_log`'s `(world_id, day)` key) but `runTickDriver`'s only clock input used to be `current_date` (a calendar string) — there was no day-count anywhere in the tick-driver's data model. `production_day` (migration 0013) looked like a candidate but isn't: it belongs to `production-manage.ts`'s opt-in production/broadcast minigame subsystem and is only ever advanced by that module's own `advance_day` action, never by the general `time.advance` tick. `world_day` (migration 0045) is a separate, general-purpose counter advanced by `time-manage.ts`'s `advance` action by the same `days_elapsed` it already computes for `current_date`, and is what `tick-hooks.ts`'s `WorldSnapshot.day` (read via `snapshotWorldState`) and `weather-manage.ts`'s `currentWorldDay()` fallback both read. A world running the production subsystem will have both counters advancing independently and at different paces — do not assume they're interchangeable or reuse one for the other's purpose.
...
**`claude/` is also an accepted branch prefix** (e.g. `claude/next-issues-juu8qd`) — this is the session branch name Claude Code agent sessions are assigned automatically and can't rename, so it's exempted from the prefix check directly rather than requiring `skip-naming-check` on every agent-driven PR. The PR title still must match conventional-commit format regardless of branch prefix.
